           ...
           (0x40000000i32 as
                uint32_t).wrapping_add(0x8000000i32 as
                                           libc::c_uint).wrapping_add(0i32 as
                                                                          libc::c_uint)
               as *mut GPIO_TypeDef {
        RCC_AHBPeriphResetCmd(0x20000i32 as uint32_t, ENABLE);
        RCC_AHBPeriphResetCmd(0x20000i32 as uint32_t, DISABLE);
    } else if GPIOx ==
                  (0x40000000i32 as
                       uint32_t).wrapping_add(0x8000000i32 as
                                                  libc::c_uint).wrapping_add(0x400i32
                                                                                 ...
                                                                                 libc::c_uint)
                      as *mut GPIO_TypeDef {
        RCC_AHBPeriphResetCmd(0x40000i32 as uint32_t, ENABLE);
        RCC_AHBPeriphResetCmd(0x40000i32 as uint32_t, DISABLE);
    } else if GPIOx ==
                  (0x40000000i32 as
                       uint32_t).wrapping_add(0x8000000i32 as
                                                  libc::c_uint).wrapping_add(0x800i32
                                                                                 ...
                                                                                 libc::c_uint)
                      as *mut GPIO_TypeDef {
        RCC_AHBPeriphResetCmd(0x80000i32 as uint32_t, ENABLE);
        RCC_AHBPeriphResetCmd(0x80000i32 as uint32_t, DISABLE);
    } else if GPIOx ==
                  (0x40000000i32 as
                       uint32_t).wrapping_add(0x8000000i32 as
                                                  libc::c_uint).wrapping_add(0xc00i32
                                                                                 ...
                                                                                 libc::c_uint)
                      as *mut GPIO_TypeDef {
        RCC_AHBPeriphResetCmd(0x100000i32 as uint32_t, ENABLE);
        RCC_AHBPeriphResetCmd(0x100000i32 as uint32_t, DISABLE);
    } else if GPIOx ==
                  (0x40000000i32 as
                       uint32_t).wrapping_add(0x8000000i32 as
                                                  libc::c_uint).wrapping_add(0x1000i32
                                                                                 ...
                                                                                 libc::c_uint)
                      as *mut GPIO_TypeDef {
        RCC_AHBPeriphResetCmd(0x200000i32 as uint32_t, ENABLE);
        RCC_AHBPeriphResetCmd(0x200000i32 as uint32_t, DISABLE);
    } else if GPIOx ==
                  (0x40000000i32 as
                       uint32_t).wrapping_add(0x8000000i32 as
                                                  libc::c_uint).wrapping_add(0x1400i32
                                                                                 ...
                                                                                 libc::c_uint)
                      as *mut GPIO_TypeDef {
        RCC_AHBPeriphResetCmd(0x400000i32 as uint32_t, ENABLE);
        RCC_AHBPeriphResetCmd(0x400000i32 as uint32_t, DISABLE);
    };
}
/* *
...
pub unsafe extern "C" fn GPIO_Init(mut GPIOx: *mut GPIO_TypeDef,
    ...
    let mut pinpos: uint32_t = 0i32 as uint32_t;
    let mut pos: uint32_t = 0i32 as uint32_t;
    let mut currentpin: uint32_t = 0i32 as uint32_t;
    let mut tmpreg: uint32_t = 0i32 as uint32_t;
    /* Check the parameters */
    /*-------------------------- Configure the port pins -----------------------*/
  /*-- GPIO Mode Configuration --*/
    pinpos = 0i32 as uint32_t;
    while pinpos < 0x10i32 as libc::c_uint {
        pos = (0x1i32 as uint32_t) << pinpos;
        ...
        if currentpin == pos {
         ...
         unexpected transition in the GPIO pin configuration. */
            ...
            tmpreg &=
                !((0x3i32 as uint32_t) <<
                      pinpos as uint16_t as libc::c_int * 2i32);
            tmpreg |=
                ((*GPIO_InitStruct).GPIO_PuPd as uint32_t) <<
                    pinpos.wrapping_mul(2i32 as libc::c_uint);
...
pub unsafe extern "C" fn GPIO_StructInit(mut GPIO_InitStruct:
    ...
    (*GPIO_InitStruct).GPIO_Pin = 0xffffi32 as uint16_t as uint32_t;
...
pub unsafe extern "C" fn GPIO_PinLockConfig(mut GPIOx: *mut GPIO_TypeDef,
    ...
    let mut tmp: uint32_t = 0x10000i32 as uint32_t;
    /* Check the parameters */
...
pub unsafe extern "C" fn GPIO_ReadInputDataBit(mut GPIOx: *mut GPIO_TypeDef,
                                               mut GPIO_Pin: uint16_t)
 -> uint8_t {
    let mut bitstatus: uint8_t = 0i32 as uint8_t;
    /* Check the parameters */
...
pub unsafe extern "C" fn GPIO_PinAFConfig(mut GPIOx: *mut GPIO_TypeDef,
                                          ...
                                          mut GPIO_AF: uint8_t) {
    let mut temp: uint32_t = 0i32 as uint32_t;
    let mut temp_2: uint32_t = 0i32 as uint32_t;
    /* Check the parameters */
    temp =
        (GPIO_AF as uint32_t) <<
            (GPIO_PinSource as uint32_t &
                 0x7i32 as uint32_t).wrapping_mul(4i32 as libc::c_uint);
    ::core::ptr::write_volatile(&mut (*GPIOx).AFR[(GPIO_PinSource as
                                ...
                                (::core::ptr::read_volatile::<uint32_t>(&(*GPIOx).AFR[(GPIO_PinSource
                                     ...
                                     !((0xfi32 as uint32_t) <<
                                           (GPIO_PinSource as uint32_t &
                                                0x7i32 as
                                                    uint32_t).wrapping_mul(4i32
                                                                               as
                                                                               libc::c_uint)))
                                    as uint32_t as uint32_t);
    temp_2 =
        (*GPIOx).AFR[(GPIO_PinSource as libc::c_int >> 0x3i32) as usize] |
            temp;
    ::core::ptr::write_volatile(&mut (*GPIOx).AFR[(GPIO_PinSource as
                                                       libc::c_int >> 0x3i32)
                                                      as usize] as
