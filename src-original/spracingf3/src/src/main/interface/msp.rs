...
pub const MSP_FLASHFS_FLAG_SUPPORTED: C2RustUnnamed_17 = 2;
pub const MSP_FLASHFS_FLAG_READY: C2RustUnnamed_17 = 1;
...
unsafe extern "C" fn mspFc4waySerialCommand(mut dst: *mut sbuf_t,
    ...
    if dataSize == 0i32 as libc::c_uint {
        // Legacy format
        escMode = 0xffi32 as uint8_t
        ...
        0 | 1 | 2 | 3 | 4 => {
            if (escPortIndex as libc::c_int) < getMotorCount() as libc::c_int
                   ...
                   escMode as libc::c_int == PROTOCOL_KISS as libc::c_int &&
                       escPortIndex as libc::c_int == 255i32 {
                ...
                current_block_16 = 18317007320854588510;
            } else { current_block_16 = 7985875234721069421; }
        }
        _ => { current_block_16 = 7985875234721069421; }
    }
    match current_block_16 {
        7985875234721069421 => { sbufWriteU8(dst, 0i32 as uint8_t); }
        _ => { }
    };
}
...
unsafe extern "C" fn serializeDataflashReadReply(mut dst: *mut sbuf_t,
    ...
    let bytesRemainingInBuf: libc::c_int = sbufBytesRemaining(dst) - 16i32;
    ...
    if compressionMethod as libc::c_int == NO_COMPRESSION as libc::c_int {
        ...
        let READ_BUFFER_SIZE: uint16_t = 256i32 as uint16_t;
        let mut readBuffer: [uint8_t; 256] = [0; 256];
        let mut state: huffmanState_t =
            {
                let mut init =
                    huffmanState_s{bytesWritten: 0i32 as uint16_t,
                                   outByte:
        ...
        *state.outByte = 0i32 as uint8_t;
        let mut bytesReadTotal: uint16_t = 0i32 as uint16_t;
        ...
        if state.outBit as libc::c_int != 0x80i32 {
 ...
 -> bool {
    match cmdMSP as libc::c_int {
        ...
        // Bit 1: 1 iff the board supports software serial
            ...
            commCapabilities =
                (commCapabilities as libc::c_int | 1i32 << 1i32) as uint8_t;
        ...
        5 => {
            sbufWriteData(dst, buildDate as *const libc::c_void, 11i32);
        ...
        160 => {
            sbufWriteU32(dst, *(0x1ffff7aci32 as *mut uint32_t));
            sbufWriteU32(dst, *(0x1ffff7b0i32 as *mut uint32_t));
            sbufWriteU32(dst, *(0x1ffff7b4i32 as *mut uint32_t));
        }
        ...
        128 => {
            ...
            count =
                (count as libc::c_int -
                     (12i32 - getMotorCount() as libc::c_int)) as uint8_t;
        ...
        84 => {
            ...
            osdFlags = (osdFlags as libc::c_int | 1i32 << 0i32) as uint8_t;
...
unsafe extern "C" fn mspProcessOutCommand(mut cmdMSP: uint8_t,
    ...
    match cmdMSP as libc::c_int {
        ...
        58 => {
            sbufWriteU32(dst, rangefinderGetLatestAltitude() as uint32_t);
        }
        38 => {
        ...
        112 => {
            let mut i_8: libc::c_int =
                0i32; // was currentPidProfile->dtermSetpointWeight
        ...
        134 => {
            ...
            } else { unsupportedCommand = 1i32 != 0 }
        }
        240 => {
            sbufWriteU16(dst,
        ...
        127 => {
            ...
            while j_0 < 11i32 {
                sbufWriteU8(dst, 6i32 as uint8_t);
        ...
        88 => {
            ...
            if !vtxDevice.is_null() {
                let mut pitmode: uint8_t = 0i32 as uint8_t;
        ...
        187 => {
            ...
            rtcDateTimeIsSet = 0xffi32 as uint8_t;
...
unsafe extern "C" fn mspFcDataFlashReadCommand(mut dst: *mut sbuf_t,
    ...
    let mut allowCompression: bool = 0i32 != 0;
    ...
    if dataSize as libc::c_ulong >=
        ...
        useLegacyFormat = 0i32 != 0
    } else { readLength = 128i32 as uint16_t; useLegacyFormat = 1i32 != 0 }
 ...
 -> mspResult_e {
    ...
    match cmdMSP as libc::c_int {
        ...
        91 => {
            ...
            (*motorConfigMutable()).dev.motorPwmProtocol =
                constrain(sbufReadU8(src) as libc::c_int, 0i32,
                          PWM_TYPE_MAX as libc::c_int - 1i32) as uint8_t;
            ...
            if !vtxDevice.is_null() {
                if vtxCommonGetDeviceType(vtxDevice) as libc::c_uint !=
                    ...
                    if newFrequency as libc::c_int <=
                           ((7i32 << 3i32) + 7i32) as uint16_t as libc::c_int
                       {
                        let newBand: uint8_t =
                            (newFrequency as libc::c_int / 8i32 + 1i32) as
                                uint8_t;
                        ...
                        (*vtxSettingsConfigMutable()).band = 0i32 as uint8_t;
                    ...
                    if sbufBytesRemaining(src) > 1i32 {
                        ...
                        let mut currentPitmode: uint8_t = 0i32 as uint8_t;
        ...
        99 => {
            ...
            let mut disableRunawayTakeoff: uint8_t = 0i32 as uint8_t;
            if sbufBytesRemaining(src) != 0 {
        ...
        49 => {
            i = sbufReadU8(src) as uint32_t;
            if i >= 32i32 as libc::c_uint ||
                   dataSize != (1i32 + 4i32) as libc::c_uint {
                return MSP_RESULT_ERROR
            }
 ...
 -> mspResult_e {
    ...
    match cmdMSP as libc::c_int {
        ...
        85 => {
            ...
            if addr as int8_t as libc::c_int == -1i32 {
            ...
            } else if addr as int8_t as libc::c_int == -2i32 {
                // Timers
                ...
                if screen as libc::c_int == 0i32 &&
                       (addr as libc::c_int) < OSD_STAT_COUNT as libc::c_int {
                    /* Set statistic item enable */
                    osdStatSetState(addr, value as libc::c_int != 0i32);
 ...
 -> mspResult_e {
    ...
    if mspCommonProcessOutCommand(cmdMSP, dst, mspPostProcessFn) {
        ...
        if !(ret != MSP_RESULT_CMD_UNKNOWN as libc::c_int) {
            if cmdMSP as libc::c_int == 245i32 {
                mspFc4waySerialCommand(dst, src, mspPostProcessFn);
                ret = MSP_RESULT_ACK as libc::c_int
            } else if cmdMSP as libc::c_int == 71i32 {
