...
static mut xBusFrameReceived: bool = 0i32 != 0;
static mut xBusDataIncoming: bool = 0i32 != 0;
...
unsafe extern "C" fn xBusRj01CRC8(mut inData: uint8_t, mut seed: uint8_t)
 -> uint8_t {
    let mut bitsLeft: uint8_t = 8i32 as uint8_t;
    while bitsLeft as libc::c_int > 0i32 {
        let temp: uint8_t =
            ((seed as libc::c_int ^ inData as libc::c_int) & 0x1i32) as
                uint8_t;
        if temp as libc::c_int == 0i32 {
            seed = (seed as libc::c_int >> 1i32) as uint8_t
        } else {
            seed = (seed as libc::c_int ^ 0x18i32) as uint8_t;
            seed = (seed as libc::c_int >> 1i32) as uint8_t;
            seed = (seed as libc::c_int | 0x80i32) as uint8_t
        }
        inData = (inData as libc::c_int >> 1i32) as uint8_t;
...
unsafe extern "C" fn xBusUnpackModeBFrame(mut offsetBytes: uint8_t) {
    ...
    let inCrc: uint16_t =
        crc16_ccitt_update(0i32 as uint16_t,
                           &mut *xBusFrame.as_mut_ptr().offset(offsetBytes as
                                                                   isize) as
                               *mut uint8_t as *mut uint8_t as
                               *const libc::c_void,
                           (xBusFrameLength as libc::c_int - 2i32) as
                               uint32_t);
    // Get the received CRC
    let crc: uint16_t =
        (((xBusFrame[(offsetBytes as libc::c_int +
                          xBusFrameLength as libc::c_int - 2i32) as usize] as
               uint16_t as libc::c_int) << 8i32) +
             xBusFrame[(offsetBytes as libc::c_int +
                            xBusFrameLength as libc::c_int - 1i32) as usize]
    ...
    if crc as libc::c_int == inCrc as libc::c_int {
        ...
        while i < xBusChannelCount as libc::c_int {
            let frameAddr: uint8_t =
                (offsetBytes as libc::c_int + 1i32 + i * 2i32) as uint8_t;
            let mut value: uint16_t =
                ((xBusFrame[frameAddr as usize] as uint16_t as libc::c_int) <<
                     8i32) as uint16_t;
            value =
                (value as libc::c_int +
                     xBusFrame[(frameAddr as libc::c_int + 1i32) as usize] as
                         uint16_t as libc::c_int) as uint16_t;
            // Convert to internal format
            xBusChannelData[i as usize] =
                (800i32 + (value as libc::c_int * 1400i32 >> 12i32)) as
                    uint16_t;
            i += 1
        }
        xBusFrameReceived = 1i32 != 0
    };
}
unsafe extern "C" fn xBusUnpackRJ01Frame() {
    // Calculate the CRC of the incoming frame
    let mut outerCrc: uint8_t = 0i32 as uint8_t;
    ...
    //
    if xBusFrame[1] as libc::c_int != 30i32 {
    ...
    //
    i = 0i32 as uint8_t;
    while (i as libc::c_int) < xBusFrameLength as libc::c_int - 1i32 {
        outerCrc = xBusRj01CRC8(outerCrc, xBusFrame[i as usize]);
        i = i.wrapping_add(1)
    }
    if outerCrc as libc::c_int !=
           xBusFrame[(xBusFrameLength as libc::c_int - 1i32) as usize] as
               libc::c_int {
        // CRC does not match, skip this frame
        return
    }
    // Now unpack the "embedded MODE B frame"
    xBusUnpackModeBFrame(3i32 as uint8_t);
}
...
unsafe extern "C" fn xBusDataReceive(mut c: uint16_t,
    ...
    xBusTimeLast = now;
    if xBusTimeInterval > 8000i32 as libc::c_uint {
        xBusFramePosition = 0i32 as uint8_t;
        xBusDataIncoming = 0i32 != 0
    }
    // Check if we shall start a frame?
    if xBusFramePosition as libc::c_int == 0i32 {
        if c as libc::c_int == 0xa1i32 {
            xBusDataIncoming = 1i32 != 0;
            xBusFrameLength = 27i32 as uint8_t
            //decrease framesize (when receiver change, otherwise board must reboot)
        } else if c as libc::c_int == 0xa2i32 {
            //16channel packet
            xBusDataIncoming = 1i32 != 0;
            xBusFrameLength = 35i32 as uint8_t
            //increase framesize
        }
    }
    // Only do this if we are receiving to a frame
    if xBusDataIncoming as libc::c_int == 1i32 {
        // Store in frame copy
    ...
    if xBusFramePosition as libc::c_int == xBusFrameLength as libc::c_int {
        ...
        match xBusProvider as libc::c_int {
            5 => {
                xBusUnpackModeBFrame(0i32 as uint8_t);
 ...
 -> bool {
    ...
    match (*rxConfig).serialrx_provider as libc::c_int {
        5 => {
            ...
            xBusDataIncoming = 0i32 != 0;
            xBusFramePosition = 0i32 as uint8_t;
            baudRate = 115200i32 as uint32_t;
            xBusFrameLength = 27i32 as uint8_t;
            xBusChannelCount = 12i32 as uint8_t;
        ...
        6 => {
            ...
            xBusDataIncoming = 0i32 != 0;
            xBusFramePosition = 0i32 as uint8_t;
            baudRate = 250000i32 as uint32_t;
            xBusFrameLength = 33i32 as uint8_t;
            xBusChannelCount = 12i32 as uint8_t;
