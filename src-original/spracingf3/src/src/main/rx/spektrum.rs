...
#[no_mangle]
pub static mut srxlEnabled: bool = 0i32 != 0;
#[no_mangle]
...
static mut rcFrameComplete: bool = 0i32 != 0;
static mut spekHiRes: bool = 0i32 != 0;
...
static mut telemetryBufLen: uint8_t = 0i32 as uint8_t;
...
unsafe extern "C" fn spektrumDataReceive(mut c: uint16_t,
    ...
    static mut spekTimeLast: uint32_t = 0i32 as uint32_t;
    static mut spekFramePosition: uint8_t = 0i32 as uint8_t;
    spekTime = micros();
    spekTimeInterval = spekTime.wrapping_sub(spekTimeLast);
    spekTimeLast = spekTime;
    if spekTimeInterval > 5000i32 as libc::c_uint {
        spekFramePosition = 0i32 as uint8_t
    }
    if (spekFramePosition as libc::c_int) < 16i32 {
        ...
        if (spekFramePosition as libc::c_int) < 16i32 {
            rcFrameComplete = 0i32 != 0
        } else { rcFrameComplete = 1i32 != 0 }
    };
}
#[no_mangle]
...
unsafe extern "C" fn spektrumFrameStatus(mut rxRuntimeConfig:
 ...
 -> uint8_t {
    ...
    let mut vtxControl: uint32_t =
        ((spekFrame[(16i32 - 4i32) as usize] as libc::c_int) << 24i32 |
             (spekFrame[(16i32 - 3i32) as usize] as libc::c_int) << 16i32 |
             (spekFrame[(16i32 - 2i32) as usize] as libc::c_int) << 8i32 |
             (spekFrame[(16i32 - 1i32) as usize] as libc::c_int) << 0i32) as
            uint32_t;
    let mut spektrumRcDataSize: int8_t = 0;
    // Handle VTX control frame.
    if vtxControl & 0xf000f000u32 == 0xe000e000u32 &&
           spekFrame[2] as libc::c_int & 0x80i32 == 0i32 {
        spektrumHandleVtxControl(vtxControl);
        spektrumRcDataSize = (16i32 - 4i32) as int8_t
    } else { spektrumRcDataSize = 16i32 as int8_t }
    // Get the RC control channel inputs
    let mut b: libc::c_int = 3i32;
    while b < spektrumRcDataSize as libc::c_int {
        let spekChannel: uint8_t =
            (0xfi32 &
                 spekFrame[(b - 1i32) as usize] as libc::c_int >>
                     spek_chan_shift as libc::c_int) as uint8_t;
        if (spekChannel as libc::c_int) <
               (*rxRuntimeConfigPtr).channelCount as libc::c_int &&
               (spekChannel as libc::c_int) < 12i32 {
            if rssi_channel as libc::c_int == 0i32 ||
                ...
                spekChannelData[spekChannel as usize] =
                    (((spekFrame[(b - 1i32) as usize] as libc::c_int &
                           spek_chan_mask as libc::c_int) as uint32_t) <<
                         8i32).wrapping_add(spekFrame[b as usize] as
                                                libc::c_uint)
            }
        }
        b += 2i32
...
pub unsafe extern "C" fn spekShouldBind(mut spektrum_sat_bind: uint8_t)
 -> bool {
    ...
    if !BindPlug.is_null() {
        IOInit(BindPlug, OWNER_RX_BIND, 0i32 as uint8_t);
        ...
        delayMicroseconds(10i32 as timeUs_t); // allow configuration to settle
        if IORead(BindPlug) { return 0i32 != 0 }
    }
    // USE_SPEKTRUM_BIND_PLUG
    return !(isMPUSoftReset() as libc::c_int != 0 ||
                 spektrum_sat_bind as libc::c_int == 0i32 ||
                 spektrum_sat_bind as libc::c_int > 10i32);
}
...
pub unsafe extern "C" fn spektrumBind(mut rxConfig_0: *mut rxConfig_t) {
    ...
    let mut bindPin: ioTag_t = 0i32 as ioTag_t;
    ...
    IOInit(bindIO, OWNER_RX_BIND, 0i32 as uint8_t);
    IOConfigGPIO(bindIO,
    ...
    ledSet(1i32, 1i32 != 0);
    // RX line, set high
    IOWrite(bindIO, 1i32 != 0);
    // Bind window is around 20-140ms after powerup
    delay(60i32 as timeMs_t);
    ledSet(1i32, 0i32 != 0);
    ...
    while i < (*rxConfig_0).spektrum_sat_bind as libc::c_int {
        ...
        IOWrite(bindIO, 0i32 != 0);
        delayMicroseconds(120i32 as timeUs_t);
        ledSet(0i32, 1i32 != 0);
        ledSet(2i32, 1i32 != 0);
    ...
    if (*rxConfig_0).spektrum_bind_pin_override_ioTag != 0 {
        delay(50i32 as timeMs_t); // Keep it high for 50msec
        IOConfigGPIO(bindIO,
                     (GPIO_Mode_IN as libc::c_int | 0i32 << 2i32 |
                          0i32 << 4i32 |
                          (GPIO_PuPd_NOPULL as libc::c_int) << 5i32) as
    ...
    if (*rxConfig_0).spektrum_bind_plug_ioTag == 0 &&
           (*rxConfig_0).spektrum_sat_bind_autoreset as libc::c_int == 1i32 &&
           !isMPUSoftReset() {
        (*rxConfig_0).spektrum_sat_bind = 0i32 as uint8_t;
        saveConfigAndNotify();
    };
}
// USE_SPEKTRUM_BIND
...
pub unsafe extern "C" fn spektrumInit(mut rxConfig_0: *const rxConfig_t,
    ...
    match (*rxConfig_0).serialrx_provider as libc::c_int {
        10 => {
            ...
            current_block_19 = 2845071489008852615;
        }
        1 => { current_block_19 = 2845071489008852615; }
        0 => {
            // 10 bit frames
            spek_chan_shift = 2i32 as uint8_t;
            spek_chan_mask = 0x3i32 as uint8_t;
            spekHiRes = 0i32 != 0;
            resolution = 1024i32;
            (*rxRuntimeConfig).channelCount = 7i32 as uint8_t;
            (*rxRuntimeConfig).rxRefreshRate = 22000i32 as uint16_t;
    ...
    match current_block_19 {
        2845071489008852615 => {
            ...
            spek_chan_mask = 0x7i32 as uint8_t;
            spekHiRes = 1i32 != 0;
            resolution = 2048i32;
    ...
    rssi_channel =
        ((*rxConfig_0).rssi_channel as libc::c_int - 1i32) as uint8_t;
...
pub unsafe extern "C" fn srxlRxSendTelemetryDataDispatch(mut self_0:
    ...
    if telemetryBufLen as libc::c_int > 0i32 {
        ...
        telemetryBufLen = 0i32 as uint8_t
