...
/* Exported functions ------------------------------------------------------- */
    ...
    #[no_mangle]
    fn TIM_DMAConfig(TIMx: *mut TIM_TypeDef, TIM_DMABase: uint16_t,
                     TIM_DMABurstLength: uint16_t);
    #[no_mangle]
...
pub type loadDmaBufferFn
    ...
    unsafe extern "C" fn(_: *mut uint32_t, _: libc::c_int, _: uint16_t)
        -> uint8_t;
pub type dmaStream_t = DMA_Channel_TypeDef;
 ...
 * If not, see <http://www.gnu.org/licenses/>.
 */
static mut dmaMotorTimerCount: uint8_t = 0i32 as uint8_t;
...
pub unsafe extern "C" fn getTimerIndex(mut timer: *mut TIM_TypeDef)
 -> uint8_t {
    ...
    return (dmaMotorTimerCount as libc::c_int - 1i32) as uint8_t;
}
#[no_mangle]
pub unsafe extern "C" fn pwmWriteDshotInt(mut index: uint8_t,
    ...
    if useBurstDshot {
        ...
        (*(*motor).timer).dmaBurstLength =
            (bufferSize as libc::c_int * 4i32) as uint16_t
    } else {
        bufferSize =
        ...
        DMA_SetCurrDataCounter((*(*motor).timerHardware).dmaRef,
                               bufferSize as uint16_t);
...
pub unsafe extern "C" fn pwmCompleteDshotMotorUpdate(mut motorCount:
    ...
    while i < dmaMotorTimerCount as libc::c_int {
        if useBurstDshot {
            DMA_SetCurrDataCounter(dmaMotorTimers[i as usize].dmaBurstRef,
                                   dmaMotorTimers[i as usize].dmaBurstLength);
            DMA_Cmd(dmaMotorTimers[i as usize].dmaBurstRef, ENABLE);
            TIM_DMAConfig(dmaMotorTimers[i as usize].timer,
                          0xdi32 as uint16_t, 0x300i32 as uint16_t);
            TIM_DMACmd(dmaMotorTimers[i as usize].timer, 0x100i32 as uint16_t,
                       ENABLE);
        } else {
            TIM_SetCounter(dmaMotorTimers[i as usize].timer,
                           0i32 as uint32_t);
            TIM_DMACmd(dmaMotorTimers[i as usize].timer,
                       dmaMotorTimers[i as usize].timerDmaSources, ENABLE);
            dmaMotorTimers[i as usize].timerDmaSources = 0i32 as uint16_t
        }
        i += 1
    };
}
unsafe extern "C" fn motor_DMA_IRQHandler(mut descriptor:
    ...
    if (*(*descriptor).dma).ISR &
        ...
        if useBurstDshot {
            ...
            TIM_DMACmd((*(*motor).timerHardware).tim, 0x100i32 as uint16_t,
                       DISABLE);
        } else {
            DMA_Cmd((*(*motor).timerHardware).dmaRef, DISABLE);
            TIM_DMACmd((*(*motor).timerHardware).tim, (*motor).timerDmaSource,
                       DISABLE);
        }
...
pub unsafe extern "C" fn pwmDshotMotorHardwareConfig(mut timerHardware:
                                                     ...
                                                     mut output: uint8_t) {
    let mut dmaRef: *mut dmaStream_t = 0 as *mut dmaStream_t;
    if useBurstDshot {
    ...
    let timerIndex: uint8_t = getTimerIndex(timer); // XXX
    let configureTimer: bool =
        timerIndex as libc::c_int == dmaMotorTimerCount as libc::c_int - 1i32;
    ...
    if useBurstDshot {
        ...
        if !configureTimer { (*motor).configured = 1i32 != 0; return }
    } else {
    ...
    if useBurstDshot {
                                ...
                                0xfi32 >>
                                    (7i32 as
                                         libc::c_uint).wrapping_sub(0x500i32
                                                                        ...
                                                                        >>
                                                                        8i32))
                           << 4i32 & 0xf0i32) as uint32_t,
                      motorIndex as uint32_t);
        DMA_InitStructure.DMA_MemoryBaseAddr =
            (*(*motor).timer).dmaBurstBuffer.as_mut_ptr() as uint32_t;
        DMA_InitStructure.DMA_DIR = 0x10i32 as uint32_t;
        DMA_InitStructure.DMA_PeripheralBaseAddr =
            &mut (*(*timerHardware).tim).DMAR as *mut uint16_t as uint32_t;
        DMA_InitStructure.DMA_BufferSize =
            if pwmProtocolType as libc::c_uint ==
                   PWM_TYPE_PROSHOT1000 as libc::c_int as libc::c_uint {
                6i32
            } else { 18i32 } as uint16_t;
        ...
        DMA_InitStructure.DMA_MemoryDataSize = 0x800i32 as uint32_t;
        ...
        dmaInit((*timerHardware).dmaIrqHandler as dmaIdentifier_e,
                OWNER_MOTOR, (motorIndex as libc::c_int + 1i32) as uint8_t);
        ...
        DMA_InitStructure.DMA_PeripheralBaseAddr =
            timerChCCR(timerHardware) as uint32_t;
        DMA_InitStructure.DMA_BufferSize =
            if pwmProtocolType as libc::c_uint ==
                   PWM_TYPE_PROSHOT1000 as libc::c_int as libc::c_uint {
                6i32
