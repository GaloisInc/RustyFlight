...
pub unsafe extern "C" fn DMA1_Channel1_IRQHandler() {
    let index: uint8_t = (DMA1_CH1_HANDLER as libc::c_int - 1i32) as uint8_t;
...
pub unsafe extern "C" fn DMA1_Channel2_IRQHandler() {
    let index: uint8_t = (DMA1_CH2_HANDLER as libc::c_int - 1i32) as uint8_t;
    if dmaDescriptors[index as usize].irqHandlerCallback.is_some() {
        dmaDescriptors[index as
                           usize].irqHandlerCallback.expect("non-null function pointer")(&mut *dmaDescriptors.as_mut_ptr().offset(index
                                                                                                                                      as
...
pub unsafe extern "C" fn DMA1_Channel3_IRQHandler() {
    let index: uint8_t = (DMA1_CH3_HANDLER as libc::c_int - 1i32) as uint8_t;
    if dmaDescriptors[index as usize].irqHandlerCallback.is_some() {
        dmaDescriptors[index as
                           usize].irqHandlerCallback.expect("non-null function pointer")(&mut *dmaDescriptors.as_mut_ptr().offset(index
                                                                                                                                      as
...
pub unsafe extern "C" fn DMA1_Channel4_IRQHandler() {
    let index: uint8_t = (DMA1_CH4_HANDLER as libc::c_int - 1i32) as uint8_t;
    if dmaDescriptors[index as usize].irqHandlerCallback.is_some() {
        dmaDescriptors[index as
                           usize].irqHandlerCallback.expect("non-null function pointer")(&mut *dmaDescriptors.as_mut_ptr().offset(index
                                                                                                                                      as
...
pub unsafe extern "C" fn DMA1_Channel5_IRQHandler() {
    let index: uint8_t = (DMA1_CH5_HANDLER as libc::c_int - 1i32) as uint8_t;
    if dmaDescriptors[index as usize].irqHandlerCallback.is_some() {
        dmaDescriptors[index as
                           usize].irqHandlerCallback.expect("non-null function pointer")(&mut *dmaDescriptors.as_mut_ptr().offset(index
                                                                                                                                      as
...
pub unsafe extern "C" fn DMA1_Channel6_IRQHandler() {
    let index: uint8_t = (DMA1_CH6_HANDLER as libc::c_int - 1i32) as uint8_t;
    if dmaDescriptors[index as usize].irqHandlerCallback.is_some() {
        dmaDescriptors[index as
                           usize].irqHandlerCallback.expect("non-null function pointer")(&mut *dmaDescriptors.as_mut_ptr().offset(index
                                                                                                                                      as
...
pub unsafe extern "C" fn DMA1_Channel7_IRQHandler() {
    let index: uint8_t = (DMA1_CH7_HANDLER as libc::c_int - 1i32) as uint8_t;
    if dmaDescriptors[index as usize].irqHandlerCallback.is_some() {
        dmaDescriptors[index as
                           usize].irqHandlerCallback.expect("non-null function pointer")(&mut *dmaDescriptors.as_mut_ptr().offset(index
                                                                                                                                      as
...
pub unsafe extern "C" fn DMA2_Channel1_IRQHandler() {
    let index: uint8_t = (DMA2_CH1_HANDLER as libc::c_int - 1i32) as uint8_t;
    if dmaDescriptors[index as usize].irqHandlerCallback.is_some() {
        dmaDescriptors[index as
                           usize].irqHandlerCallback.expect("non-null function pointer")(&mut *dmaDescriptors.as_mut_ptr().offset(index
                                                                                                                                      as
...
pub unsafe extern "C" fn DMA2_Channel2_IRQHandler() {
    let index: uint8_t = (DMA2_CH2_HANDLER as libc::c_int - 1i32) as uint8_t;
    if dmaDescriptors[index as usize].irqHandlerCallback.is_some() {
        dmaDescriptors[index as
                           usize].irqHandlerCallback.expect("non-null function pointer")(&mut *dmaDescriptors.as_mut_ptr().offset(index
                                                                                                                                      as
...
pub unsafe extern "C" fn DMA2_Channel3_IRQHandler() {
    let index: uint8_t = (DMA2_CH3_HANDLER as libc::c_int - 1i32) as uint8_t;
    if dmaDescriptors[index as usize].irqHandlerCallback.is_some() {
        dmaDescriptors[index as
                           usize].irqHandlerCallback.expect("non-null function pointer")(&mut *dmaDescriptors.as_mut_ptr().offset(index
                                                                                                                                      as
...
pub unsafe extern "C" fn DMA2_Channel4_IRQHandler() {
    let index: uint8_t = (DMA2_CH4_HANDLER as libc::c_int - 1i32) as uint8_t;
    if dmaDescriptors[index as usize].irqHandlerCallback.is_some() {
        dmaDescriptors[index as
                           usize].irqHandlerCallback.expect("non-null function pointer")(&mut *dmaDescriptors.as_mut_ptr().offset(index
                                                                                                                                      as
...
pub unsafe extern "C" fn DMA2_Channel5_IRQHandler() {
    let index: uint8_t = (DMA2_CH5_HANDLER as libc::c_int - 1i32) as uint8_t;
    if dmaDescriptors[index as usize].irqHandlerCallback.is_some() {
        dmaDescriptors[index as
                           usize].irqHandlerCallback.expect("non-null function pointer")(&mut *dmaDescriptors.as_mut_ptr().offset(index
                                                                                                                                      as
           ...
           (0x40000000i32 as
                uint32_t).wrapping_add(0x20000i32 as
                                           libc::c_uint).wrapping_add(0x6ci32
                                                                          ...
                                                                          libc::c_uint)
               as *mut DMA_Channel_TypeDef as *const DMA_Channel_TypeDef {
        return 0x200000i32 as uint32_t
    }
    if channel ==
           (0x40000000i32 as
                uint32_t).wrapping_add(0x20000i32 as
                                           libc::c_uint).wrapping_add(0x80i32
                                                                          ...
                                                                          libc::c_uint)
               as *mut DMA_Channel_TypeDef as *const DMA_Channel_TypeDef {
        return 0x2000000i32 as uint32_t
    }
    if channel ==
           (0x40000000i32 as
                uint32_t).wrapping_add(0x20000i32 as
                                           libc::c_uint).wrapping_add(0x408i32
                                                                          ...
                                                                          libc::c_uint)
               as *mut DMA_Channel_TypeDef as *const DMA_Channel_TypeDef {
        return 0x10000002i32 as uint32_t
    }
    if channel ==
           (0x40000000i32 as
                uint32_t).wrapping_add(0x20000i32 as
                                           libc::c_uint).wrapping_add(0x41ci32
                                                                          as
                                                                          libc::c_uint)
               as *mut DMA_Channel_TypeDef as *const DMA_Channel_TypeDef {
        return 0x10000020i32 as uint32_t
    }
    if channel ==
           (0x40000000i32 as
                uint32_t).wrapping_add(0x20000i32 as
                                           libc::c_uint).wrapping_add(0x430i32
                                                                          as
                                                                          libc::c_uint)
               as *mut DMA_Channel_TypeDef as *const DMA_Channel_TypeDef {
        return 0x10000200i32 as uint32_t
    }
    if channel ==
           (0x40000000i32 as
                uint32_t).wrapping_add(0x20000i32 as
                                           libc::c_uint).wrapping_add(0x444i32
                                                                          as
                                                                          libc::c_uint)
               as *mut DMA_Channel_TypeDef as *const DMA_Channel_TypeDef {
        return 0x10002000i32 as uint32_t
    }
    if channel ==
           (0x40000000i32 as
                uint32_t).wrapping_add(0x20000i32 as
                                           libc::c_uint).wrapping_add(0x458i32
                                                                          ...
                                                                          libc::c_uint)
               as *mut DMA_Channel_TypeDef as *const DMA_Channel_TypeDef {
        return 0x10020000i32 as uint32_t
    }
...
pub unsafe extern "C" fn dmaInit(mut identifier: dmaIdentifier_e,
    ...
    let index: libc::c_int =
        (identifier as libc::c_uint).wrapping_sub(1i32 as libc::c_uint) as
            libc::c_int;
    RCC_AHBPeriphClockCmd(if dmaDescriptors[index as usize].dma ==
                                 (0x40000000i32 as
                                     ...
                                     as *mut DMA_TypeDef {
                              0x1i32 as uint32_t
                          } else { 0x2i32 as uint32_t }, ENABLE);
 ...
 -> dmaIdentifier_e {
    ...
    while i < DMA_LAST_HANDLER as libc::c_int {
        if dmaDescriptors[i as usize].ref_0 ==
            ...
            return (i + 1i32) as dmaIdentifier_e
        }
        i += 1
    }
    return DMA_NONE;
        ...
        [{
             let mut init =
                 dmaChannelDescriptor_s{dma:
                                        ...
                                        irqHandlerCallback: None,
                                        flagsShift: 0i32 as uint8_t,
                                        irqN: DMA1_Channel1_IRQn,
                                        userParam: 0i32 as uint32_t,
                                        owner: OWNER_FREE,
