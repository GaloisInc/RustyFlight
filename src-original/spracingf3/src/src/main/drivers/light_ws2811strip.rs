 ...
 */
#[no_mangle]
pub static mut ledStripDMABuffer: [uint8_t; 810] = [0; 810];
#[no_mangle]
pub static mut ws2811LedDataTransferInProgress: uint8_t = 0i32 as uint8_t;
#[no_mangle]
pub static mut BIT_COMPARE_1: uint16_t = 0i32 as uint16_t;
#[no_mangle]
pub static mut BIT_COMPARE_0: uint16_t = 0i32 as uint16_t;
...
pub unsafe extern "C" fn scaleLedValue(mut index: uint16_t,
    ...
    ledColorBuffer[index as usize].v =
        (ledColorBuffer[index as usize].v as uint16_t as libc::c_int *
             scalePercent as libc::c_int / 100i32) as uint8_t;
}
#[no_mangle]
pub unsafe extern "C" fn setStripColor(mut color: *const hsvColor_t) {
    let mut index: uint16_t = 0;
    index = 0i32 as uint16_t;
    while (index as libc::c_int) < 32i32 {
...
pub unsafe extern "C" fn ws2811LedStripInit(mut ioTag: ioTag_t) {
    memset(ledStripDMABuffer.as_mut_ptr() as *mut libc::c_void, 0i32,
           ::core::mem::size_of::<[uint8_t; 810]>() as libc::c_ulong);
...
unsafe extern "C" fn fastUpdateLEDDMABuffer(mut ledFormat:
    ...
    match ledFormat as libc::c_uint {
        1 => {
            ...
            packed_colour =
                (((*color).rgb.r as libc::c_int) << 16i32 |
                     ((*color).rgb.g as libc::c_int) << 8i32 |
                     (*color).rgb.b as libc::c_int) as uint32_t
        }
        0 | _ => {
            // WS2812 drivers use GRB format
            packed_colour =
                (((*color).rgb.g as libc::c_int) << 16i32 |
                     ((*color).rgb.r as libc::c_int) << 8i32 |
                     (*color).rgb.b as libc::c_int) as uint32_t
        }
    }
    let mut index: int8_t = 23i32 as int8_t;
    while index as libc::c_int >= 0i32 {
        let fresh1 = dmaBufferOffset;
        dmaBufferOffset = dmaBufferOffset.wrapping_add(1);
        ledStripDMABuffer[fresh1 as usize] =
            if packed_colour & (1i32 << index as libc::c_int) as libc::c_uint
                   != 0 {
                BIT_COMPARE_1 as libc::c_int
            } else { BIT_COMPARE_0 as libc::c_int } as uint8_t;
        index -= 1
    };
}
/*
...
pub unsafe extern "C" fn ws2811UpdateStrip(mut ledFormat:
    ...
    dmaBufferOffset = 0i32 as uint16_t; // reset led index
    ledIndex = 0i32 as int16_t;
    // fill transmit buffer with correct compare values to achieve
    // correct pulse widths according to color values
    while (ledIndex as libc::c_int) < 32i32 {
        rgb24 =
