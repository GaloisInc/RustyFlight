...
static mut reverseMotors: bool = 0i32 != 0;
static mut applyRates:
...
unsafe extern "C" fn rcLookupThrottle(mut tmp: int32_t) -> int16_t {
    let tmp2: int32_t = tmp / 100i32;
    // [0;1000] -> expo -> [MINTHROTTLE;MAXTHROTTLE]
    return (lookupThrottleRC[tmp2 as usize] as libc::c_int +
                (tmp - tmp2 * 100i32) *
                    (lookupThrottleRC[(tmp2 + 1i32) as usize] as libc::c_int -
    ...
    if (*currentControlRateProfile).rcExpo[axis as usize] != 0 {
        ...
        rcCommandf =
            rcCommandf * (rcCommandfAbs * rcCommandfAbs * rcCommandfAbs) *
                expof + rcCommandf * (1i32 as libc::c_float - expof)
    }
...
unsafe extern "C" fn scaleRcCommandToFpvCamAngle() {
    ...
    static mut lastFpvCamAngleDegrees: uint8_t = 0i32 as uint8_t;
...
unsafe extern "C" fn checkForThrottleErrorResetState(mut rxRefreshRate:
    ...
    let rxRefreshRateMs: libc::c_int = rxRefreshRate as libc::c_int / 1000i32;
    let indexMax: libc::c_int =
        constrain(100i32 / rxRefreshRateMs, 1i32, 20i32);
...
pub unsafe extern "C" fn processRcInterpolation() -> uint8_t {
    ...
    let mut updatedChannel: uint8_t = 0i32 as uint8_t;
    if (*rxConfig()).rcInterpolation != 0 {
        // Set RC refresh rate for sampling and channels to filter
        match (*rxConfig()).rcInterpolation as libc::c_int {
            2 => {
                rxRefreshRate =
                    (currentRxRefreshRate as libc::c_int + 1000i32) as
                        uint16_t
            }
            3 => {
                rxRefreshRate =
        ...
        if isRXDataNew as libc::c_int != 0 &&
               rxRefreshRate as libc::c_int > 0i32 {
            rcInterpolationStepCount =
                (rxRefreshRate as
                     libc::c_uint).wrapping_div(targetPidLooptime) as int16_t;
            let mut channel: libc::c_int = 0i32;
            while channel < THROTTLE as libc::c_int + 1i32 {
                if 1i32 << channel & interpolationChannels as libc::c_int != 0
                   {
            ...
            if debugMode as libc::c_int ==
                ...
                debug[0] = lrintf(rcCommand[0]) as int16_t
            }
            if debugMode as libc::c_int ==
                   DEBUG_RC_INTERPOLATION as libc::c_int {
                debug[1] =
                    lrintf((currentRxRefreshRate as libc::c_int / 1000i32) as
                               libc::c_float) as int16_t
            }
        } else { rcInterpolationStepCount -= 1 }
        // Interpolate steps of rcCommand
        if rcInterpolationStepCount as libc::c_int > 0i32 {
            updatedChannel = 0i32 as uint8_t;
            while (updatedChannel as libc::c_int) <
                      THROTTLE as libc::c_int + 1i32 {
                if 1i32 << updatedChannel as libc::c_int &
        ...
        rcInterpolationStepCount = 0i32 as int16_t
...
pub unsafe extern "C" fn rcSmoothingRxRateValid(mut currentRxRefreshRate_0:
                                                    libc::c_int) -> bool {
    return currentRxRefreshRate_0 >= 1000i32 &&
               currentRxRefreshRate_0 <= 50000i32;
}
...
pub unsafe extern "C" fn rcSmoothingSetFilterCutoffs(mut smoothingData:
    ...
    if (*smoothingData).inputCutoffFrequency as libc::c_int !=
        ...
        while i < THROTTLE as libc::c_int + 1i32 {
            if 1i32 << i & interpolationChannels as libc::c_int != 0 {
    ...
    if (*rxConfig()).rc_smoothing_derivative_cutoff as libc::c_int == 0i32 &&
...
pub unsafe extern "C" fn rcSmoothingResetAccumulation(mut smoothingData:
                                                          *mut rcSmoothingFilter_t) {
    (*smoothingData).training.sum = 0i32 as libc::c_float;
    (*smoothingData).training.count = 0i32;
    (*smoothingData).training.min = 65535i32 as uint16_t;
    (*smoothingData).training.max = 0i32 as uint16_t;
}
...
pub unsafe extern "C" fn rcSmoothingAccumulateSample(mut smoothingData:
 ...
 -> bool {
    ...
    if (*smoothingData).training.count >= 50i32 {
        ...
        (*smoothingData).averageFrameTimeUs =
            lrintf((*smoothingData).training.sum /
                       ((*smoothingData).training.count - 2i32) as
...
pub unsafe extern "C" fn processRcSmoothingFilter() -> uint8_t {
    ...
    if !initialized {
        initialized = 1i32 != 0;
        rcSmoothingData.filterInitialized = 0i32 != 0;
        rcSmoothingData.averageFrameTimeUs = 0i32;
        ...
        if !calculateCutoffs {
            ...
            rcSmoothingData.filterInitialized = 1i32 != 0
        }
    }
    if isRXDataNew {
        ...
        if calculateCutoffs {
            ...
            let mut sampleState: libc::c_int = 0i32;
            ...
            if currentTimeMs > 5000i32 as libc::c_uint &&
                   targetPidLooptime > 0i32 as libc::c_uint {
                ...
                if rxIsReceivingSignal() as libc::c_int != 0 &&
                    ...
                    if validRxFrameTimeMs == 0i32 as libc::c_uint {
                        validRxFrameTimeMs =
                    ...
                    if currentTimeMs > validRxFrameTimeMs {
                        ...
                        if rcSmoothingData.filterInitialized {
                            ...
                            if percentChange < 20i32 as libc::c_float {
                        ...
                        if accumulateSample {
                            if rcSmoothingAccumulateSample(&mut rcSmoothingData,
                                                               ...
                                                               as libc::c_int)
                               {
                                ...
                                rcSmoothingSetFilterCutoffs(&mut rcSmoothingData);
                                rcSmoothingData.filterInitialized = 1i32 != 0;
                                validRxFrameTimeMs = 0i32 as timeMs_t
                            }
                        }
                    }
                } else {
            ...
            if debugMode as libc::c_int ==
                ...
                if debugMode as libc::c_int ==
                       DEBUG_RC_SMOOTHING_RATE as libc::c_int {
                    debug[0] = currentRxRefreshRate as int16_t
                }
                // indicates whether guard time is active
                if debugMode as libc::c_int ==
                       DEBUG_RC_SMOOTHING_RATE as libc::c_int {
                    debug[1] = rcSmoothingData.training.count as int16_t
                }
                if debugMode as libc::c_int ==
                       DEBUG_RC_SMOOTHING_RATE as libc::c_int {
                    debug[2] = rcSmoothingData.averageFrameTimeUs as int16_t
                }
                if debugMode as libc::c_int ==
                       DEBUG_RC_SMOOTHING_RATE as libc::c_int {
                    debug[3] = sampleState as int16_t
                }
            }
        }
    }
    if rcSmoothingData.filterInitialized as libc::c_int != 0 &&
        ...
        if debugMode as libc::c_int == DEBUG_RC_SMOOTHING as libc::c_int {
            debug[3] = rcSmoothingData.averageFrameTimeUs as int16_t
        }
    }
    // each pid loop continue to apply the last received channel value to the filter
    updatedChannel = 0i32 as uint8_t;
    while (updatedChannel as libc::c_int) < THROTTLE as libc::c_int + 1i32 {
        if 1i32 << updatedChannel as libc::c_int &
               interpolationChannels as libc::c_int != 0 {
...
pub unsafe extern "C" fn processRcCommand() {
    ...
    if isRXDataNew as libc::c_int != 0 || updatedChannel as libc::c_int != 0 {
        ...
        if debugMode as libc::c_int == DEBUG_RC_INTERPOLATION as libc::c_int {
            debug[3] = setpointRate[0] as int16_t
        }
    ...
    if isRXDataNew { isRXDataNew = 0i32 != 0 };
}
#[no_mangle]
pub unsafe extern "C" fn updateRcCommands() {
        ...
        if (rcData[THROTTLE as libc::c_int as usize] as libc::c_int) < 2000i32
           {
            ...
            prop =
                100i32 - (*currentControlRateProfile).dynThrPID as libc::c_int
        }
    ...
    if feature(FEATURE_3D as libc::c_int as uint32_t) {
        tmp_0 =
            constrain(rcData[THROTTLE as libc::c_int as usize] as libc::c_int,
                      1000i32, 2000i32);
        tmp_0 = (tmp_0 - 1000i32) as uint32_t as int32_t
    } else {
        tmp_0 =
            constrain(rcData[THROTTLE as libc::c_int as usize] as libc::c_int,
                      (*rxConfig()).mincheck as libc::c_int, 2000i32);
        tmp_0 =
            ((tmp_0 - (*rxConfig()).mincheck as libc::c_int) as
                 uint32_t).wrapping_mul(1000i32 as
                                            libc::c_uint).wrapping_div((2000i32
                                                                            -
                                                                            (*rxConfig()).mincheck
                                                                                as
                                                                                libc::c_int)
            ...
            if IS_RC_MODE_ACTIVE(BOX3D) {
                let mut throttleScaler: fix12_t =
                    qConstruct((rcCommand[THROTTLE as libc::c_int as usize] -
                                    1000i32 as libc::c_float) as int16_t,
                               1000i32 as int16_t);
                rcCommand[THROTTLE as libc::c_int as usize] =
                    ((*rxConfig()).midrc as libc::c_int +
            ...
            rcCommand[THROTTLE as libc::c_int as usize] =
                ((*rxConfig()).midrc as libc::c_int +
                     qMultiply(throttleScaler_0,
                               (1000i32 - (*rxConfig()).midrc as libc::c_int)
                                   as int16_t) as libc::c_int) as
                    libc::c_float
        } else {
            reverseMotors = 0i32 != 0;
    ...
    if flightModeFlags as libc::c_int & HEADFREE_MODE as libc::c_int != 0 {
        ...
        } else { rcCommandBuff.Z = 0i32 as libc::c_float }
...
pub unsafe extern "C" fn resetYawAxis() {
    rcCommand[YAW as libc::c_int as usize] = 0i32 as libc::c_float;
    setpointRate[YAW as libc::c_int as usize] = 0i32 as libc::c_float;
}
#[no_mangle]
...
pub unsafe extern "C" fn initRcProcessing() {
    ...
    while i < 12i32 {
        let tmp: int16_t =
            (10i32 * i - (*currentControlRateProfile).thrMid8 as libc::c_int)
                as int16_t;
        let mut y: uint8_t = 1i32 as uint8_t;
        if tmp as libc::c_int > 0i32 {
            y =
                (100i32 - (*currentControlRateProfile).thrMid8 as libc::c_int)
                    as uint8_t
        }
        if (tmp as libc::c_int) < 0i32 {
        ...
        lookupThrottleRC[i as usize] =
            (10i32 * (*currentControlRateProfile).thrMid8 as libc::c_int +
                 tmp as libc::c_int *
                     (100i32 -
                          ...
                          (*currentControlRateProfile).thrExpo8 as int32_t *
                              ...
                              (y as libc::c_int * y as libc::c_int)) / 10i32)
                as int16_t;
        lookupThrottleRC[i as usize] =
            (1000i32 +
                 (2000i32 - 1000i32) *
                     lookupThrottleRC[i as usize] as libc::c_int / 1000i32) as
                int16_t;
        i += 1
    ...
    interpolationChannels = 0i32 as uint8_t;
    ...
    match (*rxConfig()).rcInterpolationChannels as libc::c_int {
        2 => {
            ...
            current_block_22 = 1625083683255690461;
        }
        1 => { current_block_22 = 1625083683255690461; }
        0 => { current_block_22 = 17068600714731869384; }
        4 => {
            interpolationChannels =
                (interpolationChannels as libc::c_int |
                     (ROLL_FLAG as libc::c_int | PITCH_FLAG as libc::c_int))
                    as uint8_t;
            current_block_22 = 8063465107312385349;
        }
        3 => { current_block_22 = 8063465107312385349; }
    ...
    match current_block_22 {
        1625083683255690461 => {
            ...
            current_block_22 = 17068600714731869384;
        }
        8063465107312385349 => {
            interpolationChannels =
                (interpolationChannels as libc::c_int |
