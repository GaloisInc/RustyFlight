...
pub static mut rangefinderConfig_Registry: pgRegistry_t =
    unsafe {
        {
            let mut init =
                pgRegistry_s{pgn: (527i32 | 0i32 << 12i32) as pgn_t,
                             size:
...
pub static mut sonarConfig_Registry: pgRegistry_t =
    unsafe {
        {
            let mut init =
                pgRegistry_s{pgn: (516i32 | 1i32 << 12i32) as pgn_t,
                             size:
...
pub static mut pgResetTemplate_sonarConfig: sonarConfig_t =
    {
        let mut init =
            sonarConfig_s{triggerTag: (1i32 + 1i32 << 4i32 | 0i32) as ioTag_t,
                          echoTag: (1i32 + 1i32 << 4i32 | 1i32) as ioTag_t,};
        init
    };
/*
...
pub unsafe extern "C" fn rangefinderResetDynamicThreshold() {
    rangefinder.snrThresholdReached = 0i32 != 0;
    rangefinder.dynamicDistanceThreshold = 0i32;
}
#[no_mangle]
pub unsafe extern "C" fn rangefinderInit() -> bool {
    ...
    rangefinder.rawAltitude = -1i32;
    rangefinder.calculatedAltitude = -1i32;
    ...
    rangefinder.snr = 0i32 as int16_t;
...
unsafe extern "C" fn applyMedianFilter(mut newReading: int32_t) -> int32_t {
    ...
    if newReading > -1i32 {
        ...
        filterSampleIndex += 1;
        if filterSampleIndex == 5i32 {
            filterSampleIndex = 0i32;
...
unsafe extern "C" fn computePseudoSnr(mut newReading: int32_t) -> int16_t {
    ...
    static mut snrSampleIndex: uint8_t = 0i32 as uint8_t;
    static mut previousReading: int32_t = -1i32;
    static mut snrReady: bool = 0i32 != 0;
    let mut pseudoSnr: int16_t = 0i32 as int16_t;
    let delta: libc::c_int = newReading - previousReading;
    snrSamples[snrSampleIndex as usize] =
        constrain(delta * delta / 10i32, 0i32, 6400i32) as int16_t;
    snrSampleIndex = snrSampleIndex.wrapping_add(1);
    if snrSampleIndex as libc::c_int == 5i32 {
        snrSampleIndex = 0i32 as uint8_t;
        snrReady = 1i32 != 0
    }
    previousReading = newReading;
    if snrReady {
        let mut i: uint8_t = 0i32 as uint8_t;
        while (i as libc::c_int) < 5i32 {
            pseudoSnr =
                (pseudoSnr as libc::c_int +
                     snrSamples[i as usize] as libc::c_int) as int16_t;
            i = i.wrapping_add(1)
        }
        return constrain(pseudoSnr as libc::c_int, 0i32, 32000i32) as int16_t
    } else { return -1i32 as int16_t };
}
/*
...
pub unsafe extern "C" fn isSurfaceAltitudeValid() -> bool {
     ...
     */
    if rangefinder.calculatedAltitude > 0i32 && rangefinder.rawAltitude > 0i32
           && (rangefinder.snr as libc::c_int) < 600i32 {
        /*
...
pub unsafe extern "C" fn rangefinderProcess(mut cosTiltAngle: libc::c_float)
 -> bool {
    if rangefinder.dev.read.is_some() {
        ...
        if distance == -3i32 { return 0i32 != 0 }
        if distance >= 0i32 {
            rangefinder.lastValidResponseTimeMs = millis();
            rangefinder.rawAltitude = applyMedianFilter(distance)
        } else if distance == -1i32 {
            rangefinder.lastValidResponseTimeMs = millis();
            rangefinder.rawAltitude = -1i32
        } else {
            // Invalid response / hardware failure
            rangefinder.rawAltitude = -2i32
        }
        rangefinder.snr = computePseudoSnr(distance);
        if rangefinder.snrThresholdReached as libc::c_int == 0i32 &&
               rangefinder.rawAltitude > 0i32 {
            if (rangefinder.snr as libc::c_int) < 600i32 &&
                   rangefinder.dynamicDistanceThreshold <
                       rangefinder.rawAltitude {
                rangefinder.dynamicDistanceThreshold =
                    rangefinder.rawAltitude * 75i32 / 100i32
            }
            if rangefinder.snr as libc::c_int >= 600i32 {
                rangefinder.snrThresholdReached = 1i32 != 0
            }
        }
        if debugMode as libc::c_int == DEBUG_RANGEFINDER as libc::c_int {
            debug[3] = rangefinder.snr
        }
        if debugMode as libc::c_int ==
               DEBUG_RANGEFINDER_QUALITY as libc::c_int {
            debug[0] = rangefinder.rawAltitude as int16_t
        }
        if debugMode as libc::c_int ==
               DEBUG_RANGEFINDER_QUALITY as libc::c_int {
            debug[1] = rangefinder.snrThresholdReached as int16_t
        }
        if debugMode as libc::c_int ==
               DEBUG_RANGEFINDER_QUALITY as libc::c_int {
            debug[2] = rangefinder.dynamicDistanceThreshold as int16_t
        }
        if debugMode as libc::c_int ==
               DEBUG_RANGEFINDER_QUALITY as libc::c_int {
            debug[3] = isSurfaceAltitudeValid() as int16_t
        }
    } else {
        // Bad configuration
        rangefinder.rawAltitude = -1i32
    ...
    */
    if cosTiltAngle < rangefinder.maxTiltCos || rangefinder.rawAltitude < 0i32
       {
        rangefinder.calculatedAltitude = -1i32
    } else {
    ...
    if debugMode as libc::c_int == DEBUG_RANGEFINDER as libc::c_int {
        debug[1] = rangefinder.rawAltitude as int16_t
    }
    if debugMode as libc::c_int == DEBUG_RANGEFINDER as libc::c_int {
        debug[2] = rangefinder.calculatedAltitude as int16_t
    }
    return 1i32 != 0;
}
/* *
