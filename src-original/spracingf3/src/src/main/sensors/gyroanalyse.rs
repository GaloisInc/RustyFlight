...
pub unsafe extern "C" fn gyroDataAnalyseInit(mut targetLooptimeUs: uint32_t) {
    ...
    fftSamplingRateHz =
        ({
             let mut _a: libc::c_int = gyroLoopRateHz / 3i32;
             let mut _b: libc::c_int = 1333i32;
             if _a < _b { _a } else { _b }
         }) as uint16_t;
    fftBpfHz = (fftSamplingRateHz as libc::c_int / 4i32) as uint16_t;
    fftResolution =
        fftSamplingRateHz as libc::c_float / 32i32 as libc::c_float;
    dynNotchMaxCentreHz =
        (fftSamplingRateHz as libc::c_int / 2i32) as uint16_t;
    ...
    fftBinOffset =
        ({
             let mut _a: libc::c_int = 1i32;
             let mut _b: libc::c_long =
                 lrintf(90i32 as libc::c_float / fftResolution - 1.5f32);
    ...
    while i < 32i32 {
        hanningWindow[i as usize] =
            0.5f32 -
                0.5f32 *
                    cos_approx(2i32 as libc::c_float *
                                   3.14159265358979323846f32 *
                                   i as libc::c_float /
                                   (32i32 - 1i32) as libc::c_float);
        i += 1
    }
    dynamicNotchCutoff =
        (100.0f32 -
             (*gyroConfig()).dyn_notch_width_percent as libc::c_int as
                 libc::c_float) / 100i32 as libc::c_float;
}
#[no_mangle]
pub unsafe extern "C" fn gyroDataAnalyseStateInit(mut state:
    ...
    let samplingFrequency: uint16_t =
        (1000000i32 as libc::c_uint).wrapping_div(targetLooptimeUs) as
            uint16_t;
    ...
    arm_rfft_fast_init_f32(&mut (*state).fftInstance, 32i32 as uint16_t);
    ...
    let looptime: libc::c_float =
        ({
             let mut _a: libc::c_uint =
                 1000000u32.wrapping_div(fftSamplingRateHz as libc::c_uint);
             let mut _b: libc::c_uint =
                 targetLooptimeUs.wrapping_mul((3i32 * 4i32) as libc::c_uint);
             if _a > _b { _a } else { _b }
         }) as libc::c_float;
    let mut axis: libc::c_int = 0i32;
    while axis < 3i32 {
        // any init value
        (*state).centerFreq[axis as usize] = 200i32 as uint16_t;
        biquadFilterInit(&mut *(*state).gyroBandpassFilter.as_mut_ptr().offset(axis
                                                                                   as
                                                                                   isize),
                         fftBpfHz as libc::c_float,
                         (1000000i32 / fftSamplingRateHz as libc::c_int) as
                             uint32_t,
                         0.01f32 *
        ...
        biquadFilterInitLPF(&mut *(*state).detectedFrequencyFilter.as_mut_ptr().offset(axis
                                                                                           as
                                                                                           isize),
                            60i32 as libc::c_float, looptime as uint32_t);
        axis += 1
    };
}
#[no_mangle]
...
pub unsafe extern "C" fn gyroDataAnalyse(mut state: *mut gyroAnalyseState_t,
    ...
    if (*state).sampleCount as libc::c_int ==
        ...
        (*state).sampleCount = 0i32 as uint8_t;
        ...
        while axis < 3i32 {
            ...
            if axis == 0i32 {
                if debugMode as libc::c_int == DEBUG_FFT as libc::c_int {
                    debug[2] = lrintf(sample) as int16_t
                }
            }
            (*state).oversampledGyroAccumulator[axis as usize] =
        ...
        (*state).circularBufferIdx =
            (((*state).circularBufferIdx as libc::c_int + 1i32) % 32i32) as
                uint8_t;
        // We need DYN_NOTCH_CALC_TICKS tick to update all axis with newly sampled value
        (*state).updateTicks = (3i32 * 4i32) as uint8_t
    }
    // calculate FFT and update filters
    if (*state).updateTicks as libc::c_int > 0i32 {
...
unsafe extern "C" fn gyroDataAnalyseUpdate(mut state: *mut gyroAnalyseState_t,
    ...
    if debugMode as libc::c_int == DEBUG_FFT_TIME as libc::c_int {
        debug[0] = (*state).updateStep as int16_t
    }
        ...
        5 => {
            ...
            let cutoffFreq: libc::c_float =
                fmax(((*state).centerFreq[(*state).updateAxis as usize] as
                         ...
                         as libc::c_double, 105i32 as libc::c_double) as
                    libc::c_float;
            ...
            (*state).updateAxis =
                (((*state).updateAxis as libc::c_int + 1i32) % 3i32) as
                    uint8_t;
            (*state).updateStep = (*state).updateStep.wrapping_add(1);
    ...
    match current_block_97 {
        ...
        {
            let mut fftSum: libc::c_float = 0i32 as libc::c_float;
            let mut fftWeightedSum: libc::c_float = 0i32 as libc::c_float;
            let mut fftIncreasing: bool = 0i32 != 0;
            // iterate over fft data and calculate weighted indices
            let mut i: libc::c_int = 1i32 + fftBinOffset as libc::c_int;
            while i < 32i32 / 2i32 {
                let data: libc::c_float = (*state).fftData[i as usize];
                let prevData: libc::c_float =
                    (*state).fftData[(i - 1i32) as usize];
                if fftIncreasing as libc::c_int != 0 ||
                       data > prevData * 2i32 as libc::c_float {
                    let mut cubedData: libc::c_float = data * data * data;
                    // add previous bin before first rise
                    if !fftIncreasing {
                        cubedData += prevData * prevData * prevData;
                        fftIncreasing = 1i32 != 0
                    }
                    fftSum += cubedData;
                    // calculate weighted index starting at 1, not 0
                    fftWeightedSum += cubedData * (i + 1i32) as libc::c_float
                }
                i += 1
            }
            ...
            let mut fftMeanIndex: libc::c_float = 0i32 as libc::c_float;
            if fftSum > 0i32 as libc::c_float {
                // idx was shifted by 1 to start at 1, not 0
                fftMeanIndex =
                    fftWeightedSum / fftSum - 1i32 as libc::c_float;
                // the index points at the center frequency of each bin so index 0 is actually 16.125Hz
                centerFreq =
                    constrain((fftMeanIndex * fftResolution) as libc::c_int,
                              125i32, dynNotchMaxCentreHz as libc::c_int) as
                        libc::c_float
            }
            centerFreq =
                biquadFilterApply(&mut *(*state).detectedFrequencyFilter.as_mut_ptr().offset((*state).updateAxis
                                  ...
                                  centerFreq);
            centerFreq =
                constrain(centerFreq as libc::c_int, 125i32,
                          dynNotchMaxCentreHz as libc::c_int) as
                    libc::c_float;
            (*state).centerFreq[(*state).updateAxis as usize] =
                centerFreq as uint16_t;
            if (*state).updateAxis as libc::c_int == 0i32 {
                if debugMode as libc::c_int == DEBUG_FFT as libc::c_int {
                    debug[3] =
                        lrintf(fftMeanIndex * 100i32 as libc::c_float) as
                            int16_t
                }
            }
        ...
        11959176018832582207 => {
            ...
            let ringBufIdx: uint8_t =
                (32i32 - (*state).circularBufferIdx as libc::c_int) as
                    uint8_t;
            arm_mult_f32(&mut *(*(*state).downsampledGyroData.as_mut_ptr().offset((*state).updateAxis
                                                                                      ...
                                                                                      isize)).as_mut_ptr().offset((*state).circularBufferIdx
                                                                                                                      as
                                                                                                                      isize),
                         &mut *hanningWindow.as_mut_ptr().offset(0),
                         &mut *(*state).fftData.as_mut_ptr().offset(0),
                         ringBufIdx as uint32_t);
            if (*state).circularBufferIdx as libc::c_int > 0i32 {
                arm_mult_f32(&mut *(*(*state).downsampledGyroData.as_mut_ptr().offset((*state).updateAxis
                                                                                          as
    ...
    (*state).updateStep =
        (((*state).updateStep as libc::c_int + 1i32) %
