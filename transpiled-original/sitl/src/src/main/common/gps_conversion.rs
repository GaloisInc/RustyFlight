...
pub unsafe extern "C" fn GPS_coord_to_degrees(mut coordinateString:
 ...
 -> uint32_t {
    ...
    let mut degress: uint8_t = 0i32 as uint8_t;
    let mut minutes: uint8_t = 0i32 as uint8_t;
    let mut fractionalMinutes: uint16_t = 0i32 as uint16_t;
    ...
    while isdigit(*fieldSeparator as libc::c_uchar as libc::c_int) != 0 {
        if fieldSeparator >= coordinateString.offset(15) {
    ...
    while fieldSeparator.wrapping_offset_from(remainingString) as libc::c_long
              > 2i32 as libc::c_long {
        if degress != 0 {
            degress = (degress as libc::c_int * 10i32) as uint8_t
        }
    ...
    while fieldSeparator > remainingString {
        if minutes != 0 {
            minutes = (minutes as libc::c_int * 10i32) as uint8_t
        }
    ...
    if *fieldSeparator as libc::c_int == '.' as i32 {
        remainingString = fieldSeparator.offset(1);
        digitIndex = 0i32 as uint8_t;
        while (digitIndex as libc::c_int) < 4i32 {
            fractionalMinutes =
                (fractionalMinutes as libc::c_int * 10i32) as uint16_t;
    ...
    return (degress as
                libc::c_ulong).wrapping_mul(10000000u64).wrapping_add((minutes
                                                                           as
                                                                           libc::c_ulong).wrapping_mul(1000000u64).wrapping_add((fractionalMinutes
                                                                                                                                     as
                                                                                                                                     libc::c_ulong).wrapping_mul(100u64)).wrapping_div(6i32
                                                                                                                                                                                           as
                                                                                                                                                                                           libc::c_ulong))
               as uint32_t;
}
