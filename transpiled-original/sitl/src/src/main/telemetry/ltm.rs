...
unsafe extern "C" fn ltm_gframe() {
    let mut gps_fix_type: uint8_t = 0i32 as uint8_t;
    ...
    if stateFlags as libc::c_int & GPS_FIX as libc::c_int == 0 {
        gps_fix_type = 1i32 as uint8_t
    } else if (gpsSol.numSat as libc::c_int) < 5i32 {
        gps_fix_type = 2i32 as uint8_t
    } else { gps_fix_type = 3i32 as uint8_t }
    ...
    ltm_serialise_8((gpsSol.groundSpeed as libc::c_int / 100i32) as uint8_t);
    ltm_alt =
        ...
        } else { (gpsSol.llh.alt) * 100i32 };
    ltm_serialise_32(ltm_alt as uint32_t);
    ltm_serialise_8(((gpsSol.numSat as libc::c_int) << 2i32 |
...
unsafe extern "C" fn ltm_oframe() {
    ...
    ltm_serialise_32(GPS_home[0] as uint32_t); // OSD always ON
    ltm_serialise_32(GPS_home[1] as uint32_t);
