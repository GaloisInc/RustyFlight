...
pub const PWM_TYPE_MAX: C2RustUnnamed_3 = 5;
...
pub const THROTTLE_LIMIT_TYPE_CLIP: C2RustUnnamed_4 = 2;
pub const THROTTLE_LIMIT_TYPE_SCALE: C2RustUnnamed_4 = 1;
pub const THROTTLE_LIMIT_TYPE_OFF: C2RustUnnamed_4 = 0;
#[derive ( Copy, Clone )]
#[repr(C)]
...
pub static mut mixerConfig_Registry: pgRegistry_t =
    unsafe {
        {
            let mut init =
                pgRegistry_s{pgn: (20i32 | 0i32 << 12i32) as pgn_t,
                             size:
...
pub static mut pgResetTemplate_mixerConfig: mixerConfig_t =
    {
        let mut init =
            mixerConfig_s{mixerMode: MIXER_QUADX as libc::c_int as uint8_t,
                          yaw_motors_reversed: 0i32 != 0,
                          crashflip_motor_percent: 0i32 as uint8_t,};
        init
    };
#[no_mangle]
#[link_section = ".pg_registry"]
...
pub static mut motorConfig_Registry: pgRegistry_t =
    unsafe {
        {
            let mut init =
                pgRegistry_s{pgn: (6i32 | 1i32 << 12i32) as pgn_t,
                             size:
...
pub unsafe extern "C" fn pgResetFn_motorConfig(mut motorConfig_0:
                                                   *mut motorConfig_t) {
    (*motorConfig_0).minthrottle = 1070i32 as uint16_t;
    (*motorConfig_0).dev.motorPwmRate = 480i32 as uint16_t;
    (*motorConfig_0).dev.motorPwmProtocol =
        PWM_TYPE_ONESHOT125 as libc::c_int as uint8_t;
    (*motorConfig_0).maxthrottle = 2000i32 as uint16_t;
    (*motorConfig_0).mincommand = 1000i32 as uint16_t;
    (*motorConfig_0).digitalIdleOffsetValue = 450i32 as uint16_t;
    (*motorConfig_0).dev.useBurstDshot = 0i32 as uint8_t;
    let mut motorIndex: libc::c_int = 0i32;
    while motorIndex < 8i32 {
        (*motorConfig_0).dev.ioTags[motorIndex as usize] =
            timerioTagGetByUsage(TIM_USE_MOTOR, motorIndex as uint8_t);
        motorIndex += 1
    }
    (*motorConfig_0).motorPoleCount = 14i32 as uint8_t;
...
static mut mixerBicopter: [motorMixer_t; 2] =
    [{
         let mut init =
             motorMixer_s{throttle: 1.0f32,
                          roll: 1.0f32,
...
static mut mixerHex6H: [motorMixer_t; 6] =
    [{
         let mut init =
             motorMixer_s{throttle: 1.0f32,
                          roll: -1.0f32,
...
static mut mixerHex6P: [motorMixer_t; 6] =
    [{
         let mut init =
             motorMixer_s{throttle: 1.0f32,
                          roll: -0.866025f32,
...
static mut mixerY6: [motorMixer_t; 6] =
    [{
         let mut init =
             motorMixer_s{throttle: 1.0f32,
                          roll: 0.0f32,
...
// USE_UNCOMMON_MIXERS
// MAX_SUPPORTED_MOTORS >= 6
static mut mixerOctoX8: [motorMixer_t; 8] =
    [{
         let mut init =
             motorMixer_s{throttle: 1.0f32,
                          roll: -1.0f32,
...
static mut mixerOctoFlatP: [motorMixer_t; 8] =
    [{
         let mut init =
             motorMixer_s{throttle: 1.0f32,
                          roll: 0.707107f32,
...
static mut mixerOctoFlatX: [motorMixer_t; 8] =
    [{
         let mut init =
             motorMixer_s{throttle: 1.0f32,
                          roll: 1.0f32,
...
static mut mixerDualcopter: [motorMixer_t; 2] =
    [{
         let mut init =
             motorMixer_s{throttle: 1.0f32,
                          roll: 0.0f32,
...
pub static mut mixers: [mixer_t; 27] =
    unsafe {
        [{
             let mut init =
                 mixer_s{motorCount: 0i32 as uint8_t,
                         useServo: 0i32 as uint8_t,
                         motor: 0 as *const motorMixer_t,};
             init
         },
         {
             let mut init =
                 mixer_s{motorCount: 3i32 as uint8_t,
                         useServo: 1i32 as uint8_t,
                         motor: mixerTricopter.as_ptr(),};
             init
         },
         {
             let mut init =
                 mixer_s{motorCount: 4i32 as uint8_t,
                         useServo: 0i32 as uint8_t,
         ...
         {
             let mut init =
                 mixer_s{motorCount: 2i32 as uint8_t,
                         useServo: 1i32 as uint8_t,
                         motor: mixerBicopter.as_ptr(),};
             init
         },
         {
             let mut init =
         ...
         {
             let mut init =
                 mixer_s{motorCount: 6i32 as uint8_t,
                         useServo: 0i32 as uint8_t,
                         motor: mixerY6.as_ptr(),};
             init
         },
         {
             let mut init =
                 mixer_s{motorCount: 6i32 as uint8_t,
                         useServo: 0i32 as uint8_t,
                         motor: mixerHex6P.as_ptr(),};
             init
         },
         {
             let mut init =
                 mixer_s{motorCount: 1i32 as uint8_t,
                         useServo: 1i32 as uint8_t,
         ...
         {
             let mut init =
                 mixer_s{motorCount: 8i32 as uint8_t,
                         useServo: 0i32 as uint8_t,
                         motor: mixerOctoX8.as_ptr(),};
             init
         },
         {
             let mut init =
...
pub unsafe extern "C" fn areMotorsRunning() -> bool {
    let mut motorsRunning: bool = 0i32 != 0;
    if armingFlags as libc::c_int & ARMED as libc::c_int != 0 {
        motorsRunning = 1i32 != 0
    } else {
...
pub unsafe extern "C" fn initEscEndpoints() {
    ...
    match (*motorConfig()).dev.motorPwmProtocol as libc::c_int { _ => { } }
    if feature(FEATURE_3D as libc::c_int as uint32_t) {
        disarmMotorOutput = (*flight3DConfig()).neutral3d as libc::c_float;
        motorOutputLow = (*flight3DConfig()).limit3d_low as libc::c_float;
        motorOutputHigh = (*flight3DConfig()).limit3d_high as libc::c_float;
        deadbandMotor3dHigh =
            (*flight3DConfig()).deadband3d_high as libc::c_float;
        deadbandMotor3dLow =
            (*flight3DConfig()).deadband3d_low as libc::c_float
    } else {
        disarmMotorOutput = (*motorConfig()).mincommand as libc::c_float;
        motorOutputLow = (*motorConfig()).minthrottle as libc::c_float;
        motorOutputHigh = (*motorConfig()).maxthrottle as libc::c_float
    }
    rcCommandThrottleRange =
        (2000i32 - (*rxConfig()).mincheck as libc::c_int) as libc::c_float;
    ...
    rcCommandThrottleRange3dLow =
        (rcCommand3dDeadBandLow as libc::c_int - 1000i32) as libc::c_float;
    rcCommandThrottleRange3dHigh =
        (2000i32 - rcCommand3dDeadBandHigh as libc::c_int) as libc::c_float;
}
#[no_mangle]
...
pub unsafe extern "C" fn mixerConfigureOutput() {
    ...
    if currentMixerMode as libc::c_uint ==
        ...
        if motorCount as libc::c_int > 8i32 { motorCount = 8i32 as uint8_t }
...
pub unsafe extern "C" fn mixerResetDisarmedMotors() {
    ...
    let mut i: libc::c_int = 0i32;
    while i < 8i32 { motor_disarmed[i as usize] = disarmMotorOutput; i += 1 };
}
#[no_mangle]
...
pub unsafe extern "C" fn stopPwmAllMotors() {
    ...
    delayMicroseconds(1500i32 as
                          timeUs_t); // time when motors last reversed in 3D mode
}
static mut throttle: libc::c_float = 0i32 as libc::c_float;
...
unsafe extern "C" fn calculateThrottleAndCurrentMotorEndpoints(mut currentTimeUs:
                                                                   timeUs_t) {
    static mut rcThrottlePrevious: uint16_t = 0i32 as uint16_t;
    static mut reversalTimeUs: timeUs_t = 0i32 as timeUs_t;
    let mut currentThrottleInputRange: libc::c_float = 0i32 as libc::c_float;
    if feature(FEATURE_3D as libc::c_int as uint32_t) {
        if armingFlags as libc::c_int & ARMED as libc::c_int == 0 {
        ...
        if rcCommand[THROTTLE as libc::c_int as usize] <=
            ...
            if motorOutputMixSign as libc::c_int != -1i32 {
                reversalTimeUs = currentTimeUs
            }
            motorOutputMixSign = -1i32 as int8_t;
            rcThrottlePrevious =
...
unsafe extern "C" fn applyFlipOverAfterCrashModeToMotors() {
    if armingFlags as libc::c_int & ARMED as libc::c_int != 0 {
        ...
        let mut signRoll: libc::c_float =
            if getRcDeflection(FD_ROLL as libc::c_int) < 0i32 as libc::c_float
               {
                1i32
            } else { -1i32 } as libc::c_float;
        ...
        if stickDeflectionYawAbs >
            ...
            signRoll = 0i32 as libc::c_float;
            signPitch = 0i32 as libc::c_float
        } else {
            // If pitch/roll dominant, disable yaw
            signYaw = 0i32 as libc::c_float
        } // cos(PI/6.0f)
        ...
        if cosPhi < cosThreshold {
            ...
            if stickDeflectionRollAbs > stickDeflectionPitchAbs {
                signPitch = 0i32 as libc::c_float
            } else { signRoll = 0i32 as libc::c_float }
        }
        ...
        while i < motorCount as libc::c_int {
            ...
            if motorOutput < 0i32 as libc::c_float {
            ...
            motorOutput =
                if motorOutput < motorOutputMin + 20i32 as libc::c_float {
                    disarmMotorOutput
                } else { (motorOutput) - 20i32 as libc::c_float };
...
pub unsafe extern "C" fn mixTable(mut currentTimeUs: timeUs_t,
    ...
    if yawSpinDetected {
        yawPidSumLimit = 1000i32 as uint16_t
    ...
    let mut motorMixMax: libc::c_float = 0i32 as libc::c_float;
    let mut motorMixMin: libc::c_float = 0i32 as libc::c_float;
    let mut i: libc::c_int = 0i32;
    while i < motorCount as libc::c_int {
...
pub unsafe extern "C" fn convertExternalToMotor(mut externalValue: uint16_t)
 -> libc::c_float {
    ...
    match isMotorProtocolDshot() as libc::c_int { _ => { } }
...
pub unsafe extern "C" fn convertMotorToExternal(mut motorValue: libc::c_float)
 -> uint16_t {
    ...
    match isMotorProtocolDshot() as libc::c_int { _ => { } }
    externalValue = motorValue as uint16_t;
...
unsafe extern "C" fn run_static_initializers() {
    customMotorMixer_Registry =
        {
            let mut init =
                pgRegistry_s{pgn: (4i32 | 0i32 << 12i32) as pgn_t,
                             size:
