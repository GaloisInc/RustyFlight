...
extern "C" {
    ...
    #[no_mangle]
    fn serTcpOpen(id: libc::c_int, rxCallback: serialReceiveCallbackPtr,
...
pub const UARTDEV_8: C2RustUnnamed_1 = 7;
pub const UARTDEV_7: C2RustUnnamed_1 = 6;
pub const UARTDEV_6: C2RustUnnamed_1 = 5;
pub const UARTDEV_5: C2RustUnnamed_1 = 4;
pub const UARTDEV_4: C2RustUnnamed_1 = 3;
pub const UARTDEV_3: C2RustUnnamed_1 = 2;
pub const UARTDEV_2: C2RustUnnamed_1 = 1;
pub const UARTDEV_1: C2RustUnnamed_1 = 0;
 ...
 * If not, see <http://www.gnu.org/licenses/>.
 */
static mut serialPortUsageList: [serialPortUsage_t; 8] =
    [serialPortUsage_t{serialPort:
                           0 as *const serialPort_t as *mut serialPort_t,
                       function: FUNCTION_NONE,
                       identifier: SERIAL_PORT_USART1,}; 8];
#[no_mangle]
pub static mut serialPortIdentifiers: [serialPortIdentifier_e; 8] =
    [SERIAL_PORT_USART1, SERIAL_PORT_USART2, SERIAL_PORT_USART3,
     SERIAL_PORT_UART4, SERIAL_PORT_UART5, SERIAL_PORT_USART6,
     SERIAL_PORT_USART7, SERIAL_PORT_USART8];
static mut serialPortCount: uint8_t = 0;
#[no_mangle]
pub static mut baudRates: [uint32_t; 16] =
    [0i32 as uint32_t, 9600i32 as uint32_t, 19200i32 as uint32_t,
     38400i32 as uint32_t, 57600i32 as uint32_t, 115200i32 as uint32_t,
     230400i32 as uint32_t, 250000i32 as uint32_t, 400000i32 as uint32_t,
     460800i32 as uint32_t, 500000i32 as uint32_t, 921600i32 as uint32_t,
     1000000i32 as uint32_t, 1500000i32 as uint32_t, 2000000i32 as uint32_t,
     2470000i32 as uint32_t];
#[no_mangle]
...
pub static mut serialConfig_Registry: pgRegistry_t =
    unsafe {
        {
            let mut init =
                pgRegistry_s{pgn: (13i32 | 0i32 << 12i32) as pgn_t,
                             size:
...
pub unsafe extern "C" fn pgResetFn_serialConfig(mut serialConfig_0:
                                                    *mut serialConfig_t) {
    memset(serialConfig_0 as *mut libc::c_void, 0i32,
    ...
    (*serialConfig_0).portConfigs[0].functionMask =
        FUNCTION_MSP as libc::c_int as uint16_t;
    (*serialConfig_0).reboot_character = 'R' as i32 as uint8_t;
    (*serialConfig_0).serial_update_rate_hz = 100i32 as uint16_t;
}
#[no_mangle]
...
pub unsafe extern "C" fn findSerialPortUsageByIdentifier(mut identifier:
 ...
 -> *mut serialPortUsage_t {
    let mut index: uint8_t = 0;
    index = 0i32 as uint8_t;
    while (index as libc::c_int) < 8i32 {
...
pub unsafe extern "C" fn findNextSerialPortConfig(mut function:
                                                      serialPortFunction_e)
 -> *mut serialPortConfig_t {
    while (findSerialPortConfigState.lastIndex as libc::c_int) < 8i32 {
 ...
 -> portSharing_e {
    if portConfig.is_null() ||
           (*portConfig).functionMask as libc::c_uint &
               function as libc::c_uint == 0i32 as libc::c_uint {
...
pub unsafe extern "C" fn isSerialConfigValid(mut serialConfigToCheck:
 ...
 -> bool {
     ...
     */
    let mut mspPortCount: uint8_t = 0i32 as uint8_t;
    let mut index: libc::c_int = 0i32;
    while index < 8i32 {
        ...
        let mut bitCount: uint8_t =
            (((*portConfig).functionMask as libc::c_int -
                  ((*portConfig).functionMask as libc::c_int >> 1i32 &
                       0x77777777i32) -
                  ((*portConfig).functionMask as libc::c_int >> 2i32 &
                       0x33333333i32) -
                  ((*portConfig).functionMask as libc::c_int >> 3i32 &
                       0x11111111i32) +
                  ((*portConfig).functionMask as libc::c_int -
                       ((*portConfig).functionMask as libc::c_int >> 1i32 &
                            0x77777777i32) -
                       ((*portConfig).functionMask as libc::c_int >> 2i32 &
                            0x33333333i32) -
                       ((*portConfig).functionMask as libc::c_int >> 3i32 &
                            0x11111111i32) >> 4i32) & 0xf0f0f0fi32) % 255i32)
                as uint8_t;
        if bitCount as libc::c_int > 1i32 {
            // shared
            if bitCount as libc::c_int > 2i32 { return 0i32 != 0 }
            if !((*portConfig).functionMask as libc::c_int &
                     FUNCTION_MSP as libc::c_int != 0 &&
                     (*portConfig).functionMask as libc::c_int &
    ...
    if mspPortCount as libc::c_int == 0i32 ||
           mspPortCount as libc::c_int > 3i32 {
        return 0i32 != 0
    }
    return 1i32 != 0;
}
 ...
 -> *mut serialPort_t {
    ...
    match identifier as libc::c_int {
        0 | 1 | 2 | 3 | 4 | 5 | 6 | 7 => {
            ...
            serialPort =
                serTcpOpen(identifier as libc::c_int -
                               SERIAL_PORT_USART1 as libc::c_int +
                               UARTDEV_1 as libc::c_int, rxCallback,
...
pub unsafe extern "C" fn serialInit(mut softserialEnabled: bool,
                                    mut serialPortToDisable:
                                        serialPortIdentifier_e) {
    serialPortCount = 8i32 as uint8_t;
    memset(&mut serialPortUsageList as *mut [serialPortUsage_t; 8] as
               *mut libc::c_void, 0i32,
           ::core::mem::size_of::<[serialPortUsage_t; 8]>() as libc::c_ulong);
    let mut index: libc::c_int = 0i32;
    while index < 8i32 {
...
pub unsafe extern "C" fn serialRemovePort(mut identifier:
                                              serialPortIdentifier_e) {
    let mut index: uint8_t = 0i32 as uint8_t;
    while (index as libc::c_int) < 8i32 {
        if serialPortUsageList[index as usize].identifier as libc::c_int ==
               identifier as libc::c_int {
            serialPortUsageList[index as usize].identifier = SERIAL_PORT_NONE;
