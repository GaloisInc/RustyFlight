        ...
        let mut ioCfg: ioConfig_t =
            (GPIO_Mode_AF as libc::c_int |
                 ...
                 (if options as libc::c_uint &
                  ...
                  } else { GPIO_OType_OD as libc::c_int }) << 4i32 |
                 (if options as libc::c_uint &
                         SERIAL_INVERTED as libc::c_int as libc::c_uint != 0
                         ||
                         options as libc::c_uint &
                  ...
                  } else { GPIO_PuPd_UP as libc::c_int }) << 5i32) as
                ioConfig_t;
        IOInit(txIO, OWNER_SERIAL_TX,
               (index as libc::c_int + 1i32) as uint8_t);
 ...
 -> *mut uartPort_t {
    ...
    if !(*hardware).rxDMAChannel.is_null() {
        dmaInit(dmaGetIdentifier((*hardware).rxDMAChannel), OWNER_SERIAL_RX,
                (device as libc::c_uint).wrapping_add(1i32 as libc::c_uint) as
                    uint8_t);
...
pub unsafe extern "C" fn uartIrqHandler(mut s: *mut uartPort_t) {
    ...
    if (*s).rxDMAChannel.is_null() && ISR & 0x20i32 as uint32_t != 0 {
    ...
    if (*s).txDMAChannel.is_null() && ISR & 0x80i32 as uint32_t != 0 {
        if (*s).port.txBufferTail != (*s).port.txBufferHead {
            ...
            USART_ITConfig((*s).USARTx, 0x70107i32 as uint32_t, DISABLE);
        }
    }
    if ISR & 0x8i32 as uint32_t != 0 {
        USART_ClearITPendingBit((*s).USARTx, 0x30300i32 as uint32_t);
    };
}
        ...
        [{
             let mut init =
                 uartHardware_s{device: UARTDEV_1,
                                ...
                                rxDMAChannel: 0 as *mut DMA_Channel_TypeDef,
                                rxPins:
                                    [(0i32 + 1i32 << 4i32 | 10i32) as ioTag_t,
                                     (1i32 + 1i32 << 4i32 | 7i32) as ioTag_t,
                                     0i32 as ioTag_t, 0i32 as ioTag_t],
                                txPins:
                                    [(0i32 + 1i32 << 4i32 | 9i32) as ioTag_t,
                                     (1i32 + 1i32 << 4i32 | 6i32) as ioTag_t,
                                     0i32 as ioTag_t, 0i32 as ioTag_t],
                                rcc:
         ...
         },
         {
             let mut init =
                 uartHardware_s{device: UARTDEV_2,
                                reg:
                                    (0x40000000i32 as
                                         uint32_t).wrapping_add(0x4400i32 as
                                                                    libc::c_uint)
                                        as *mut USART_TypeDef,
                                txDMAChannel: 0 as *mut DMA_Channel_TypeDef,
                                rxDMAChannel: 0 as *mut DMA_Channel_TypeDef,
                                rxPins:
                                    [(0i32 + 1i32 << 4i32 | 15i32) as ioTag_t,
                                     (0i32 + 1i32 << 4i32 | 3i32) as ioTag_t,
                                     (1i32 + 1i32 << 4i32 | 4i32) as ioTag_t,
                                     0i32 as ioTag_t],
                                txPins:
                                    [(0i32 + 1i32 << 4i32 | 14i32) as ioTag_t,
                                     (0i32 + 1i32 << 4i32 | 2i32) as ioTag_t,
                                     (1i32 + 1i32 << 4i32 | 3i32) as ioTag_t,
                                     0i32 as ioTag_t],
                                rcc:
         ...
         },
         {
             let mut init =
                 uartHardware_s{device: UARTDEV_3,
                                reg:
                                    (0x40000000i32 as
                                         uint32_t).wrapping_add(0x4800i32 as
                                                                    libc::c_uint)
                                        as *mut USART_TypeDef,
                                txDMAChannel: 0 as *mut DMA_Channel_TypeDef,
                                rxDMAChannel: 0 as *mut DMA_Channel_TypeDef,
