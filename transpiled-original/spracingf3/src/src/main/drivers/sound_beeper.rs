...
static mut beeperInverted: bool = 0i32 != 0;
static mut beeperFrequency: uint16_t = 0i32 as uint16_t;
/*
 * This file is part of Cleanflight and Betaflight.
 *
 * Cleanflight and Betaflight are free software. You can redistribute
...
pub unsafe extern "C" fn systemBeep(mut onoff: bool) {
    if beeperFrequency as libc::c_int == 0i32 {
        IOWrite(beeperIO,
...
pub unsafe extern "C" fn beeperInit(mut config: *const beeperDevConfig_t) {
    ...
    if beeperFrequency as libc::c_int == 0i32 {
        ...
        if !beeperIO.is_null() {
            IOInit(beeperIO, OWNER_BEEPER, 0i32 as uint8_t);
