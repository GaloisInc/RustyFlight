...
pub static mut adcTagMap: [adcTagMap_t; 39] =
    [{
         let mut init =
             adcTagMap_s{tag: (0i32 + 1i32 << 4i32 | 0i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_1 as libc::c_int) as uint8_t,
                         channel: 0x1i32 as uint8_t,};
         init
     },
     {
         let mut init =
             adcTagMap_s{tag: (0i32 + 1i32 << 4i32 | 1i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_1 as libc::c_int) as uint8_t,
                         channel: 0x2i32 as uint8_t,};
         init
     },
     {
         let mut init =
             adcTagMap_s{tag: (0i32 + 1i32 << 4i32 | 2i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_1 as libc::c_int) as uint8_t,
                         channel: 0x3i32 as uint8_t,};
         init
     },
     {
         let mut init =
             adcTagMap_s{tag: (0i32 + 1i32 << 4i32 | 3i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_1 as libc::c_int) as uint8_t,
                         channel: 0x4i32 as uint8_t,};
         init
     },
     {
         let mut init =
             adcTagMap_s{tag: (0i32 + 1i32 << 4i32 | 4i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_2 as libc::c_int) as uint8_t,
                         channel: 0x1i32 as uint8_t,};
         init
     },
     {
         let mut init =
             adcTagMap_s{tag: (0i32 + 1i32 << 4i32 | 5i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_2 as libc::c_int) as uint8_t,
                         channel: 0x2i32 as uint8_t,};
         init
     ...
     {
         let mut init =
             adcTagMap_s{tag: (0i32 + 1i32 << 4i32 | 6i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_2 as libc::c_int) as uint8_t,
                         channel: 0x3i32 as uint8_t,};
         init
     ...
     {
         let mut init =
             adcTagMap_s{tag: (0i32 + 1i32 << 4i32 | 7i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_4 as libc::c_int) as uint8_t,
                         channel: 0x4i32 as uint8_t,};
         init
     },
     {
         let mut init =
             adcTagMap_s{tag: (1i32 + 1i32 << 4i32 | 0i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_3 as libc::c_int) as uint8_t,
                         channel: 0xci32 as uint8_t,};
         init
     },
     {
         let mut init =
             adcTagMap_s{tag: (1i32 + 1i32 << 4i32 | 1i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_3 as libc::c_int) as uint8_t,
                         channel: 0x1i32 as uint8_t,};
         init
     ...
     {
         let mut init =
             adcTagMap_s{tag: (1i32 + 1i32 << 4i32 | 2i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_2 as libc::c_int) as uint8_t,
                         channel: 0xci32 as uint8_t,};
         init
     ...
     {
         let mut init =
             adcTagMap_s{tag: (1i32 + 1i32 << 4i32 | 12i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_4 as libc::c_int) as uint8_t,
                         channel: 0x3i32 as uint8_t,};
         init
     ...
     {
         let mut init =
             adcTagMap_s{tag: (1i32 + 1i32 << 4i32 | 13i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_3 as libc::c_int) as uint8_t,
                         channel: 0x5i32 as uint8_t,};
         init
     },
     {
         let mut init =
             adcTagMap_s{tag: (1i32 + 1i32 << 4i32 | 14i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_4 as libc::c_int) as uint8_t,
                         channel: 0x4i32 as uint8_t,};
         init
     ...
     {
         let mut init =
             adcTagMap_s{tag: (1i32 + 1i32 << 4i32 | 15i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_4 as libc::c_int) as uint8_t,
                         channel: 0x5i32 as uint8_t,};
         init
     ...
     {
         let mut init =
             adcTagMap_s{tag: 0i32 as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_1 as libc::c_int |
                                  1i32 << ADCDEV_2 as libc::c_int) as uint8_t,
                         channel: 0x6i32 as uint8_t,};
         init
     },
     {
         let mut init =
     ...
     {
         let mut init =
             adcTagMap_s{tag: (5i32 + 1i32 << 4i32 | 4i32) as ioTag_t,
                         devices:
                             (1i32 << ADCDEV_1 as libc::c_int) as uint8_t,
                         channel: 0x5i32 as uint8_t,};
         init
...
pub unsafe extern "C" fn adcInit(mut config: *const adcConfig_t) {
    ...
    let mut DMA_InitStructure: DMA_InitTypeDef =
        DMA_InitTypeDef{DMA_PeripheralBaseAddr: 0,
                        ...
                        DMA_Priority: 0,
                        DMA_M2M: 0,};
    let mut adcChannelCount: uint8_t = 0i32 as uint8_t;
    ...
    let mut device: ADCDevice =
        adcDeviceByInstance((0x40000000i32 as
    ...
    while i < ADC_CHANNEL_COUNT as libc::c_int {
        if adcVerifyPin(adcOperatingConfig[i as usize].tag, device) {
            ...
            IOConfigGPIO(IOGetByTag(adcOperatingConfig[i as usize].tag),
                         (GPIO_Mode_AN as libc::c_int | 0i32 << 2i32 |
            ...
            adcOperatingConfig[i as usize].sampleTime = 0x7i32 as uint8_t;
            adcOperatingConfig[i as usize].enabled = 1i32 != 0
        }
        i += 1
    }
    if !adcActive { return }
    if device as libc::c_int == ADCDEV_1 as libc::c_int ||
        ...
        RCC_ADCCLKConfig(0x1b0i32 as uint32_t);
        // 72 MHz divided by 256 = 281.25 kHz
    } else {
        // enable clock for ADC3+4
        RCC_ADCCLKConfig(0x10003600i32 as uint32_t);
        // 72 MHz divided by 256 = 281.25 kHz
    }
    RCC_ClockCmd(adc.rccADC, ENABLE);
    dmaInit(dmaGetIdentifier(adc.DMAy_Channelx), OWNER_ADC, 0i32 as uint8_t);
    ...
    DMA_InitStructure.DMA_MemoryInc =
        if adcChannelCount as libc::c_int > 1i32 {
            0x80i32 as uint32_t
        } else { 0i32 as uint32_t };
    DMA_InitStructure.DMA_PeripheralDataSize = 0x100i32 as uint32_t;
    DMA_InitStructure.DMA_MemoryDataSize = 0x400i32 as uint32_t;
    ...
    ADC_SelectCalibrationMode(adc.ADCx, 0i32 as uint32_t);
    ...
    ADC_CommonInitStructure.ADC_Mode = 0i32 as uint32_t;
    ADC_CommonInitStructure.ADC_Clock = 0x30000i32 as uint32_t;
    ADC_CommonInitStructure.ADC_DMAAccessMode = 0x8000i32 as uint32_t;
    ADC_CommonInitStructure.ADC_DMAMode = 0x2i32 as uint32_t;
    ADC_CommonInitStructure.ADC_TwoSamplingDelay = 0i32 as uint8_t;
    ADC_CommonInit(adc.ADCx, &mut ADC_CommonInitStructure);
    ADC_StructInit(&mut ADC_InitStructure);
    ADC_InitStructure.ADC_ContinuousConvMode = 0x2000i32 as uint32_t;
    ADC_InitStructure.ADC_Resolution = 0i32 as uint32_t;
    ADC_InitStructure.ADC_ExternalTrigConvEvent =
        0i32 as uint16_t as uint32_t;
    ADC_InitStructure.ADC_ExternalTrigEventEdge =
        0i32 as uint16_t as uint32_t;
    ADC_InitStructure.ADC_DataAlign = 0i32 as uint32_t;
    ADC_InitStructure.ADC_OverrunMode = 0i32 as uint32_t;
    ADC_InitStructure.ADC_AutoInjMode = 0i32 as uint32_t;
    ADC_InitStructure.ADC_NbrOfRegChannel = adcChannelCount;
    ADC_Init(adc.ADCx, &mut ADC_InitStructure);
    let mut rank: uint8_t = 1i32 as uint8_t;
    ...
    while ADC_GetFlagStatus(adc.ADCx, 0x1i32 as uint16_t as uint32_t) as u64
              == 0 {
    }
    ADC_DMAConfig(adc.ADCx, 0x2i32 as uint32_t);
             ...
             let mut init =
                 adcDevice_s{ADCx:
                             ...
                             DMAy_Channelx:
                                 (0x40000000i32 as
                                      uint32_t).wrapping_add(0x20000i32 as
                                                                 libc::c_uint).wrapping_add(0x408i32
                                                                                                as
                                                                                                libc::c_uint)
                                     as *mut DMA_Channel_TypeDef,};
             init
             ...
             let mut init =
                 adcDevice_s{ADCx:
                             ...
                             DMAy_Channelx:
                                 (0x40000000i32 as
                                      uint32_t).wrapping_add(0x20000i32 as
                                                                 libc::c_uint).wrapping_add(0x458i32
                                                                                                as
                                                                                                libc::c_uint)
                                     as *mut DMA_Channel_TypeDef,};
             init
