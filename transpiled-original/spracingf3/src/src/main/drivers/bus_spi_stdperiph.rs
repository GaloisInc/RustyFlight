...
pub unsafe extern "C" fn spiInitDevice(mut device: SPIDevice) {
    ...
    IOInit(IOGetByTag((*spi).sck), OWNER_SPI_SCK,
           (device as libc::c_int + 1i32) as uint8_t);
    ...
    let mut spiInit: SPI_InitTypeDef =
        SPI_InitTypeDef{SPI_Direction: 0,
                        ...
                        SPI_FirstBit: 0,
                        SPI_CRCPolynomial: 0,};
    spiInit.SPI_Mode = 0x104i32 as uint16_t;
    spiInit.SPI_Direction = 0i32 as uint16_t;
    spiInit.SPI_DataSize = 0x700i32 as uint16_t;
    spiInit.SPI_NSS = 0x200i32 as uint16_t;
    spiInit.SPI_FirstBit = 0i32 as uint16_t;
    spiInit.SPI_CRCPolynomial = 7i32 as uint16_t;
    spiInit.SPI_BaudRatePrescaler = 0x10i32 as uint16_t;
    if (*spi).leadingEdge {
        spiInit.SPI_CPOL = 0i32 as uint16_t;
        spiInit.SPI_CPHA = 0i32 as uint16_t
    } else {
        spiInit.SPI_CPOL = 0x2i32 as uint16_t;
        spiInit.SPI_CPHA = 0x1i32 as uint16_t
    }
    // Configure for 8-bit reads.
    SPI_RxFIFOThresholdConfig((*spi).dev, 0x1000i32 as uint16_t);
...
pub unsafe extern "C" fn spiTransferByte(mut instance: *mut SPI_TypeDef,
    ...
    let mut spiTimeout: uint16_t = 1000i32 as uint16_t;
    while SPI_I2S_GetFlagStatus(instance, 0x2i32 as uint16_t) as libc::c_uint
    ...
    spiTimeout = 1000i32 as uint16_t;
    while SPI_I2S_GetFlagStatus(instance, 0x1i32 as uint16_t) as libc::c_uint
              == RESET as libc::c_int as libc::c_uint {
...
pub unsafe extern "C" fn spiIsBusBusy(mut instance: *mut SPI_TypeDef)
 -> bool {
    return SPI_GetTransmissionFIFOStatus(instance) as libc::c_int !=
               ...
               SPI_I2S_GetFlagStatus(instance, 0x80i32 as uint16_t) as
...
pub unsafe extern "C" fn spiTransfer(mut instance: *mut SPI_TypeDef,
    ...
    loop  {
        ...
        b =
            ...
            } else { 0xffi32 } as uint8_t;
        while SPI_I2S_GetFlagStatus(instance, 0x2i32 as uint16_t) as
        ...
        spiTimeout = 1000i32 as uint16_t;
        while SPI_I2S_GetFlagStatus(instance, 0x1i32 as uint16_t) as
                  libc::c_uint == RESET as libc::c_int as libc::c_uint {
            let fresh5 = spiTimeout;
            spiTimeout = spiTimeout.wrapping_sub(1);
            if fresh5 as libc::c_int == 0i32 {
                return spiTimeoutUserCallback(instance) != 0
            }
        }
...
pub unsafe extern "C" fn spiSetDivisor(mut instance: *mut SPI_TypeDef,
    ...
    let tempRegister: uint16_t =
        ((*instance).CR1 as libc::c_int &
             !(1i32 << 5i32 | 1i32 << 4i32 | 1i32 << 3i32)) as uint16_t;
