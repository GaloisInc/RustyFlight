...
unsafe extern "C" fn qmc5883lInit(mut magDev: *mut magDev_t) -> bool {
    let mut ack: bool = 1i32 != 0;
    let mut busdev: *mut busDevice_t = &mut (*magDev).busdev;
    ack =
        ack as libc::c_int != 0 &&
            busWriteRegister(busdev, 0xbi32 as uint8_t, 0x1i32 as uint8_t) as
                libc::c_int != 0;
    ack =
        ack as libc::c_int != 0 &&
            busWriteRegister(busdev, 0x9i32 as uint8_t,
                             (0x1i32 | 0x3i32 << 2i32 | 0i32 << 6i32 |
                                  0x1i32 << 4i32) as uint8_t) as libc::c_int
                != 0;
...
unsafe extern "C" fn qmc5883lRead(mut magDev: *mut magDev_t,
    ...
    *magData.offset(X as libc::c_int as isize) = 0i32 as int16_t;
    *magData.offset(Y as libc::c_int as isize) = 0i32 as int16_t;
    *magData.offset(Z as libc::c_int as isize) = 0i32 as int16_t;
    let mut busdev: *mut busDevice_t = &mut (*magDev).busdev;
    let mut ack: bool =
        busReadRegisterBuffer(busdev, 0x6i32 as uint8_t, &mut status,
                              1i32 as uint8_t);
    if !ack || status as libc::c_int & 0x4i32 == 0i32 { return 0i32 != 0 }
    ack =
        busReadRegisterBuffer(busdev, 0i32 as uint8_t, buf.as_mut_ptr(),
                              6i32 as uint8_t);
    if !ack { return 0i32 != 0 }
    *magData.offset(X as libc::c_int as isize) =
        ((buf[1] as libc::c_int) << 8i32 | buf[0] as libc::c_int) as int16_t;
    *magData.offset(Y as libc::c_int as isize) =
        ((buf[3] as libc::c_int) << 8i32 | buf[2] as libc::c_int) as int16_t;
    *magData.offset(Z as libc::c_int as isize) =
        ((buf[5] as libc::c_int) << 8i32 | buf[4] as libc::c_int) as int16_t;
    return 1i32 != 0;
}
/*
 * This file is part of Cleanflight and Betaflight.
...
pub unsafe extern "C" fn qmc5883lDetect(mut magDev: *mut magDev_t) -> bool {
    ...
    if (*busdev).bustype as libc::c_uint ==
        ...
        (*busdev).busdev_u.i2c.address = 0xdi32 as uint8_t
    }
    // Must write reset first  - don't care about the result
    busWriteRegister(busdev, 0xai32 as uint8_t, 0x80i32 as uint8_t);
    delay(20i32 as timeMs_t);
    let mut sig: uint8_t = 0i32 as uint8_t;
    let mut ack: bool =
        busReadRegisterBuffer(busdev, 0xdi32 as uint8_t, &mut sig,
                              1i32 as uint8_t);
    if ack as libc::c_int != 0 && sig as libc::c_int == 0xffi32 {
        ...
        ack =
            busReadRegisterBuffer(busdev, 0x9i32 as uint8_t, &mut sig,
                                  1i32 as uint8_t);
        if ack as libc::c_int != 0 && sig as libc::c_int != 0i32 {
            return 0i32 != 0
        }
        (*magDev).init =
