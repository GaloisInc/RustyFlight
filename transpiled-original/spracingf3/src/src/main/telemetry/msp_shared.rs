...
static mut checksum: uint8_t = 0i32 as uint8_t;
...
unsafe extern "C" fn processMspPacket() {
    (*mspPackage.responsePacket).cmd = 0i32 as int16_t;
    (*mspPackage.responsePacket).result = 0i32 as int16_t;
    (*mspPackage.responsePacket).buf.end = mspPackage.responseBuffer;
...
pub unsafe extern "C" fn sendMspErrorResponse(mut error: uint8_t,
    ...
    (*mspPackage.responsePacket).result = -10i32 as int16_t;
    sbufSwitchToReader(&mut (*mspPackage.responsePacket).buf,
                       mspPackage.responseBuffer);
}
#[no_mangle]
pub unsafe extern "C" fn handleMspFrame(mut frameStart: *mut uint8_t,
 ...
 -> bool {
    static mut mspStarted: uint8_t = 0i32 as uint8_t;
    static mut lastSeq: uint8_t = 0i32 as uint8_t;
    if sbufBytesRemaining(&mut (*mspPackage.responsePacket).buf) > 0i32 {
        mspStarted = 0i32 as uint8_t
    }
    if mspStarted as libc::c_int == 0i32 { initSharedMsp(); }
    ...
    let seqNumber: uint8_t = (header as libc::c_int & 0xfi32) as uint8_t;
    let version: uint8_t =
        ((header as libc::c_int & 0x7i32 << 5i32) >> 5i32) as uint8_t;
    if version as libc::c_int != 1i32 {
        sendMspErrorResponse(TELEMETRY_MSP_VER_MISMATCH as libc::c_int as
                                 uint8_t, 0i32 as int16_t);
        return 1i32 != 0
    }
    if header as libc::c_int & 1i32 << 4i32 != 0 {
        ...
        checksum =
            (mspPayloadSize as libc::c_int ^ (*packet).cmd as libc::c_int) as
                uint8_t;
        mspStarted = 1i32 as uint8_t
    } else if mspStarted == 0 {
        // no start packet yet, throw this one away
        return 0i32 != 0
    } else {
        if lastSeq as libc::c_int + 1i32 & 0xfi32 != seqNumber as libc::c_int
           {
 ...
 -> bool {
    ...
    if (*txBuf).ptr == mspPackage.responseBuffer {
        ...
        let mut head: uint8_t =
            (1i32 << 4i32 | fresh0 as libc::c_int & 0xfi32) as uint8_t;
        if ((*mspPackage.responsePacket).result as libc::c_int) < 0i32 {
            head = (head as libc::c_int | 1i32 << 5i32) as uint8_t
        }
        ...
        let fresh1 = seq;
        seq = seq.wrapping_add(1);
        sbufWriteU8(payloadBuf, (fresh1 as libc::c_int & 0xfi32) as uint8_t);
    }
    ...
    if bufferBytesRemaining as libc::c_int >=
        ...
        while sbufBytesRemaining(payloadBuf) > 1i32 {
            sbufWriteU8(payloadBuf, 0i32 as uint8_t);
        }
    }
    responseFn.expect("non-null function pointer")(payloadOut.as_mut_ptr());
    return 0i32 != 0;
