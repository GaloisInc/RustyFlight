...
pub static mut spek2commonBand: [uint8_t; 5] =
    [4i32 as uint8_t, 5i32 as uint8_t, 3i32 as uint8_t, 2i32 as uint8_t,
     1i32 as uint8_t];
...
pub static mut vtxTrampPi: [uint8_t; 8] =
    [0i32 as uint8_t, 0i32 as uint8_t, 1i32 as uint8_t, 2i32 as uint8_t,
     3i32 as uint8_t, 4i32 as uint8_t, 5i32 as uint8_t, 3i32 as uint8_t];
// USE_VTX_TRAMP
//USE_VTX_RTC6705
// SmartAudio "---", 25, 200, 500. 800 mW
#[no_mangle]
pub static mut vtxSaPi: [uint8_t; 8] =
    [0i32 as uint8_t, 0i32 as uint8_t, 1i32 as uint8_t, 1i32 as uint8_t,
     2i32 as uint8_t, 3i32 as uint8_t, 4i32 as uint8_t, 2i32 as uint8_t];
// USE_VTX_SMARTAUDIO
#[no_mangle]
pub unsafe extern "C" fn convertSpektrumVtxPowerIndex(mut sPower: uint8_t)
 -> uint8_t {
    let mut devicePower: uint8_t = 0i32 as uint8_t;
...
static mut vtxControl_ipc: uint32_t = !0xe000e000u32;
// ############ RX task ######################
#[no_mangle]
...
pub unsafe extern "C" fn spektrumVtxControl() {
    static mut prevVtxControl: uint32_t = 0i32 as uint32_t;
    let mut vtxControl: uint32_t = 0;
    // Check for invalid VTX ctrl frames
    if vtxControl_ipc & 0xf000f000u32 != 0xe000e000u32 { return }
    vtxControl = vtxControl_ipc;
    vtxControl_ipc = 0i32 as uint32_t;
    ...
    let mut vtx: spektrumVtx_t =
        {
            let mut init =
                spektrumVtx_t{band:
                                  ((vtxControl & 0xe00000i32 as libc::c_uint)
                                       >> 21i32) as uint8_t,
                              channel:
                                  ((vtxControl & 0xf0000i32 as libc::c_uint)
                                       >> 16i32) as uint8_t,
                              power:
                                  ((vtxControl & 0x7i32 as libc::c_uint) >>
                                       0i32) as uint8_t,
                              region:
                                  ((vtxControl & 0x8i32 as libc::c_uint) >>
                                       3i32) as uint8_t,
                              pitMode:
                                  ((vtxControl & 0x10i32 as libc::c_uint) >>
                                       4i32) as uint8_t,
                              powerValue: 0,};
            init
        };
    ...
    if !vtxDevice.is_null() {
        ...
        let channel: uint8_t = (vtx.channel as libc::c_int + 1i32) as uint8_t;
