...
pub static mut vtxSettingsConfig_Registry: pgRegistry_t =
    unsafe {
        {
            let mut init =
                pgRegistry_s{pgn: (259i32 | 0i32 << 12i32) as pgn_t,
                             size:
...
pub static mut pgResetTemplate_vtxSettingsConfig: vtxSettingsConfig_t =
    {
        let mut init =
            vtxSettingsConfig_s{band: 4i32 as uint8_t,
                                channel: 1i32 as uint8_t,
                                power: 1i32 as uint8_t,
                                freq: 5740i32 as uint16_t,
                                pitModeFreq: 0i32 as uint16_t,
                                lowPowerDisarm: 0i32 as uint8_t,};
        init
    };
#[no_mangle]
pub unsafe extern "C" fn vtxInit() {
    let mut settingsUpdated: bool = 0i32 != 0;
...
unsafe extern "C" fn vtxGetSettings() -> vtxSettingsConfig_t {
    ...
    if IS_RC_MODE_ACTIVE(BOXVTXPITMODE) as libc::c_int != 0 &&
        ...
        settings.band = 0i32 as uint8_t;
        settings.freq = settings.pitModeFreq;
        settings.power = 1i32 as uint8_t
    }
...
unsafe extern "C" fn vtxProcessPitMode(mut vtxDevice: *mut vtxDevice_t)
 -> bool {
    ...
    static mut prevPmSwitchState: bool = 0i32 != 0;
    if armingFlags as libc::c_int & ARMED as libc::c_int == 0 &&
        ...
        if currPmSwitchState as libc::c_int !=
            ...
            if currPmSwitchState {
                ...
                if isModeActivationConditionPresent(BOXVTXPITMODE) {
                    if pitOnOff == 0 {
                        vtxCommonSetPitMode(vtxDevice, 1i32 as uint8_t);
                        return 1i32 != 0
                    }
                }
            } else if pitOnOff != 0 {
                vtxCommonSetPitMode(vtxDevice, 0i32 as uint8_t);
                return 1i32 != 0
            }
        }
    }
...
pub unsafe extern "C" fn vtxUpdate(mut currentTimeUs: timeUs_t) {
    static mut currentSchedule: uint8_t = 0i32 as uint8_t;
    if cliMode != 0 { return }
    ...
    if !vtxDevice.is_null() {
        ...
        let mut vtxUpdatePending: bool = 0i32 != 0;
        loop  {
            ...
            currentSchedule =
                ((currentSchedule as libc::c_int + 1i32) %
