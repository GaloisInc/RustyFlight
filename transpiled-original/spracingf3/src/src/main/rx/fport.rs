...
static mut emptySmartPortFrame: smartPortPayload_t =
    {
        let mut init =
            smartPortPayload_s{frameId: 0i32 as uint8_t,
                               valueId: 0i32 as uint16_t,
                               data: 0i32 as uint32_t,};
        init
    };
static mut rxBuffer: [fportBuffer_t; 3] =
    [fportBuffer_t{data: [0; 27], length: 0,}; 3];
static mut rxBufferWriteIndex: uint8_t = 0i32 as uint8_t;
static mut rxBufferReadIndex: uint8_t = 0i32 as uint8_t;
static mut lastTelemetryFrameReceivedUs: timeUs_t = 0;
static mut clearToSend: bool = 0i32 != 0;
static mut framePosition: uint8_t = 0i32 as uint8_t;
static mut mspPayload: *mut smartPortPayload_t =
    0 as *const smartPortPayload_t as *mut smartPortPayload_t;
static mut lastRcFrameReceivedMs: timeUs_t = 0i32 as timeUs_t;
static mut fportPort: *mut serialPort_t =
    0 as *const serialPort_t as *mut serialPort_t;
static mut telemetryEnabled: bool = 0i32 != 0;
unsafe extern "C" fn reportFrameError(mut errorReason: uint8_t) {
    static mut frameErrors: uint16_t = 0i32 as uint16_t;
...
unsafe extern "C" fn fportDataReceive(mut c: uint16_t,
    ...
    static mut frameStartAt: timeUs_t = 0i32 as timeUs_t;
    static mut escapedCharacter: bool = 0i32 != 0;
    static mut lastFrameReceivedUs: timeUs_t = 0i32 as timeUs_t;
    static mut telemetryFrame: bool = 0i32 != 0;
    let currentTimeUs: timeUs_t = micros();
    ::core::ptr::write_volatile(&mut clearToSend as *mut bool, 0i32 != 0);
    if framePosition as libc::c_int > 1i32 &&
           cmpTimeUs(currentTimeUs, frameStartAt) > 3000i32 + 500i32 {
    ...
    if val as libc::c_int == 0x7ei32 {
        if framePosition as libc::c_int > 1i32 {
            ...
            escapedCharacter = 0i32 != 0
        }
        frameStartAt = currentTimeUs;
        ::core::ptr::write_volatile(&mut framePosition as *mut uint8_t,
                                    1i32 as uint8_t)
    } else if framePosition as libc::c_int > 0i32 {
        if framePosition as libc::c_ulong >=
                                                                                                                      ...
                                                                                                                      libc::c_ulong)
           {
            ...
            if escapedCharacter {
                val = (val as libc::c_int ^ 0x20i32) as uint8_t;
                escapedCharacter = 0i32 != 0
            } else if val as libc::c_int == 0x7di32 {
                escapedCharacter = 1i32 != 0;
                return
            }
            if framePosition as libc::c_int == 2i32 &&
                   val as libc::c_int ==
                       FPORT_FRAME_TYPE_TELEMETRY_REQUEST as libc::c_int {
                telemetryFrame = 1i32 != 0
            }
            rxBuffer[rxBufferWriteIndex as
                         usize].data[(framePosition as libc::c_int - 1i32) as
                                         usize] = val;
            ::core::ptr::write_volatile(&mut framePosition as *mut uint8_t,
                                        (framePosition as libc::c_int + 1i32)
                                            as uint8_t)
        }
    };
}
unsafe extern "C" fn smartPortWriteFrameFport(mut payload:
    ...
    ::core::ptr::write_volatile(&mut framePosition as *mut uint8_t,
                                0i32 as uint8_t);
    let mut checksum: uint16_t = 0i32 as uint16_t;
...
unsafe extern "C" fn checkChecksum(mut data: *mut uint8_t,
    ...
    return checksum as libc::c_int == 0xffi32;
}
unsafe extern "C" fn fportFrameStatus(mut rxRuntimeConfig:
    ...
    static mut hasTelemetryRequest: bool = 0i32 != 0;
    let mut result: uint8_t = RX_FRAME_PENDING as libc::c_int as uint8_t;
    static mut rxDrivenFrameRate: bool = 0i32 != 0;
    static mut consecutiveTelemetryFrameCount: uint8_t = 0i32 as uint8_t;
    if rxBufferReadIndex as libc::c_int != rxBufferWriteIndex as libc::c_int {
        ...
        let mut frameLength: uint8_t =
            rxBuffer[rxBufferReadIndex as usize].data[0];
        if frameLength as libc::c_int != bufferLength as libc::c_int - 2i32 {
                        ...
                        match (*frame).data.telemetryData.frameId as
                            ...
                            16 => {
                                if !rxDrivenFrameRate {
                                    rxDrivenFrameRate = 1i32 != 0
                                }
                                hasTelemetryRequest = 1i32 != 0
                            }
                            0 => {
                                if !rxDrivenFrameRate {
        ...
        ::core::ptr::write_volatile(&mut rxBufferReadIndex as *mut uint8_t,
                                    ((rxBufferReadIndex as libc::c_int + 1i32)
                                         % 3i32) as uint8_t)
    }
    if (!mspPayload.is_null() || hasTelemetryRequest as libc::c_int != 0) &&
           cmpTimeUs(micros(), lastTelemetryFrameReceivedUs) >= 500i32 {
        hasTelemetryRequest = 0i32 != 0;
        result =
    ...
    if lastRcFrameReceivedMs != 0 &&
        ...
        setRssiDirect(0i32 as uint16_t, RSSI_SOURCE_RX_PROTOCOL);
        lastRcFrameReceivedMs = 0i32 as timeUs_t
    }
    return result;
}
unsafe extern "C" fn fportProcessFrame(mut rxRuntimeConfig:
    ...
    if cmpTimeUs(currentTimeUs, lastTelemetryFrameReceivedUs) > 2000i32 {
        ::core::ptr::write_volatile(&mut clearToSend as *mut bool, 0i32 != 0)
    }
    if clearToSend {
...
pub unsafe extern "C" fn fportRxInit(mut rxConfig: *const rxConfig_t,
    ...
    (*rxRuntimeConfig).channelCount = 18i32 as uint8_t;
    ...
    fportPort =
        openSerialPort((*portConfig).identifier, FUNCTION_RX_SERIAL,
                       ...
                       115200i32 as uint32_t, MODE_RXTX,
