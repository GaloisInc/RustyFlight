...
#[no_mangle]
pub static mut jetiTimeStampRequest: uint32_t = 0i32 as uint32_t;
...
pub unsafe extern "C" fn jetiExBusCalcCRC16(mut pt: *mut uint8_t,
    ...
    let mut crc16_data: uint16_t = 0i32 as uint16_t;
    let mut data: uint8_t = 0i32 as uint8_t;
        ...
        data =
            (*pt.offset(mlen as isize) as libc::c_int ^
                 crc16_data as uint8_t as libc::c_int &
                     0xffi32 as uint8_t as libc::c_int) as uint8_t;
        data =
            (data as libc::c_int ^ (data as libc::c_int) << 4i32) as uint8_t;
        crc16_data =
            (((data as uint16_t as libc::c_int) << 8i32 |
                  (crc16_data as libc::c_int & 0xff00i32) >> 8i32) ^
                 (data as libc::c_int >> 4i32) as uint8_t as libc::c_int ^
                 (data as uint16_t as libc::c_int) << 3i32) as uint16_t;
...
pub unsafe extern "C" fn jetiExBusDecodeChannelFrame(mut exBusFrame:
    ...
    match (*exBusFrame.offset(EXBUS_HEADER_SYNC as libc::c_int as isize) as
        ...
        15873 | 15875 => {
            ...
            while (i as libc::c_int) < 16i32 {
                frameAddr = (6i32 + i as libc::c_int * 2i32) as uint8_t;
                value =
                    ((*exBusFrame.offset((frameAddr as libc::c_int + 1i32) as
                ...
                jetiExBusChannelData[i as usize] =
                    (value as libc::c_int >> 3i32) as uint16_t;
                i = i.wrapping_add(1)
            }
        }
        _ => { }
...
pub unsafe extern "C" fn jetiExBusFrameReset() {
    jetiExBusFramePosition = 0i32 as uint8_t;
    jetiExBusFrameLength = (6i32 + 16i32 * 2i32 + 2i32) as uint8_t;
}
/*
  supported:
...
unsafe extern "C" fn jetiExBusDataReceive(mut c: uint16_t,
    ...
    static mut jetiExBusTimeLast: uint32_t = 0i32 as uint32_t;
    ...
    if jetiExBusTimeInterval > 1000i32 {
        jetiExBusFrameReset();
    ...
    if jetiExBusFramePosition as libc::c_int == 0i32 {
    ...
    *jetiExBusFrame.offset(jetiExBusFramePosition as isize) = c as uint8_t;
    ...
    if jetiExBusFramePosition as libc::c_int == 6i32 {
...
pub unsafe extern "C" fn jetiExBusInit(mut rxConfig: *const rxConfig_t,
    ...
    (*rxRuntimeConfig).rxRefreshRate = 5500i32 as uint16_t;
    ...
    jetiExBusPort =
        openSerialPort((*portConfig).identifier, FUNCTION_RX_SERIAL,
                       ...
                       125000i32 as uint32_t, MODE_RXTX,
