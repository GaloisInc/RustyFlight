...
pub unsafe extern "C" fn rxInit() {
    ...
    } else if (*rxConfig()).rssi_channel as libc::c_int > 0i32 {
 ...
 -> bool {
    ...
    if feature(FEATURE_RX_PPM as libc::c_int as uint32_t) {
        if isPPMDataBeingReceived() {
            signalReceived = 1i32 != 0;
            rxIsInFailsafeMode = 0i32 != 0;
            needRxSignalBefore =
        ...
        if isPWMDataBeingReceived() {
            ...
            needRxSignalBefore =
                currentTimeUs.wrapping_add(needRxSignalMaxDelayUs);
            useDataDrivenProcessing = 0i32 != 0
        }
    } else {
...
unsafe extern "C" fn updateRSSIADC(mut currentTimeUs: timeUs_t) {
    static mut rssiUpdateAt: uint32_t = 0i32 as uint32_t;
    if (currentTimeUs.wrapping_sub(rssiUpdateAt) as int32_t) < 0i32 { return }
    rssiUpdateAt =
        currentTimeUs.wrapping_add((1000000i32 / 50i32) as libc::c_uint);
    let adcRssiSample: uint16_t =
        adcGetChannel(ADC_RSSI as libc::c_int as uint8_t);
    let mut rssiValue: uint16_t =
        (adcRssiSample as libc::c_int / (4096i32 / 1024i32)) as uint16_t;
    // RSSI_Invert option
    if (*rxConfig()).rssi_invert != 0 {
        rssiValue = (1023i32 - rssiValue as libc::c_int) as uint16_t
    }
