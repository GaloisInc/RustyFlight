...
#[no_mangle]
pub static mut currentMeterIds: [uint8_t; 16] =
    [CURRENT_METER_ID_BATTERY_1 as libc::c_int as uint8_t,
     ...
     CURRENT_METER_ID_MSP_1 as libc::c_int as uint8_t];
...
pub unsafe extern "C" fn currentMeterESCRefresh(mut lastUpdateAt: int32_t) {
    ...
    if !escData.is_null() && (*escData).dataAge as libc::c_int <= 10i32 {
        currentMeterESCState.amperage =
            (*escData).current +
                (*escSensorConfig()).offset as libc::c_int / 10i32;
        currentMeterESCState.mAhDrawn =
            ((*escData).consumption as libc::c_float +
                 ((*escSensorConfig()).offset as
                     ...
                     (1000.0f32 * 3600i32 as libc::c_float)) as int32_t
    } else {
        currentMeterESCState.amperage = 0i32;
        currentMeterESCState.mAhDrawn = 0i32
    };
}
#[no_mangle]
...
#[no_mangle]
pub static mut currentMeterMSPState: currentMeterMSPState_t =
    currentMeterMSPState_t{mAhDrawn: 0, amperage: 0,};
#[no_mangle]
pub unsafe extern "C" fn currentMeterMSPSet(mut amperage: uint16_t,
                                            mut mAhDrawn: uint16_t) {
    // We expect the FC's MSP_ANALOG response handler to call this function
    currentMeterMSPState.amperage = amperage as int32_t;
    currentMeterMSPState.mAhDrawn = mAhDrawn as int32_t;
}
#[no_mangle]
pub unsafe extern "C" fn currentMeterMSPInit() {
    memset(&mut currentMeterMSPState as *mut currentMeterMSPState_t as
               *mut libc::c_void, 0i32,
           ::core::mem::size_of::<currentMeterMSPState_t>() as libc::c_ulong);
}
#[no_mangle]
pub unsafe extern "C" fn currentMeterMSPRefresh(mut currentTimeUs: timeUs_t) {
    // periodically request MSP_ANALOG
    static mut streamRequestAt: timeUs_t = 0i32 as timeUs_t; // 10hz
    if cmp32(currentTimeUs, streamRequestAt) > 0i32 {
        streamRequestAt =
            currentTimeUs.wrapping_add((1000i32 * 1000i32 / 10i32) as
                                           libc::c_uint);
        mspSerialPush(110i32 as uint8_t, 0 as *mut uint8_t, 0i32,
                      MSP_DIRECTION_REQUEST);
    };
}
#[no_mangle]
pub unsafe extern "C" fn currentMeterMSPRead(mut meter: *mut currentMeter_t) {
...
pub unsafe extern "C" fn currentMeterRead(mut id: currentMeterId_e,
    ...
    if id as libc::c_uint ==
        ...
        currentMeterVirtualRead(meter);
                  ...
                  CURRENT_METER_ID_MSP_1 as libc::c_int as libc::c_uint {
