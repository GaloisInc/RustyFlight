...
pub unsafe extern "C" fn init() {
    ...
    if !readSuccess || !isEEPROMVersionValid() ||
           strncasecmp((*systemConfig()).boardIdentifier.as_ptr(),
                       b"SRFM\x00" as *const u8 as *const libc::c_char,
    ...
    if !isMPUSoftReset() {
        ...
        let mut secondsRemaining: uint8_t = 5 as libc::c_int as uint8_t;
        let mut bothButtonsHeld: bool = false;
        loop  {
            bothButtonsHeld =
                buttonAPressed() as libc::c_int != 0 &&
                    buttonBPressed() as libc::c_int != 0;
            if bothButtonsHeld {
                secondsRemaining = secondsRemaining.wrapping_sub(1);
                if secondsRemaining as libc::c_int == 0 as libc::c_int {
                    resetEEPROM();
                    systemReset();
                }
