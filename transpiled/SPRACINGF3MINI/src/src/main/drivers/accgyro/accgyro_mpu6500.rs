...
pub type icm_high_range_gyro_fsr_e = libc::c_uint;
pub const NUM_ICM_HIGH_RANGE_GYRO_FSR: icm_high_range_gyro_fsr_e = 4;
pub const ICM_HIGH_RANGE_FSR_4000DPS: icm_high_range_gyro_fsr_e = 3;
pub const ICM_HIGH_RANGE_FSR_2000DPS: icm_high_range_gyro_fsr_e = 2;
pub const ICM_HIGH_RANGE_FSR_1000DPS: icm_high_range_gyro_fsr_e = 1;
pub const ICM_HIGH_RANGE_FSR_500DPS: icm_high_range_gyro_fsr_e = 0;
...
pub type icm_high_range_accel_fsr_e = libc::c_uint;
pub const NUM_ICM_HIGH_RANGE_ACCEL_FSR: icm_high_range_accel_fsr_e = 4;
pub const ICM_HIGH_RANGE_FSR_32G: icm_high_range_accel_fsr_e = 3;
pub const ICM_HIGH_RANGE_FSR_16G: icm_high_range_accel_fsr_e = 2;
pub const ICM_HIGH_RANGE_FSR_8G: icm_high_range_accel_fsr_e = 1;
pub const ICM_HIGH_RANGE_FSR_4G: icm_high_range_accel_fsr_e = 0;
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn mpu6500AccInit(mut acc: *mut accDev_t) {
    (*acc).acc_1G = (512 as libc::c_int * 4 as libc::c_int) as uint16_t;
}
// Register 0x6a/106 - USER_CTRL / User Control
#[no_mangle]
pub unsafe extern "C" fn mpu6500AccDetect(mut acc: *mut accDev_t) -> bool {
    if (*acc).mpuDetectionResult.sensor as libc::c_uint !=
           MPU_65xx_I2C as libc::c_int as libc::c_uint {
        return 0 as libc::c_int != 0
    } // Get Divider Drops
    (*acc).initFn =
        Some(mpu6500AccInit as unsafe extern "C" fn(_: *mut accDev_t) -> ());
    (*acc).readFn =
        Some(mpuAccRead as unsafe extern "C" fn(_: *mut accDev_s) -> bool);
    return 1 as libc::c_int != 0;
}
#[no_mangle]
pub unsafe extern "C" fn mpu6500GyroInit(mut gyro: *mut gyroDev_t) {
    mpuGyroInit(gyro);
    let mut gyro_range: libc::c_int = INV_FSR_2000DPS as libc::c_int;
    let mut accel_range: libc::c_int = INV_FSR_16G as libc::c_int;
    if (*gyro).mpuDetectionResult.sensor as libc::c_uint ==
           ICM_20601_SPI as libc::c_int as libc::c_uint {
        gyro_range =
            if (*gyro).gyro_high_fsr as libc::c_int != 0 {
                ICM_HIGH_RANGE_FSR_4000DPS as libc::c_int
            } else { ICM_HIGH_RANGE_FSR_2000DPS as libc::c_int };
        accel_range = ICM_HIGH_RANGE_FSR_16G as libc::c_int
    }
    ...
    busWriteRegister(&mut (*gyro).bus, 0x68 as libc::c_int as uint8_t,
    ...
    busWriteRegister(&mut (*gyro).bus, 0x1b as libc::c_int as uint8_t,
                     (gyro_range << 3 as libc::c_int |
                          mpuGyroFCHOICE(gyro) as libc::c_int) as uint8_t);
    delay(15 as libc::c_int as timeMs_t);
    busWriteRegister(&mut (*gyro).bus, 0x1c as libc::c_int as uint8_t,
                     (accel_range << 3 as libc::c_int) as uint8_t);
    delay(15 as libc::c_int as timeMs_t);
    ...
    delay(15 as libc::c_int as timeMs_t); // RAW_RDY_EN interrupt enable
    busWriteRegister(&mut (*gyro).bus, 0x38 as libc::c_int as uint8_t,
                     ((1 as libc::c_int) << 0 as libc::c_int) as uint8_t);
    delay(15 as libc::c_int as timeMs_t);
}
#[no_mangle]
pub unsafe extern "C" fn mpu6500GyroDetect(mut gyro: *mut gyroDev_t) -> bool {
    if (*gyro).mpuDetectionResult.sensor as libc::c_uint !=
           MPU_65xx_I2C as libc::c_int as libc::c_uint {
        return 0 as libc::c_int != 0
    }
    (*gyro).initFn =
        Some(mpu6500GyroInit as
