 ...
 */
    ...
    #[no_mangle]
    fn frSkySpiInit(rxSpiConfig: *const rxSpiConfig_t,
                    rxRuntimeConfig: *mut rxRuntimeConfig_t) -> bool;
    #[no_mangle]
    fn frSkySpiDataReceived(packet: *mut uint8_t) -> rx_spi_received_e;
    #[no_mangle]
    fn frSkySpiSetRcData(rcData: *mut uint16_t, payload: *const uint8_t);
}
...
unsafe extern "C" fn rxSpiSetProtocol(mut protocol: rx_spi_protocol_e)
 -> bool {
    match protocol as libc::c_uint { 8 | 9 | _ => { } }
    protocolInit =
        Some(frSkySpiInit as
                 unsafe extern "C" fn(_: *const rxSpiConfig_t,
                                      _: *mut rxRuntimeConfig_t) -> bool);
    protocolDataReceived =
        Some(frSkySpiDataReceived as
                 unsafe extern "C" fn(_: *mut uint8_t) -> rx_spi_received_e);
    protocolSetRcDataFromPayload =
        Some(frSkySpiSetRcData as
