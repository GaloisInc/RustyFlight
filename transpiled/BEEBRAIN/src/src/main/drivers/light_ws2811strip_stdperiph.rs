...
pub unsafe extern "C" fn ws2811LedStripHardwareInit(mut ioTag: ioTag_t) {
    ...
    IOConfigGPIO(ws2811IO,
                 (GPIO_Speed_50MHz as libc::c_int |
                      GPIO_Mode_AF_PP as libc::c_int) as ioConfig_t);
    RCC_ClockCmd(timerRCC(timer), ENABLE);
    // Stop timer
    TIM_Cmd(timer, DISABLE);
    /* Compute the prescaler value */
    let mut prescaler: uint16_t =
        timerGetPrescalerByDesiredMhz(timer, 48 as libc::c_int as uint16_t);
    ...
    TIM_OCInitStructure.TIM_Pulse = 0 as libc::c_int as uint16_t;
    ...
    DMA_InitStructure.DMA_BufferSize =
        (24 as libc::c_int * 32 as libc::c_int + 42 as libc::c_int) as
            uint32_t;
...
pub unsafe extern "C" fn ws2811LedStripDMAEnable() {
    ...
    TIM_SetCounter(timer, 0 as libc::c_int as uint16_t);
    TIM_Cmd(timer, ENABLE);
