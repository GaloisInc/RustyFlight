...
pub unsafe extern "C" fn handleVTXControlButton() {
    let mut buttonWasPressed: bool = 0 as libc::c_int != 0;
    let start: timeMs_t = millis();
    let mut ledToggleAt: timeMs_t = start;
    let mut ledEnabled: bool = 0 as libc::c_int != 0;
    let mut flashesDone: uint8_t = 0 as libc::c_int as uint8_t;
    let mut actionCounter: uint8_t = 0 as libc::c_int as uint8_t;
    let mut buttonHeld: bool = false;
    loop  {
        buttonHeld = buttonAPressed();
        if !buttonHeld { break ; }
        let end: timeMs_t = millis();
        let mut diff: int32_t = cmp32(end, start);
        if diff > 25 as libc::c_int && diff <= 1000 as libc::c_int {
            actionCounter = 4 as libc::c_int as uint8_t
        } else if diff > 1000 as libc::c_int && diff <= 3000 as libc::c_int {
            actionCounter = 3 as libc::c_int as uint8_t
        } else if diff > 3000 as libc::c_int && diff <= 5000 as libc::c_int {
            actionCounter = 2 as libc::c_int as uint8_t
        } else if diff > 5000 as libc::c_int {
            actionCounter = 1 as libc::c_int as uint8_t
        }
        if actionCounter != 0 {
            diff = cmp32(ledToggleAt, end);
            if diff < 0 as libc::c_int {
                ledEnabled = !ledEnabled;
                let updateDuration: uint8_t = 60 as libc::c_int as uint8_t;
                ledToggleAt =
                    end.wrapping_add(updateDuration as libc::c_uint);
                if ledEnabled {
                    ledSet(1 as libc::c_int, 1 as libc::c_int != 0);
                } else {
                    ledSet(1 as libc::c_int, 0 as libc::c_int != 0);
                    flashesDone = flashesDone.wrapping_add(1)
                }
                if flashesDone as libc::c_int == actionCounter as libc::c_int
                   {
                    ledToggleAt =
                        (ledToggleAt as
                                                             ...
                                                             flashesDone as
                                                                 ...
                                                                 *
                                                                 2 as
                                                                     libc::c_int)
                                                            as libc::c_uint)
                            as timeMs_t as timeMs_t;
                    flashesDone = 0 as libc::c_int as uint8_t
                }
            }
            buttonWasPressed = 1 as libc::c_int != 0
        }
    }
    if !buttonWasPressed { return }
    ledSet(1 as libc::c_int, 0 as libc::c_int != 0);
    match actionCounter as libc::c_int {
        4 => {
            vtxCycleBandOrChannel(0 as libc::c_int as uint8_t,
                                  1 as libc::c_int as uint8_t);
        }
        3 => {
            vtxCycleBandOrChannel(1 as libc::c_int as uint8_t,
                                  0 as libc::c_int as uint8_t);
        }
        2 => { vtxCyclePower(1 as libc::c_int as uint8_t); }
        1 => { saveConfigAndNotify(); }
        _ => { }
    };
}
