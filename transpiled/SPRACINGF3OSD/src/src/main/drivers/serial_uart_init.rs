...
unsafe extern "C" fn usartConfigurePinInversion(mut uartPort:
    ...
    let mut inversionPins: uint32_t = 0 as libc::c_int as uint32_t;
    if (*uartPort).port.mode as libc::c_uint &
           MODE_TX as libc::c_int as libc::c_uint != 0 {
        inversionPins |= 0x20000 as libc::c_int as uint32_t
    }
    if (*uartPort).port.mode as libc::c_uint &
           MODE_RX as libc::c_int as libc::c_uint != 0 {
        inversionPins |= 0x10000 as libc::c_int as uint32_t
    }
...
pub unsafe extern "C" fn uartReconfigure(mut uartPort: *mut uartPort_t) {
    ...
    if (*uartPort).port.options as libc::c_uint &
        ...
        USART_InitStructure.USART_WordLength = 0 as libc::c_int as uint32_t
    }
    ...
    USART_InitStructure.USART_Mode = 0 as libc::c_int as uint32_t;
    if (*uartPort).port.mode as libc::c_uint &
           MODE_RX as libc::c_int as libc::c_uint != 0 {
        USART_InitStructure.USART_Mode |= 0x4 as libc::c_int as uint32_t
    }
    if (*uartPort).port.mode as libc::c_uint &
           MODE_TX as libc::c_int as libc::c_uint != 0 {
        USART_InitStructure.USART_Mode |= 0x8 as libc::c_int as uint32_t
    }
 ...
 -> *mut serialPort_t {
    ...
    if mode as libc::c_uint & MODE_RX as libc::c_int as libc::c_uint != 0 {
        if !(*s).rxDMAChannel.is_null() {
            ...
            USART_DMACmd((*s).USARTx, 0x40 as libc::c_int as uint32_t,
                         ENABLE);
            (*s).rxDMAPos =
                DMA_GetCurrDataCounter((*s).rxDMAChannel) as uint32_t
        } else {
            USART_ClearITPendingBit((*s).USARTx,
                                    0x50105 as libc::c_int as uint32_t);
            USART_ITConfig((*s).USARTx, 0x50105 as libc::c_int as uint32_t,
                           ENABLE);
        }
    }
    // Transmit DMA or IRQ
    if mode as libc::c_uint & MODE_TX as libc::c_int as libc::c_uint != 0 {
        if !(*s).txDMAChannel.is_null() {
            ...
            ::core::ptr::write_volatile(&mut (*(*s).txDMAChannel).CNDTR as
                                            *mut uint32_t,
                                        0 as libc::c_int as uint32_t);
            USART_DMACmd((*s).USARTx, 0x80 as libc::c_int as uint32_t,
                         ENABLE);
        } else {
