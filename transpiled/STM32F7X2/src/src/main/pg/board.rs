                             ...
                             reset:
                                 C2RustUnnamed_0{fn_0:
                                                     ::core::mem::transmute::<Option<unsafe extern "C" fn(_:
                                                                                         ...
                                                                                         ->
                                                                                             ()>,
                                                                              Option<pgResetFunc>>(Some(pgResetFn_boardConfig
                                                                                                            as
...
pub unsafe extern "C" fn pgResetFn_boardConfig(mut boardConfig:
    ...
    if boardInformationIsSet() {
        strncpy((*boardConfig).manufacturerId.as_mut_ptr(),
                getManufacturerId(), 4 as libc::c_int as libc::c_ulong);
        strncpy((*boardConfig).boardName.as_mut_ptr(), getBoardName(),
                20 as libc::c_int as libc::c_ulong);
        (*boardConfig).boardInformationSet = 1 as libc::c_int as uint8_t
    } else {
        (*boardConfig).boardInformationSet = 0 as libc::c_int as uint8_t
        // GENERIC_TARGET
    }
    if signatureIsSet() {
        ...
        (*boardConfig).signatureSet = 1 as libc::c_int as uint8_t
    } else { (*boardConfig).signatureSet = 0 as libc::c_int as uint8_t };
}
// USE_BOARD_INFO:
