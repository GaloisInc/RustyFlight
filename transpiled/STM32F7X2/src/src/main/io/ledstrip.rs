...
pub const timIndicator: timId_e = 7;
pub const timVtx: timId_e = 6;
pub const timWarning: timId_e = 5;
...
pub const timTimerCount: timId_e = 9;
pub const timRing: timId_e = 8;
pub const timGps: timId_e = 4;
...
pub const COLOR_ORANGE: C2RustUnnamed_8 = 3;
...
pub const COLOR_DEEP_PINK: C2RustUnnamed_8 = 13;
pub const COLOR_DARK_VIOLET: C2RustUnnamed_8 = 11;
pub const COLOR_BLUE: C2RustUnnamed_8 = 10;
pub const COLOR_GREEN: C2RustUnnamed_8 = 6;
pub const COLOR_YELLOW: C2RustUnnamed_8 = 4;
pub const COLOR_RED: C2RustUnnamed_8 = 2;
pub const COLOR_WHITE: C2RustUnnamed_8 = 1;
pub const COLOR_BLACK: C2RustUnnamed_8 = 0;
...
pub const COLOR_LIGHT_BLUE: C2RustUnnamed_8 = 9;
pub const COLOR_MINT_GREEN: C2RustUnnamed_8 = 7;
pub const COLOR_CYAN: C2RustUnnamed_8 = 8;
pub const COLOR_LIME_GREEN: C2RustUnnamed_8 = 5;
pub type C2RustUnnamed_8 = libc::c_uint;
pub const COLOR_MAGENTA: C2RustUnnamed_8 = 12;
#[inline]
...
unsafe extern "C" fn applyLedVtxLayer(mut updateNow: bool,
    ...
    if showSettings != 0 {
        ...
        if frequency as libc::c_int <= 5672 as libc::c_int {
            ...
            colorIndex = COLOR_DEEP_PINK as libc::c_int
        } // each interval starts with pause
...
unsafe extern "C" fn applyLedGpsLayer(mut updateNow: bool,
                                      mut timer: *mut timeUs_t) {
    static mut gpsPauseCounter: uint8_t = 0 as libc::c_int as uint8_t;
    let blinkPauseLength: uint8_t = 4 as libc::c_int as uint8_t;
    if updateNow {
        static mut gpsFlashCounter: uint8_t = 0 as libc::c_int as uint8_t;
        if gpsPauseCounter as libc::c_int > 0 as libc::c_int {
            gpsPauseCounter = gpsPauseCounter.wrapping_sub(1)
        } else if gpsFlashCounter as libc::c_int >=
                      gpsSol.numSat as libc::c_int {
            gpsFlashCounter = 0 as libc::c_int as uint8_t;
            gpsPauseCounter = blinkPauseLength
        } else {
            gpsFlashCounter = gpsFlashCounter.wrapping_add(1);
            gpsPauseCounter = 1 as libc::c_int as uint8_t
        }
        *timer =
            (*timer as
                 libc::c_uint).wrapping_add(((1000 as libc::c_int *
                                                  1000 as libc::c_int) as
                                                 libc::c_float / 2.5f32) as
                                                int32_t as libc::c_uint) as
                timeUs_t as timeUs_t
    }
    let mut gpsColor: *const hsvColor_t = 0 as *const hsvColor_t;
    if gpsSol.numSat as libc::c_int == 0 as libc::c_int ||
           !sensors(SENSOR_GPS as libc::c_int as uint32_t) {
        gpsColor = getSC(LED_SCOLOR_GPSNOSATS)
    } else {
        let mut colorOn: bool =
            gpsPauseCounter as libc::c_int == 0 as libc::c_int;
        if stateFlags as libc::c_int & GPS_FIX as libc::c_int != 0 {
            gpsColor =
                if colorOn as libc::c_int != 0 {
                    getSC(LED_SCOLOR_GPSLOCKED)
                } else { getSC(LED_SCOLOR_BACKGROUND) }
        } else {
            gpsColor =
                if colorOn as libc::c_int != 0 {
                    getSC(LED_SCOLOR_GPSNOLOCK)
                } else { getSC(LED_SCOLOR_GPSNOSATS) }
        }
    }
    applyLedHsv(((LED_FUNCTION_GPS as libc::c_int) << 8 as libc::c_int) as
                    uint32_t, gpsColor);
}
...
static mut timerVal: [timeUs_t; 9] = [0; 9];
static mut disabledTimerMask: uint16_t = 0;
static mut layerTable: [Option<applyLayerFn_timed>; 9] =
...
pub unsafe extern "C" fn ledStripUpdate(mut currentTimeUs: timeUs_t) {
    ...
    while (timId_0 as libc::c_ulong) <
              (::core::mem::size_of::<[Option<applyLayerFn_timed>; 9]>() as
