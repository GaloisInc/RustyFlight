...
//
    ...
    #[no_mangle]
    fn currentMeterMSPInit();
    #[no_mangle]
    fn currentMeterMSPRefresh(currentTimeUs: timeUs_t);
    #[no_mangle]
    fn currentMeterMSPRead(meter: *mut currentMeter_t);
    #[no_mangle]
...
pub static mut batteryConfig_Registry: pgRegistry_t =
    unsafe {
        {
            let mut init =
                pgRegistry_s{pgn: (11i32 | 2i32 << 12i32) as pgn_t,
                             size:
...
pub static mut pgResetTemplate_batteryConfig: batteryConfig_t =
    {
        let mut init =
            batteryConfig_s{vbatmaxcellvoltage: 43i32 as uint8_t,
                            vbatmincellvoltage: 33i32 as uint8_t,
                            vbatwarningcellvoltage: 35i32 as uint8_t,
                            vbatnotpresentcellvoltage: 30i32 as uint8_t,
                            lvcPercentage: 100i32 as uint8_t,
                            voltageMeterSource: VOLTAGE_METER_ADC,
                            currentMeterSource: CURRENT_METER_ADC,
                            batteryCapacity: 0i32 as uint16_t,
                            useVBatAlerts: 1i32 != 0,
                            useConsumptionAlerts: 0i32 != 0,
                            consumptionWarningPercentage: 10i32 as uint8_t,
                            vbathysteresis: 1i32 as uint8_t,
                            vbatfullcellvoltage: 41i32 as uint8_t,};
        init
    };
#[no_mangle]
pub unsafe extern "C" fn batteryUpdateVoltage(mut currentTimeUs: timeUs_t) {
    ...
    if debugMode as libc::c_int == DEBUG_BATTERY as libc::c_int {
        debug[0] = voltageMeter.unfiltered as int16_t;
        debug[1] = voltageMeter.filtered as int16_t
    };
}
...
pub unsafe extern "C" fn batteryUpdatePresence() {
    ...
    if (voltageState as libc::c_uint ==
        ...
        let mut cells: libc::c_uint =
            (voltageMeter.filtered as libc::c_int /
                 (*batteryConfig()).vbatmaxcellvoltage as libc::c_int + 1i32)
                as libc::c_uint;
        if cells > 8i32 as libc::c_uint {
            // something is wrong, we expect 8 cells maximum (and autodetection will be problematic at 6+ cells)
            cells = 8i32 as libc::c_uint
        }
        ...
        batteryCriticalVoltage =
            (batteryCellCount as libc::c_int *
                ...
                uint16_t;
        lowVoltageCutoff.percentage = 100i32 as uint8_t;
        lowVoltageCutoff.startTime = 0i32 as timeUs_t
        ...
        consumptionState = voltageState;
        batteryCellCount = 0i32 as uint8_t;
        batteryWarningVoltage = 0i32 as uint16_t;
        batteryCriticalVoltage = 0i32 as uint16_t
    }
    if debugMode as libc::c_int == DEBUG_BATTERY as libc::c_int {
        debug[2] = batteryCellCount as int16_t;
        debug[3] = isVoltageStable as int16_t
    };
}
...
unsafe extern "C" fn batteryUpdateLVC(mut currentTimeUs: timeUs_t) {
    if ((*batteryConfig()).lvcPercentage as libc::c_int) < 100i32 {
        if voltageState as libc::c_uint ==
               BATTERY_CRITICAL as libc::c_int as libc::c_uint &&
               !lowVoltageCutoff.enabled {
            lowVoltageCutoff.enabled = 1i32 != 0;
            lowVoltageCutoff.startTime = currentTimeUs;
            lowVoltageCutoff.percentage = 100i32 as uint8_t
        }
        if lowVoltageCutoff.enabled {
            if cmp32(currentTimeUs, lowVoltageCutoff.startTime) < 10000000i32
               {
                         ...
                         cmp32(currentTimeUs, lowVoltageCutoff.startTime) *
                             (100i32 -
                                  (*batteryConfig()).lvcPercentage as
                                      libc::c_int) / 10000000i32) as uint8_t
            } else {
...
pub unsafe extern "C" fn batteryInit() {
    ...
    batteryWarningVoltage = 0i32 as uint16_t;
    batteryCriticalVoltage = 0i32 as uint16_t;
    lowVoltageCutoff.enabled = 0i32 != 0;
    lowVoltageCutoff.percentage = 100i32 as uint8_t;
    lowVoltageCutoff.startTime = 0i32 as timeUs_t;
...
unsafe extern "C" fn batteryUpdateConsumptionState() {
    if (*batteryConfig()).useConsumptionAlerts as libc::c_int != 0 &&
           (*batteryConfig()).batteryCapacity as libc::c_int > 0i32 &&
           batteryCellCount as libc::c_int > 0i32 {
        let mut batteryPercentageRemaining: uint8_t =
            calculateBatteryPercentageRemaining();
        if batteryPercentageRemaining as libc::c_int == 0i32 {
...
pub unsafe extern "C" fn batteryUpdateCurrentMeter(mut currentTimeUs:
                                                       timeUs_t) {
    if batteryCellCount as libc::c_int == 0i32 {
        currentMeterReset(&mut currentMeter);
        return
    }
    static mut ibatLastServiced: uint32_t = 0i32 as uint32_t;
    ...
    match (*batteryConfig()).currentMeterSource as libc::c_uint {
        ...
        4 => {
            ...
            currentMeterMSPRead(&mut currentMeter);
        }
...
pub unsafe extern "C" fn calculateBatteryPercentageRemaining() -> uint8_t {
    let mut batteryPercentage: uint8_t = 0i32 as uint8_t;
    if batteryCellCount as libc::c_int > 0i32 {
        let mut batteryCapacity: uint16_t =
            (*batteryConfig()).batteryCapacity;
        if batteryCapacity as libc::c_int > 0i32 {
            batteryPercentage =
                constrain(((batteryCapacity as libc::c_float -
                               ...
                               batteryCapacity as libc::c_int as
                                   libc::c_float) as libc::c_int, 0i32,
                          100i32) as uint8_t
        } else {
            batteryPercentage =
                constrain((voltageMeter.filtered as
                               uint32_t).wrapping_sub(((*batteryConfig()).vbatmincellvoltage
                                                           ...
                                                           batteryCellCount as
                                                               libc::c_int) as
                                                          libc::c_uint).wrapping_mul(100i32
                                                                                         as
                                                                                         libc::c_uint).wrapping_div((((*batteryConfig()).vbatmaxcellvoltage
                                                                                                                        ...
                                                                                                                        as
                                                                                                                        libc::c_uint)
                              as libc::c_int, 0i32, 100i32) as uint8_t
        }
    }
