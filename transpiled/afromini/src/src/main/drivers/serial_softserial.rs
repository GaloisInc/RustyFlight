...
pub const GPIO_Mode_AN: C2RustUnnamed_0 = 3;
/* !< GPIO Output Mode */
pub const GPIO_Mode_AF: C2RustUnnamed_0 = 2;
/* !< GPIO Input Mode */
pub const GPIO_Mode_OUT: C2RustUnnamed_0 = 1;
pub const GPIO_Mode_IN: C2RustUnnamed_0 = 0;
/* *
  * @}
  */
...
pub const GPIO_OType_OD: C2RustUnnamed_1 = 1;
pub const GPIO_OType_PP: C2RustUnnamed_1 = 0;
/* *
  * @}
  */
...
unsafe extern "C" fn serialInputPortActivate(mut softSerial:
    ...
    if (*softSerial).port.options as libc::c_uint &
        ...
        let pinConfig: uint8_t =
            if (*softSerial).port.options as libc::c_uint &
                ...
                (GPIO_Mode_AF as libc::c_int | 0i32 << 2i32 |
                     (GPIO_OType_PP as libc::c_int) << 4i32) |
                    (GPIO_PuPd_DOWN as libc::c_int) << 5i32
            } as uint8_t;
    ...
    (*softSerial).rxActive = 1i32 as uint8_t;
    (*softSerial).isSearchingForStartBit = 1i32 as uint8_t;
    (*softSerial).rxBitIndex = 0i32 as uint8_t;
...
unsafe extern "C" fn serialInputPortDeActivate(mut softSerial:
    ...
    IOConfigGPIO((*softSerial).rxIO,
                 (GPIO_Mode_IN as libc::c_int | 0i32 << 2i32 | 0i32 << 4i32 |
                     ...
                     ioConfig_t);
    (*softSerial).rxActive = 0i32 as uint8_t;
}
...
unsafe extern "C" fn resetBuffers(mut softSerial: *mut softSerial_t) {
    (*softSerial).port.rxBufferSize = 256i32 as uint32_t;
    (*softSerial).port.rxBuffer = (*softSerial).rxBuffer.as_mut_ptr();
    (*softSerial).port.rxBufferTail = 0i32 as uint32_t;
    (*softSerial).port.rxBufferHead = 0i32 as uint32_t;
    (*softSerial).port.txBuffer = (*softSerial).txBuffer.as_mut_ptr();
    (*softSerial).port.txBufferSize = 256i32 as uint32_t;
    (*softSerial).port.txBufferTail = 0i32 as uint32_t;
    (*softSerial).port.txBufferHead = 0i32 as uint32_t;
}
#[no_mangle]
pub unsafe extern "C" fn openSoftSerial(mut portIndex: softSerialPortIndex_e,
 ...
 -> *mut serialPort_t {
    ...
    let mut pinCfgIndex: libc::c_int =
        (portIndex as libc::c_uint).wrapping_add(10i32 as libc::c_uint) as
            libc::c_int;
    ...
    if options as libc::c_uint & SERIAL_BIDIR as libc::c_int as libc::c_uint
        ...
        IOInit(txIO, OWNER_SERIAL_TX,
               (portIndex as
                    libc::c_uint).wrapping_add(10i32 as
    ...
    (*softSerial).transmissionErrors = 0i32 as uint16_t;
    (*softSerial).receiveErrors = 0i32 as uint16_t;
    (*softSerial).rxActive = 0i32 as uint8_t;
    (*softSerial).isTransmittingData = 0i32 as uint8_t;
...
pub unsafe extern "C" fn processTxState(mut softSerial: *mut softSerial_t) {
    ...
    if (*softSerial).isTransmittingData == 0 {
        ...
        (*softSerial).internalTxBuffer =
            (1i32 << 10i32 - 1i32 | (byteToSend as libc::c_int) << 1i32) as
                uint16_t;
        (*softSerial).bitsLeftToTransmit = 10i32 as int8_t;
        (*softSerial).isTransmittingData = 1i32 as uint8_t;
    ...
    if (*softSerial).bitsLeftToTransmit != 0 {
        mask =
            ((*softSerial).internalTxBuffer as libc::c_int & 1i32) as uint8_t;
        (*softSerial).internalTxBuffer =
            ((*softSerial).internalTxBuffer as libc::c_int >> 1i32) as
                uint16_t;
...
pub unsafe extern "C" fn extractAndStoreRxByte(mut softSerial:
    ...
    let mut haveStartBit: uint8_t =
        ((*softSerial).internalRxBuffer as libc::c_int & 1i32 << 10i32 - 1i32
             == 0i32) as libc::c_int as uint8_t;
    let mut haveStopBit: uint8_t =
        ((*softSerial).internalRxBuffer as libc::c_int & 1i32 << 0i32 == 1i32)
    ...
    let mut rxByte: uint8_t =
        ((*softSerial).internalRxBuffer as libc::c_int >> 1i32 & 0xffi32) as
            uint8_t;
    if (*softSerial).port.rxCallback.is_some() {
        ...
        (*softSerial).port.rxBufferHead =
            (*softSerial).port.rxBufferHead.wrapping_add(1i32 as
...
pub unsafe extern "C" fn processRxState(mut softSerial: *mut softSerial_t) {
    ...
    if (*softSerial).rxBitIndex as libc::c_int == 10i32 - 1i32 {
        applyChangedBits(softSerial);
        return
    }
    if (*softSerial).rxBitIndex as libc::c_int == 10i32 {
        if (*softSerial).rxEdge as libc::c_int == TRAILING as libc::c_int {
            (*softSerial).internalRxBuffer =
                ((*softSerial).internalRxBuffer as libc::c_int | 1i32 << 0i32)
                    as uint16_t
        }
...
pub unsafe extern "C" fn onSerialTimerOverflow(mut cbRec:
    ...
    let mut self_0: *mut softSerial_t =
        ({
             ...
             (__mptr as *mut libc::c_char).offset(-656) as *mut softSerial_t
         });
...
pub unsafe extern "C" fn onSerialRxPinChange(mut cbRec:
    ...
    let mut self_0: *mut softSerial_t =
        ({
             ...
             (__mptr as *mut libc::c_char).offset(-672) as *mut softSerial_t
         });
    ...
    if (*self_0).isSearchingForStartBit != 0 {
        ...
        TIM_SetCounter((*(*self_0).timerHardware).tim,
                       (*(*(*self_0).timerHardware).tim).ARR.wrapping_div(2i32
                                                                              as
                                                                              libc::c_uint));
        ...
        (*self_0).rxBitIndex = 0i32 as uint8_t;
        (*self_0).rxLastLeadingEdgeAtBitIndex = 0i32 as uint8_t;
        (*self_0).internalRxBuffer = 0i32 as uint16_t;
        (*self_0).isSearchingForStartBit = 0i32 as uint8_t;
        return
    }
...
pub unsafe extern "C" fn softSerialReadByte(mut instance: *mut serialPort_t)
 -> uint8_t {
    ...
    if softSerialRxBytesWaiting(instance) == 0i32 as libc::c_uint {
        return 0i32 as uint8_t
    }
