...
pub const ESC_INFO_BLHELI32: C2RustUnnamed_16 = 2;
pub const ESC_INFO_KISS_V2: C2RustUnnamed_16 = 1;
pub const ESC_INFO_KISS_V1: C2RustUnnamed_16 = 0;
...
#[no_mangle]
pub static mut cmdTable: [clicmd_t; 42] =
    unsafe {
        [{
             let mut init =
                 clicmd_t{name:
...
unsafe extern "C" fn cliHelp(mut cmdline: *mut libc::c_char) {
    let mut i: uint32_t = 0 as libc::c_int as uint32_t;
    while (i as libc::c_ulong) <
              (::core::mem::size_of::<[clicmd_t; 42]>() as
...
pub unsafe extern "C" fn cliProcess() {
    ...
    while serialRxBytesWaiting(cliPort) != 0 {
        ...
        if c as libc::c_int == '\t' as i32 || c as libc::c_int == '?' as i32 {
            ...
            cmd = cmdTable.as_ptr();
            while cmd <
                      cmdTable.as_ptr().offset((::core::mem::size_of::<[clicmd_t; 42]>()
                                                    as
                                                    libc::c_ulong).wrapping_div(::core::mem::size_of::<clicmd_t>()
                                                                                    as
                                                                                    libc::c_ulong)
