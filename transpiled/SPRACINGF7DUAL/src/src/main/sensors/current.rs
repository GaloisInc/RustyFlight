...
pub static mut pgResetTemplate_currentSensorADCConfig:
    ...
    {
        let mut init =
            currentSensorADCConfig_s{scale: 300 as libc::c_int as int16_t,
...
pub unsafe extern "C" fn currentMeterMSPRefresh(mut currentTimeUs: timeUs_t) {
    ...
    static mut streamRequestAt: timeUs_t =
        0 as libc::c_int as timeUs_t; // 10hz
    if cmp32(currentTimeUs, streamRequestAt) > 0 as libc::c_int {
        streamRequestAt =
            currentTimeUs.wrapping_add((1000 as libc::c_int *
                                            1000 as libc::c_int /
                                            10 as libc::c_int) as
                                           libc::c_uint);
        mspSerialPush(110 as libc::c_int as uint8_t, 0 as *mut uint8_t,
                      0 as libc::c_int, MSP_DIRECTION_REQUEST);
    };
}
#[no_mangle]
