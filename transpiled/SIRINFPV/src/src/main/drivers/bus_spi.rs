...
pub unsafe extern "C" fn spiDeviceByInstance(mut instance: *mut SPI_TypeDef)
 -> SPIDevice {
    ...
    if instance ==
           (0x40000000 as libc::c_int as
                uint32_t).wrapping_add(0x3800 as libc::c_int as libc::c_uint)
               as *mut SPI_TypeDef {
        return SPIDEV_2
    }
    if instance ==
           (0x40000000 as libc::c_int as
                uint32_t).wrapping_add(0x3c00 as libc::c_int as libc::c_uint)
               as *mut SPI_TypeDef {
        return SPIDEV_3
    }
    return SPIINVALID;
...
pub unsafe extern "C" fn spiInit(mut device: SPIDevice) -> bool {
    match device as libc::c_int {
        ...
        1 => { spiInitDevice(device); return 1 as libc::c_int != 0 }
        2 => { spiInitDevice(device); return 1 as libc::c_int != 0 }
        3 | _ => { }
    }
...
pub unsafe extern "C" fn spiBusReadRegister(mut bus: *const busDevice_t,
    ...
    let mut data: uint8_t = 0; // read transaction
    IOLo((*bus).busdev_u.spi.csnPin);
    spiTransferByte((*bus).busdev_u.spi.instance,
                    (reg as libc::c_int | 0x80 as libc::c_int) as uint8_t);
