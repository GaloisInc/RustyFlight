 ...
 * If not, see <http://www.gnu.org/licenses/>.
 */
unsafe extern "C" fn icm20689SpiInit(mut bus: *const busDevice_t) {
    static mut hardwareInitialised: bool = 0 as libc::c_int != 0; //low speed
...
#[no_mangle]
pub unsafe extern "C" fn icm20689SpiDetect(mut bus: *const busDevice_t)
 -> uint8_t {
    icm20689SpiInit(bus);
    spiSetDivisor((*bus).busdev_u.spi.instance,
    ...
    loop  {
        ...
        match whoAmI as libc::c_int {
            172 => { icmDetected = ICM_20601_SPI as libc::c_int as uint8_t }
            18 => { icmDetected = ICM_20602_SPI as libc::c_int as uint8_t }
            175 => { icmDetected = ICM_20608_SPI as libc::c_int as uint8_t }
            152 => { icmDetected = ICM_20689_SPI as libc::c_int as uint8_t }
            _ => { icmDetected = MPU_NONE as libc::c_int as uint8_t }
        }
...
#[no_mangle]
pub unsafe extern "C" fn icm20689AccInit(mut acc: *mut accDev_t) {
    (*acc).acc_1G = (512 as libc::c_int * 4 as libc::c_int) as uint16_t;
}
#[no_mangle]
pub unsafe extern "C" fn icm20689SpiAccDetect(mut acc: *mut accDev_t)
 -> bool {
    ...
    (*acc).initFn =
        Some(icm20689AccInit as unsafe extern "C" fn(_: *mut accDev_t) -> ());
    (*acc).readFn =
        Some(mpuAccRead as unsafe extern "C" fn(_: *mut accDev_s) -> bool);
    return 1 as libc::c_int != 0;
}
#[no_mangle]
pub unsafe extern "C" fn icm20689GyroInit(mut gyro: *mut gyroDev_t) {
    mpuGyroInit(gyro);
...
#[no_mangle]
pub unsafe extern "C" fn icm20689SpiGyroDetect(mut gyro: *mut gyroDev_t)
 -> bool {
