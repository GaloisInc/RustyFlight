...
unsafe extern "C" fn hmc5883lConfigureDataReadyInterruptHandling(mut mag:
                                                                     *mut magDev_t) {
    if (*mag).magIntExtiTag as libc::c_int == 0 as libc::c_int { return }
    let magIntIO: IO_t = IOGetByTag((*mag).magIntExtiTag);
    let mut status: uint8_t = IORead(magIntIO) as uint8_t;
    if status == 0 { return }
    IOInit(magIntIO, OWNER_COMPASS_EXTI, 0 as libc::c_int as uint8_t);
    ...
    EXTIConfig(magIntIO, &mut (*mag).exti,
               ...
               EXTI_Trigger_Rising);
    EXTIEnable(magIntIO, 1 as libc::c_int != 0);
}
