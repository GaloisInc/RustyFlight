...
pub const PWM_TYPE_DSHOT300: C2RustUnnamed_5 = 6;
pub const PWM_TYPE_DSHOT150: C2RustUnnamed_5 = 5;
...
pub const PWM_TYPE_MAX: C2RustUnnamed_5 = 10;
pub const PWM_TYPE_PROSHOT1000: C2RustUnnamed_5 = 9;
pub const PWM_TYPE_DSHOT1200: C2RustUnnamed_5 = 8;
pub const PWM_TYPE_DSHOT600: C2RustUnnamed_5 = 7;
...
pub static mut pgResetTemplate_systemConfig: systemConfig_t =
    {
        let mut init =
            systemConfig_s{pidProfileIndex: 0 as libc::c_int as uint8_t,
                           ...
                           boardIdentifier: [69, 65, 67, 72, 0, 0],};
        init
    };
#[no_mangle]
...
unsafe extern "C" fn validateAndFixConfig() {
    ...
    if !(featureConfigured(FEATURE_RX_PARALLEL_PWM as libc::c_int as uint32_t)
             ...
             featureConfigured(FEATURE_RX_SPI as libc::c_int as uint32_t) as
                 libc::c_int != 0) {
        featureSet(FEATURE_RX_SPI as libc::c_int as uint32_t);
    }
    ...
    if featureConfigured(FEATURE_RX_SPI as libc::c_int as uint32_t) {
        featureClear((FEATURE_RX_SERIAL as libc::c_int |
                          FEATURE_RX_PARALLEL_PWM as libc::c_int |
                          FEATURE_RX_PPM as libc::c_int |
                          FEATURE_RX_MSP as libc::c_int) as uint32_t);
    }
    // USE_RX_SPI
    if featureConfigured(FEATURE_RX_PARALLEL_PWM as libc::c_int as uint32_t) {
        featureClear((FEATURE_RX_SERIAL as libc::c_int |
