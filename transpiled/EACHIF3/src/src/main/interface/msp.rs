    ...
    match cmdMSP as libc::c_int {
        ...
        134 => {
            if feature(FEATURE_ESC_SENSOR as libc::c_int as uint32_t) {
                ...
                while i_9 < getMotorCount() as libc::c_int {
                    let mut escData: *const escSensorData_t =
                        getEscSensorData(i_9 as uint8_t);
        ...
        77 => {
            ...
            while i_10 < rxRuntimeConfig.channelCount as libc::c_int {
                sbufWriteU8(dst, (*rxFailsafeChannelConfigs(i_10)).mode);
                sbufWriteU16(dst,
                             (750 as libc::c_int +
                                  25 as libc::c_int *
                                      (*rxFailsafeChannelConfigs(i_10)).step
        ...
        54 => {
            ...
            while i_11 < 3 as libc::c_int {
                if serialIsPortAvailable((*serialConfig()).portConfigs[i_11 as
                                                                           usize].identifier)
                   {
                    sbufWriteU8(dst,
                                (*serialConfig()).portConfigs[i_11 as
                                                                  usize].identifier
                                    as uint8_t);
                    sbufWriteU16(dst,
                                 (*serialConfig()).portConfigs[i_11 as
        ...
        46 => {
            ...
            while i_12 < 16 as libc::c_int {
                let mut color: *const hsvColor_t =
                    &*(*(ledStripConfig as
                             unsafe extern "C" fn()
                                 ->
                                     *const ledStripConfig_t)()).colors.as_ptr().offset(i_12
                                                                                            as
                                                                                            isize)
                        as *const hsvColor_t;
        ...
        48 => {
            ...
            while i_13 < 32 as libc::c_int {
                let mut ledConfig: *const ledConfig_t =
                    &*(*(ledStripConfig as
                             unsafe extern "C" fn()
                                 ->
                                     *const ledStripConfig_t)()).ledConfigs.as_ptr().offset(i_13
                                                                                                as
                                                                                                isize)
        ...
        127 => {
            ...
            while i_14 < 6 as libc::c_int {
                ...
                while j < 6 as libc::c_int {
                    sbufWriteU8(dst, i_14 as uint8_t);
                    sbufWriteU8(dst, j as uint8_t);
                    sbufWriteU8(dst,
                                (*ledStripConfig()).modeColors[i_14 as
                                                                   usize].color[j
                                                                                    as
                                                                                    usize]);
                    j += 1
