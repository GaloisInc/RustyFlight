...
pub unsafe extern "C" fn COMP_StructInit(mut COMP_InitStruct:
                                             *mut COMP_InitTypeDef) {
    (*COMP_InitStruct).COMP_InvertingInput = 0i32 as uint32_t;
    (*COMP_InitStruct).COMP_NonInvertingInput = 0i32 as uint32_t;
    (*COMP_InitStruct).COMP_Output = 0i32 as uint32_t;
    (*COMP_InitStruct).COMP_BlankingSrce = 0i32 as uint32_t;
    (*COMP_InitStruct).COMP_OutputPol = 0i32 as uint32_t;
    (*COMP_InitStruct).COMP_Hysteresis = 0i32 as uint32_t;
    (*COMP_InitStruct).COMP_Mode = 0xci32 as uint32_t;
}
/* *
...
pub unsafe extern "C" fn COMP_GetOutputLevel(mut COMP_Selection: uint32_t)
 -> uint32_t {
    let mut compout: uint32_t = 0i32 as uint32_t;
    /* Check the parameters */
    /* Check if selected comparator output is high */
    if *((0x40000000i32 as
              uint32_t).wrapping_add(0x10000i32 as
                                         libc::c_uint).wrapping_add(0x1ci32 as
                                                                        libc::c_uint).wrapping_add(COMP_Selection)
             as *mut uint32_t) & 0x40000000i32 as uint32_t !=
           0i32 as libc::c_uint {
        compout = 0x40000000i32 as uint32_t
    } else { compout = 0i32 as uint32_t }
    ...
    if NewState as libc::c_uint != DISABLE as libc::c_int as libc::c_uint {
        ...
        ::core::ptr::write_volatile(fresh5,
                                    (::core::ptr::read_volatile::<uint32_t>(fresh5
                                                                                ...
                                                                                *const uint32_t)
                                         as libc::c_uint &
                                         !(0x200i32 as uint32_t)) as uint32_t
                                        as uint32_t)
    };
}
/* *
