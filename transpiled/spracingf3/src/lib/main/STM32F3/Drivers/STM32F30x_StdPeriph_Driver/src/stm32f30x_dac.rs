...
pub unsafe extern "C" fn DAC_DeInit(mut DACx: *mut DAC_TypeDef) {
    ...
    if DACx ==
           (0x40000000i32 as uint32_t).wrapping_add(0x7400i32 as libc::c_uint)
               as *mut DAC_TypeDef {
        /* Enable DAC1 reset state */
        RCC_APB1PeriphResetCmd(0x20000000i32 as uint32_t, ENABLE);
        /* Release DAC1 from reset state */
        RCC_APB1PeriphResetCmd(0x20000000i32 as uint32_t, DISABLE);
    } else {
        /* Enable DAC2 reset state */
        RCC_APB1PeriphResetCmd(0x4000000i32 as uint32_t, ENABLE);
        /* Release DAC2 from reset state */
        RCC_APB1PeriphResetCmd(0x4000000i32 as uint32_t, DISABLE);
    };
}
/* *
...
pub unsafe extern "C" fn DAC_Init(mut DACx: *mut DAC_TypeDef,
  ...
  /* Get the DAC CR value */
    ...
    tmpreg1 &= !((0xffei32 as uint32_t) << DAC_Channel);
...
pub unsafe extern "C" fn DAC_StructInit(mut DAC_InitStruct:
  ...
  /* Initialize the DAC_Trigger member */
    (*DAC_InitStruct).DAC_Trigger = 0i32 as uint32_t;
    /* Initialize the DAC_WaveGeneration member */
    (*DAC_InitStruct).DAC_WaveGeneration = 0i32 as uint32_t;
    /* Initialize the DAC_LFSRUnmask_TriangleAmplitude member */
    (*DAC_InitStruct).DAC_LFSRUnmask_TriangleAmplitude = 0i32 as uint32_t;
    /* Initialize the DAC_Buffer_Switch member */
    (*DAC_InitStruct).DAC_Buffer_Switch = 0x2i32 as uint32_t;
}
/* *
    ...
    if NewState as libc::c_uint != DISABLE as libc::c_int as libc::c_uint {
        ...
        ::core::ptr::write_volatile(&mut (*DACx).CR as *mut uint32_t,
                                    (::core::ptr::read_volatile::<uint32_t>(&(*DACx).CR
                                                                                ...
                                                                                *const uint32_t)
                                         as libc::c_uint |
                                         (0x1i32 as uint32_t) << DAC_Channel)
    ...
    if NewState as libc::c_uint != DISABLE as libc::c_int as libc::c_uint {
        ...
        ::core::ptr::write_volatile(&mut (*DACx).SWTRIGR as *mut uint32_t,
                                    (::core::ptr::read_volatile::<uint32_t>(&(*DACx).SWTRIGR
                                                                                as
                                                                                *const uint32_t)
                                         as libc::c_uint | 0x3i32 as uint32_t)
                                        as uint32_t as uint32_t)
    } else {
        ...
        ::core::ptr::write_volatile(&mut (*DACx).SWTRIGR as *mut uint32_t,
                                    (::core::ptr::read_volatile::<uint32_t>(&(*DACx).SWTRIGR
                                                                                as
                                                                                *const uint32_t)
                                         as libc::c_uint & 0xfffffffcu32) as
                                        uint32_t as uint32_t)
    };
}
/* *
...
pub unsafe extern "C" fn DAC_SetChannel1Data(mut DACx: *mut DAC_TypeDef,
    ...
    ::core::ptr::write_volatile(&mut tmp as *mut uint32_t,
                                (::core::ptr::read_volatile::<uint32_t>(&tmp
                                                                            ...
                                                                            *const uint32_t)
                                     as
                                     libc::c_uint).wrapping_add((0x8i32 as
...
pub unsafe extern "C" fn DAC_SetChannel2Data(mut DACx: *mut DAC_TypeDef,
    ...
    ::core::ptr::write_volatile(&mut tmp as *mut uint32_t,
                                (::core::ptr::read_volatile::<uint32_t>(&tmp
                                                                            ...
                                                                            *const uint32_t)
                                     as
                                     libc::c_uint).wrapping_add((0x14i32 as
                                                                     uint32_t).wrapping_add(DAC_Align))
                                    as uint32_t as uint32_t);
...
pub unsafe extern "C" fn DAC_SetDualChannelData(mut DACx: *mut DAC_TypeDef,
                                                ...
                                                mut Data2: uint16_t,
                                                mut Data1: uint16_t) {
    let mut data: uint32_t = 0i32 as uint32_t;
    let mut tmp: uint32_t = 0i32 as uint32_t;
    /* Check the parameters */
    /* Calculate and set dual DAC data holding register value */
    if DAC_Align == 0x8i32 as uint32_t {
        data = (Data2 as uint32_t) << 8i32 | Data1 as libc::c_uint
    } else { data = (Data2 as uint32_t) << 16i32 | Data1 as libc::c_uint }
    tmp = DACx as uint32_t;
    tmp =
        (tmp as
             libc::c_uint).wrapping_add((0x20i32 as
                                             uint32_t).wrapping_add(DAC_Align))
            as uint32_t as uint32_t;
 ...
 -> uint16_t {
    ...
    ::core::ptr::write_volatile(&mut tmp as *mut uint32_t,
                                (::core::ptr::read_volatile::<uint32_t>(&tmp
                                                                            ...
                                                                            *const uint32_t)
                                     as
                                     libc::c_uint).wrapping_add((0x2ci32 as
