...
pub static mut pgResetTemplate_systemConfig: systemConfig_t =
    {
        let mut init =
            systemConfig_s{pidProfileIndex: 0i32 as uint8_t,
                           ...
                           boardIdentifier: [83, 82, 70, 51, 0, 0],};
        init
    };
#[no_mangle]
...
unsafe extern "C" fn validateAndFixConfig() {
    ...
    if featureConfigured(FEATURE_RSSI_ADC as libc::c_int as uint32_t) {
        (*rxConfigMutable()).rssi_channel = 0i32 as uint8_t;
...
// I have kept them all here in one place, some could be moved to sections of code above.
    ...
    if (*beeperDevConfig()).frequency as libc::c_int != 0 &&
        ...
        (*beeperDevConfigMutable()).frequency = 0i32 as uint16_t
    }
    if (*beeperConfig()).beeper_off_flags &
           !(1i32 << BEEPER_GYRO_CALIBRATED as libc::c_int - 1i32 |
                 1i32 << BEEPER_RX_LOST as libc::c_int - 1i32 |
                 1i32 << BEEPER_RX_LOST_LANDING as libc::c_int - 1i32 |
                 1i32 << BEEPER_DISARMING as libc::c_int - 1i32 |
                 1i32 << BEEPER_ARMING as libc::c_int - 1i32 |
                 1i32 << BEEPER_ARMING_GPS_FIX as libc::c_int - 1i32 |
                 1i32 << BEEPER_BAT_CRIT_LOW as libc::c_int - 1i32 |
                 1i32 << BEEPER_BAT_LOW as libc::c_int - 1i32 |
                 1i32 << BEEPER_GPS_STATUS as libc::c_int - 1i32 |
                 1i32 << BEEPER_RX_SET as libc::c_int - 1i32 |
                 1i32 << BEEPER_ACC_CALIBRATION as libc::c_int - 1i32 |
                 1i32 << BEEPER_ACC_CALIBRATION_FAIL as libc::c_int - 1i32 |
                 1i32 << BEEPER_READY_BEEP as libc::c_int - 1i32 |
                 1i32 << BEEPER_MULTI_BEEPS as libc::c_int - 1i32 |
                 1i32 << BEEPER_DISARM_REPEAT as libc::c_int - 1i32 |
                 1i32 << BEEPER_ARMED as libc::c_int - 1i32 |
                 1i32 << BEEPER_SYSTEM_INIT as libc::c_int - 1i32 |
                 1i32 << BEEPER_USB as libc::c_int - 1i32 |
                 1i32 << BEEPER_BLACKBOX_ERASE as libc::c_int - 1i32 |
                 1i32 << BEEPER_CRASH_FLIP_MODE as libc::c_int - 1i32 |
                 1i32 << BEEPER_CAM_CONNECTION_OPEN as libc::c_int - 1i32 |
                 1i32 << BEEPER_CAM_CONNECTION_CLOSE as libc::c_int - 1i32 |
                 1i32 << BEEPER_RC_SMOOTHING_INIT_FAIL as libc::c_int - 1i32)
               as libc::c_uint != 0 {
        (*beeperConfigMutable()).beeper_off_flags = 0i32 as uint32_t
    }
    if (*beeperConfig()).dshotBeaconOffFlags &
           !(1i32 << BEEPER_RX_LOST as libc::c_int - 1i32 |
                 1i32 << BEEPER_RX_SET as libc::c_int - 1i32) as libc::c_uint
           != 0 {
        (*beeperConfigMutable()).dshotBeaconOffFlags = 0i32 as uint32_t
    }
...
pub unsafe extern "C" fn validateAndFixGyroConfig() {
    ...
    if gyro.targetLooptime > (1000000i32 / 2000i32) as libc::c_uint {
