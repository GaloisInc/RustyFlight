...
extern "C" {
    ...
    fn uartOpen(device: UARTDevice_e, rxCallback: serialReceiveCallbackPtr,
     ...
     -> *mut serialPort_t;
    #[no_mangle]
    fn openSoftSerial(portIndex: softSerialPortIndex_e,
                      rxCallback: serialReceiveCallbackPtr,
                      rxCallbackData: *mut libc::c_void, baud: uint32_t,
                      mode: portMode_e, options: portOptions_e)
     -> *mut serialPort_t;
    #[no_mangle]
 ...
 * If not, see <http://www.gnu.org/licenses/>.
 */
static mut serialPortUsageList: [serialPortUsage_t; 5] =
    [serialPortUsage_t{serialPort:
                           0 as *const serialPort_t as *mut serialPort_t,
                       function: FUNCTION_NONE,
                       identifier: SERIAL_PORT_USART1,}; 5];
#[no_mangle]
pub static mut serialPortIdentifiers: [serialPortIdentifier_e; 5] =
    [SERIAL_PORT_USART1, SERIAL_PORT_USART2, SERIAL_PORT_USART3,
     SERIAL_PORT_SOFTSERIAL1, SERIAL_PORT_SOFTSERIAL2];
...
pub unsafe extern "C" fn findSerialPortUsageByIdentifier(mut identifier:
 ...
 -> *mut serialPortUsage_t {
    let mut index: uint8_t = 0;
    index = 0i32 as uint8_t;
    while (index as libc::c_int) < 5i32 {
...
pub unsafe extern "C" fn findNextSerialPortConfig(mut function:
                                                      serialPortFunction_e)
 -> *mut serialPortConfig_t {
    while (findSerialPortConfigState.lastIndex as libc::c_int) < 5i32 {
...
pub unsafe extern "C" fn openSerialPort(mut identifier:
 ...
 -> *mut serialPort_t {
    ...
    match identifier as libc::c_int {
        ...
        30 => {
            serialPort =
                openSoftSerial(SOFTSERIAL1, rxCallback, rxCallbackData,
...
pub unsafe extern "C" fn serialInit(mut softserialEnabled: bool,
                                    mut serialPortToDisable:
                                        serialPortIdentifier_e) {
    serialPortCount = 5i32 as uint8_t;
    memset(&mut serialPortUsageList as *mut [serialPortUsage_t; 5] as
               *mut libc::c_void, 0i32,
           ::core::mem::size_of::<[serialPortUsage_t; 5]>() as libc::c_ulong);
    let mut index: libc::c_int = 0i32;
    while index < 5i32 {
        ...
        if serialPortToDisable as libc::c_int !=
            ...
            if !((*serialPinConfig()).ioTagTx[resourceIndex as usize] as
                ...
                serialPortCount = serialPortCount.wrapping_sub(1)
                      ...
                      !(softserialEnabled as libc::c_int != 0 &&
                            ((*serialPinConfig()).ioTagTx[(10i32 +
                                                               SOFTSERIAL1 as
                                                                   libc::c_int)
                                                              as usize] as
                                 libc::c_int != 0 ||
                                 (*serialPinConfig()).ioTagRx[(10i32 +
                                                                   SOFTSERIAL1
                                                                       as
                                                                       libc::c_int)
                                                                  as usize] as
