 ...
 */
static mut bufferHead: uint8_t = 0i32 as uint8_t;
static mut bufferTail: uint8_t = 0i32 as uint8_t;
// The position of the buffer's tail in the overall flash address space:
static mut tailAddress: uint32_t = 0i32 as uint32_t;
unsafe extern "C" fn flashfsClearBuffer() {
    bufferHead = 0i32 as uint8_t;
...
pub unsafe extern "C" fn flashfsEraseCompletely() {
    ...
    flashfsClearBuffer();
    flashfsSetTailAddress(0i32 as uint32_t);
}
/* *
...
pub unsafe extern "C" fn flashfsEraseRange(mut start: uint32_t,
    ...
    if (*geometry).sectorSize <= 0i32 as libc::c_uint { return }
    ...
    if endRemainder > 0i32 { endSector += 1 }
...
pub unsafe extern "C" fn flashfsIsSupported() -> bool {
    return flashfsGetSize() > 0i32 as libc::c_uint;
}
#[no_mangle]
...
unsafe extern "C" fn flashfsTransmitBufferUsed() -> uint32_t {
    ...
    return (128i32 - bufferTail as libc::c_int + bufferHead as libc::c_int) as
               uint32_t;
}
/* *
...
pub unsafe extern "C" fn flashfsGetWriteBufferSize() -> uint32_t {
    return (128i32 - 1i32) as uint32_t;
}
/* *
...
unsafe extern "C" fn flashfsWriteBuffers(mut buffers: *mut *const uint8_t,
    ...
    let mut bytesTotal: uint32_t = 0i32 as uint32_t;
    ...
    while bytesTotalRemaining > 0i32 as libc::c_uint {
        ...
        if flashfsIsEOF() {
            ...
            i = 0i32;
            while i < bufferCount {
                if *bufferSizes.offset(i as isize) > 0i32 as libc::c_uint {
                    ...
                    if *bufferSizes.offset(i as isize) >=
                        ...
                        *fresh1 =
                            (*fresh1 as
                                ...
                                as uint32_t as uint32_t;
                        bytesRemainThisIteration = 0i32 as uint32_t;
                        break ;
                    } else {
                        ...
                        *fresh2 =
                            (*fresh2).offset(*bufferSizes.offset(i as isize)
                                                 as isize);
                        *bufferSizes.offset(i as isize) = 0i32 as uint32_t
                    }
                }
                i += 1
            }
...
unsafe extern "C" fn flashfsGetDirtyDataBuffers(mut buffers:
                                                ...
                                                mut bufferSizes:
                                                    *mut uint32_t) {
    let ref mut fresh3 = *buffers.offset(0);
    *fresh3 =
        flashWriteBuffer.as_mut_ptr().offset(bufferTail as libc::c_int as
                                                 isize);
    let ref mut fresh4 = *buffers.offset(1);
    *fresh4 = flashWriteBuffer.as_mut_ptr().offset(0);
    if bufferHead as libc::c_int >= bufferTail as libc::c_int {
        *bufferSizes.offset(0) =
            (bufferHead as libc::c_int - bufferTail as libc::c_int) as
                uint32_t;
        *bufferSizes.offset(1) = 0i32 as uint32_t
    } else {
        *bufferSizes.offset(0) =
            (128i32 - bufferTail as libc::c_int) as uint32_t;
        *bufferSizes.offset(1) = bufferHead as uint32_t
    };
}
/* *
...
pub unsafe extern "C" fn flashfsGetOffset() -> uint32_t {
    ...
    return tailAddress.wrapping_add(bufferSizes[0]).wrapping_add(bufferSizes[1]);
}
/* *
...
unsafe extern "C" fn flashfsAdvanceTailInBuffer(mut delta: uint32_t) {
    ...
    if bufferTail as libc::c_int >= 128i32 {
        bufferTail = (bufferTail as libc::c_int - 128i32) as uint8_t
    }
...
pub unsafe extern "C" fn flashfsFlushSync() {
    ...
    flashfsGetDirtyDataBuffers(buffers.as_mut_ptr(),
                               bufferSizes.as_mut_ptr());
    flashfsWriteBuffers(buffers.as_mut_ptr(), bufferSizes.as_mut_ptr(), 2i32,
                        1i32 != 0);
...
pub unsafe extern "C" fn flashfsWriteByte(mut byte: uint8_t) {
    ...
    if bufferHead as libc::c_int >= 128i32 { bufferHead = 0i32 as uint8_t }
    if flashfsTransmitBufferUsed() >= 64i32 as libc::c_uint {
        flashfsFlushAsync();
    };
}
/* *
...
pub unsafe extern "C" fn flashfsWrite(mut data: *const uint8_t,
     ...
     * to the flash now
     */
    if bufferSizes[0].wrapping_add(bufferSizes[1]).wrapping_add(bufferSizes[2])
        ...
        bytesWritten =
            flashfsWriteBuffers(buffers.as_mut_ptr(),
                                bufferSizes.as_mut_ptr(), 3i32, 0i32 != 0);
        if bufferSizes[0] == 0i32 as libc::c_uint &&
               bufferSizes[1] == 0i32 as libc::c_uint {
            // We wrote all the data that was previously buffered
            flashfsClearBuffer();
            if bufferSizes[2] == 0i32 as libc::c_uint {
        ...
        if bufferSizes[0].wrapping_add(bufferSizes[1]).wrapping_add(bufferSizes[2])
               > (128i32 - 1i32) as libc::c_uint {
            if sync {
                // Write it through synchronously
                flashfsWriteBuffers(buffers.as_mut_ptr(),
                                    bufferSizes.as_mut_ptr(), 3i32,
                                    1i32 != 0);
                flashfsClearBuffer();
            }
            return
    ...
    let mut bufferBytesBeforeWrap: libc::c_uint =
        (128i32 - bufferHead as libc::c_int) as libc::c_uint;
    ...
    if bufferHead as libc::c_int == 128i32 {
        memcpy(flashWriteBuffer.as_mut_ptr().offset(0) as *mut libc::c_void,
               data as *const libc::c_void, len as libc::c_ulong);
...
pub unsafe extern "C" fn flashfsIdentifyStartOfFreeSpace() -> libc::c_int {
    ...
    let mut left: libc::c_int =
        0i32; // One past the largest block index in the search region
    ...
    while left < right {
        ...
        while i < FREE_BLOCK_TEST_SIZE_INTS as libc::c_int {
            if testBuffer.ints[i as usize] != 0xffffffffu32 {
                blockErased = 0i32 != 0;
                break ;
            } else { i += 1 }
        }
...
pub unsafe extern "C" fn flashfsClose() {
    match (*flashfsGetGeometry()).flashType as libc::c_uint {
        1 => {
            ...
            flashfsSetTailAddress(tailAddress.wrapping_add(pageSize).wrapping_sub(1i32
                                                                                      as
                                                                                      libc::c_uint)
                                      &
                                      !pageSize.wrapping_sub(1i32 as
                                                                 libc::c_uint));
        }
        0 | _ => { }
    };
...
pub unsafe extern "C" fn flashfsInit() {
    ...
    if flashfsGetSize() > 0i32 as libc::c_uint {
