...
pub unsafe extern "C" fn trampCmsUpdateStatusString() {
    trampCmsStatusString[0] = '*' as i32 as libc::c_char;
    trampCmsStatusString[1] = ' ' as i32 as libc::c_char;
    trampCmsStatusString[2] =
        *vtx58BandLetter.as_ptr().offset(trampBand as isize);
    trampCmsStatusString[3] =
        *(*vtx58ChannelNames.as_ptr().offset(trampChannel as
                                                 isize)).offset(0);
    trampCmsStatusString[4] = ' ' as i32 as libc::c_char;
    if trampCurFreq != 0 {
...
#[no_mangle]
pub static mut trampCmsPitMode: uint8_t = 0i32 as uint8_t;
#[no_mangle]
pub static mut trampCmsBand: uint8_t = 1i32 as uint8_t;
#[no_mangle]
pub static mut trampCmsChan: uint8_t = 1i32 as uint8_t;
#[no_mangle]
...
static mut trampCmsPower: uint8_t = 1i32 as uint8_t;
...
unsafe extern "C" fn trampCmsUpdateFreqRef() {
    if trampCmsBand as libc::c_int > 0i32 &&
           trampCmsChan as libc::c_int > 0i32 {
        trampCmsFreqRef =
            vtx58frequencyTable[(trampCmsBand as libc::c_int - 1i32) as
...
unsafe extern "C" fn trampCmsConfigChan(mut pDisp: *mut displayPort_t,
                                        mut self_0: *const libc::c_void)
 -> libc::c_long {
    if trampCmsChan as libc::c_int == 0i32 {
        // Bounce back
        trampCmsChan = 1i32 as uint8_t
    } else { trampCmsUpdateFreqRef(); }
    return 0i32 as libc::c_long;
}
unsafe extern "C" fn trampCmsConfigPower(mut pDisp: *mut displayPort_t,
                                         mut self_0: *const libc::c_void)
 -> libc::c_long {
    if trampCmsPower as libc::c_int == 0i32 {
        // Bounce back
        trampCmsPower = 1i32 as uint8_t
    }
    return 0i32 as libc::c_long;
}
            ...
            let mut init =
                OSD_INT16_t{val:
                                &trampTemperature as *const int16_t as
                                    *mut int16_t,
                            min: -100i32 as int16_t,
                            max: 300i32 as int16_t,
...
unsafe extern "C" fn trampCmsSetPitMode(mut pDisp: *mut displayPort_t,
                                        mut self_0: *const libc::c_void)
 -> libc::c_long {
    if trampCmsPitMode as libc::c_int == 0i32 {
        // Bouce back
        trampCmsPitMode = 1i32 as uint8_t
    } else {
        trampSetPitMode((trampCmsPitMode as libc::c_int - 1i32) as uint8_t);
    }
    return 0i32 as libc::c_long;
}
unsafe extern "C" fn trampCmsCommence(mut pDisp: *mut displayPort_t,
 ...
 -> libc::c_long {
    ...
    trampSetRFPower(trampPowerTable[(trampCmsPower as libc::c_int - 1i32) as
                                        usize]);
...
unsafe extern "C" fn trampCmsInitSettings() {
    if trampBand as libc::c_int > 0i32 { trampCmsBand = trampBand }
    if trampChannel as libc::c_int > 0i32 { trampCmsChan = trampChannel }
    trampCmsUpdateFreqRef();
    trampCmsPitMode = (trampPitMode as libc::c_int + 1i32) as uint8_t;
    if trampConfiguredPower as libc::c_int > 0i32 {
        ...
        while (i as libc::c_int) < 5i32 {
            if trampConfiguredPower as libc::c_int <=
                ...
                trampCmsPower = (i as libc::c_int + 1i32) as uint8_t;
                break ;
