         ...
         let mut init =
             ioPortDef_s{rcc:
                             (((RCC_AHB as libc::c_int) << 5i32) as
                                  libc::c_long |
                                  (16i32 *
                                       (0x20000i32 as uint32_t as libc::c_long
                                            > 65535i64) as libc::c_int) as
                                      libc::c_long +
                                      ((8i32 *
         ...
         let mut init =
             ioPortDef_s{rcc:
                             (((RCC_AHB as libc::c_int) << 5i32) as
                                  libc::c_long |
                                  (16i32 *
                                       (0x40000i32 as uint32_t as libc::c_long
                                            > 65535i64) as libc::c_int) as
                                      libc::c_long +
                                      ((8i32 *
         ...
         let mut init =
             ioPortDef_s{rcc:
                             (((RCC_AHB as libc::c_int) << 5i32) as
                                  libc::c_long |
                                  (16i32 *
                                       (0x80000i32 as uint32_t as libc::c_long
                                            > 65535i64) as libc::c_int) as
                                      libc::c_long +
                                      ((8i32 *
...
pub unsafe extern "C" fn IO_GPIOPortIdx(mut io: IO_t) -> libc::c_int {
    ...
    return ((IO_GPIO(io) as
                 size_t).wrapping_sub((0x40000000i32 as
                                           uint32_t).wrapping_add(0x8000000i32
                                                                      as
                                                                      libc::c_uint).wrapping_add(0i32
                                                                                                     as
                                                                                                     libc::c_uint)
                                          as libc::c_ulong) >> 10i32) as
               libc::c_int;
    ...
    let mut init: GPIO_InitTypeDef =
        {
            let mut init =
                GPIO_InitTypeDef{GPIO_Pin: IO_Pin(io) as uint32_t,
                                 GPIO_Mode:
                                     (cfg as libc::c_int >> 0i32 & 0x3i32) as
                                         GPIOMode_TypeDef,
                                 GPIO_Speed:
                                     (cfg as libc::c_int >> 2i32 & 0x3i32) as
                                         GPIOSpeed_TypeDef,
                                 GPIO_OType:
                                     (cfg as libc::c_int >> 4i32 & 0x1i32) as
                                         GPIOOType_TypeDef,
                                 GPIO_PuPd:
                                     (cfg as libc::c_int >> 5i32 & 0x3i32) as
                                         GPIOPuPd_TypeDef,};
            init
        };
...
static mut ioDefUsedMask: [uint16_t; 6] =
    [0xffffi32 as uint16_t, 0xffffi32 as uint16_t,
     (1i32 << 13i32 | 1i32 << 14i32 | 1i32 << 15i32) as uint16_t,
     0i32 as uint16_t, 0i32 as uint16_t,
     (1i32 << 0i32 | 1i32 << 1i32 | 1i32 << 3i32 | 1i32 << 4i32) as uint16_t];
static mut ioDefUsedOffset: [uint8_t; 6] =
    [0i32 as uint8_t,
     ((0xffffi32 - (0xffffi32 >> 1i32 & 0x77777777i32) -
           ...
           (0xffffi32 - (0xffffi32 >> 1i32 & 0x77777777i32) -
                (0xffffi32 >> 2i32 & 0x33333333i32) -
                (0xffffi32 >> 3i32 & 0x11111111i32) >> 4i32) & 0xf0f0f0fi32) %
          255i32) as uint8_t,
     ((0xffffi32 - (0xffffi32 >> 1i32 & 0x77777777i32) -
           (0xffffi32 >> 2i32 & 0x33333333i32) -
           (0xffffi32 >> 3i32 & 0x11111111i32) +
          ...
          (0xffffi32 - (0xffffi32 >> 1i32 & 0x77777777i32) -
               ...
               0xf0f0f0fi32) % 255i32) as uint8_t,
     ((0xffffi32 - (0xffffi32 >> 1i32 & 0x77777777i32) -
           (0xffffi32 >> 2i32 & 0x33333333i32) -
           (0xffffi32 >> 3i32 & 0x11111111i32) +
           (0xffffi32 - (0xffffi32 >> 1i32 & 0x77777777i32) -
          ...
          ((1i32 << 13i32 | 1i32 << 14i32 | 1i32 << 15i32) -
               ((1i32 << 13i32 | 1i32 << 14i32 | 1i32 << 15i32) >> 1i32 &
                    0x77777777i32) -
               ((1i32 << 13i32 | 1i32 << 14i32 | 1i32 << 15i32) >> 2i32 &
                    0x33333333i32) -
               ((1i32 << 13i32 | 1i32 << 14i32 | 1i32 << 15i32) >> 3i32 &
                    0x11111111i32) +
               ((1i32 << 13i32 | 1i32 << 14i32 | 1i32 << 15i32) -
                    ((1i32 << 13i32 | 1i32 << 14i32 | 1i32 << 15i32) >> 1i32 &
                         0x77777777i32) -
                    ((1i32 << 13i32 | 1i32 << 14i32 | 1i32 << 15i32) >> 2i32 &
                         0x33333333i32) -
                    ((1i32 << 13i32 | 1i32 << 14i32 | 1i32 << 15i32) >> 3i32 &
                         0x11111111i32) >> 4i32) & 0xf0f0f0fi32) % 255i32) as
         uint8_t,
     ((0xffffi32 - (0xffffi32 >> 1i32 & 0x77777777i32) -
           (0xffffi32 >> 2i32 & 0x33333333i32) -
           (0xffffi32 >> 3i32 & 0x11111111i32) +
     ...
     ((0xffffi32 - (0xffffi32 >> 1i32 & 0x77777777i32) -
          ...
          ((1i32 << 13i32 | 1i32 << 14i32 | 1i32 << 15i32) -
               ...
               ((1i32 << 13i32 | 1i32 << 14i32 | 1i32 << 15i32) -
                    ...
                    ((1i32 << 13i32 | 1i32 << 14i32 | 1i32 << 15i32) >> 3i32 &
                         0x11111111i32) >> 4i32) & 0xf0f0f0fi32) % 255i32 +
          0i32 + 0i32) as uint8_t];
#[no_mangle]
pub static mut ioRecs: [ioRec_t; 39] =
...
pub unsafe extern "C" fn IOGetByTag(mut tag: ioTag_t) -> IO_t {
    ...
    if portIdx < 0i32 || portIdx >= 6i32 { return 0 as *mut libc::c_void }
    // check if pin exists
