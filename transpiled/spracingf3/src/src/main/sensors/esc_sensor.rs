...
pub static mut escSensorConfig_Registry: pgRegistry_t =
    unsafe {
        {
            let mut init =
                pgRegistry_s{pgn: (517i32 | 0i32 << 12i32) as pgn_t,
                             size:
...
pub static mut pgResetTemplate_escSensorConfig: escSensorConfig_t =
    {
        let mut init =
            escSensorConfig_s{halfDuplex: 0i32 as uint8_t, offset: 0,};
        init
    };
static mut telemetryBuffer: [uint8_t; 10] =
    [0i32 as uint8_t, 0, 0, 0, 0, 0, 0, 0, 0, 0];
static mut buffer: *mut uint8_t = 0 as *const uint8_t as *mut uint8_t;
static mut bufferSize: uint8_t = 0i32 as uint8_t;
static mut bufferPosition: uint8_t = 0i32 as uint8_t;
...
static mut escSensorMotor: uint8_t = 0i32 as uint8_t;
// motor index
static mut combinedEscSensorData: escSensorData_t =
    escSensorData_t{dataAge: 0,
                    ...
                    consumption: 0,
                    rpm: 0,};
static mut combinedDataNeedsUpdate: bool = 1i32 != 0;
static mut totalTimeoutCount: uint16_t = 0i32 as uint16_t;
static mut totalCrcErrorCount: uint16_t = 0i32 as uint16_t;
#[no_mangle]
...
pub unsafe extern "C" fn getEscSensorData(mut motorNumber: uint8_t)
 -> *mut escSensorData_t {
    ...
    } else if motorNumber as libc::c_int == 255i32 {
        if combinedDataNeedsUpdate {
            combinedEscSensorData.dataAge = 0i32 as uint8_t;
            combinedEscSensorData.temperature = 0i32 as int8_t;
            combinedEscSensorData.voltage = 0i32 as int16_t;
            combinedEscSensorData.current = 0i32;
            combinedEscSensorData.consumption = 0i32;
            combinedEscSensorData.rpm = 0i32 as int16_t;
            ...
            combinedEscSensorData.rpm =
                (combinedEscSensorData.rpm as libc::c_int /
                     getMotorCount() as libc::c_int) as int16_t;
            combinedDataNeedsUpdate = 0i32 != 0;
...
pub unsafe extern "C" fn escSensorInit() -> bool {
    ...
    escSensorPort =
        openSerialPort((*portConfig).identifier, FUNCTION_ESC_SENSOR,
                       ...
                       115200i32 as uint32_t, MODE_RX, options);
    let mut i: libc::c_int = 0i32;
    while i < 8i32 {
        escSensorData[i as usize].dataAge = 255i32 as uint8_t;
        i = i + 1i32
    }
    return !escSensorPort.is_null();
}
unsafe extern "C" fn updateCrc8(mut crc: uint8_t, mut crc_seed: uint8_t)
 -> uint8_t {
    ...
    while i < 8i32 {
        crc_u =
            if crc_u as libc::c_int & 0x80i32 != 0 {
                (0x7i32) ^ (crc_u as libc::c_int) << 1i32
            } else { ((crc_u as libc::c_int)) << 1i32 } as uint8_t;
        i += 1
    }
    return crc_u;
}
...
unsafe extern "C" fn decodeEscFrame() -> uint8_t {
    ...
    let mut chksum: uint16_t =
        calculateCrc8(telemetryBuffer.as_mut_ptr(), (10i32 - 1i32) as uint8_t)
            as uint16_t; // last byte contains CRC value
    let mut tlmsum: uint16_t =
        telemetryBuffer[(10i32 - 1i32) as usize] as uint16_t;
    let mut frameStatus: uint8_t = 0;
    if chksum as libc::c_int == tlmsum as libc::c_int {
        escSensorData[escSensorMotor as usize].dataAge = 0i32 as uint8_t;
        escSensorData[escSensorMotor as usize].temperature =
            telemetryBuffer[0] as int8_t;
        escSensorData[escSensorMotor as usize].voltage =
            ((telemetryBuffer[1] as libc::c_int) << 8i32 |
                 telemetryBuffer[2] as libc::c_int) as int16_t;
        escSensorData[escSensorMotor as usize].current =
            (telemetryBuffer[3] as libc::c_int) << 8i32 |
                telemetryBuffer[4] as libc::c_int;
        escSensorData[escSensorMotor as usize].consumption =
            (telemetryBuffer[5] as libc::c_int) << 8i32 |
                telemetryBuffer[6] as libc::c_int;
        escSensorData[escSensorMotor as usize].rpm =
            ((telemetryBuffer[7] as libc::c_int) << 8i32 |
                 telemetryBuffer[8] as libc::c_int) as int16_t;
        combinedDataNeedsUpdate = 1i32 != 0;
...
unsafe extern "C" fn selectNextMotor() {
    ...
    if escSensorMotor as libc::c_int == getMotorCount() as libc::c_int {
        escSensorMotor = 0i32 as uint8_t
    };
}
#[no_mangle]
pub unsafe extern "C" fn escSensorProcess(mut currentTimeUs: timeUs_t) {
    ...
    match escSensorTriggerState as libc::c_uint {
        ...
        1 => {
            ...
            startEscDataRead(telemetryBuffer.as_mut_ptr(), 10i32 as uint8_t);
            ...
            if debugMode as libc::c_int == DEBUG_ESC_SENSOR as libc::c_int {
                debug[DEBUG_ESC_MOTOR_INDEX as libc::c_int as usize] =
                    (escSensorMotor as libc::c_int + 1i32) as int16_t
            }
        }
        2 => {
            if currentTimeMs <
                   escTriggerTimestamp.wrapping_add(100i32 as libc::c_uint) {
...
pub unsafe extern "C" fn calcEscRpm(mut erpm: libc::c_int) -> libc::c_int {
    return erpm * 100i32 /
               ((*motorConfig()).motorPoleCount as libc::c_int / 2i32);
}
