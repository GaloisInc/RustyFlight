 ...
 */
    ...
    #[no_mangle]
    static mut gpsRescueAngle: [int32_t; 2];
    #[no_mangle]
...
pub static mut pgResetTemplate_pidConfig: pidConfig_t =
    {
        let mut init =
            pidConfig_s{pid_process_denom: 4 as libc::c_int as uint8_t,
...
#[link_section = ".fastram_bss"]
static mut axisError: [libc::c_float; 3] = [0.; 3];
#[link_section = ".fastram_bss"]
static mut acGain: libc::c_float = 0.;
#[link_section = ".fastram_bss"]
static mut acLimit: libc::c_float = 0.;
#[link_section = ".fastram_bss"]
...
pub unsafe extern "C" fn pidInitConfig(mut pidProfile: *const pidProfile_t) {
    ...
    // USE_ACRO_TRAINER
    acGain =
        (*pidProfile).abs_control_gain as
            libc::c_float; // Initialize pid looptime
    acLimit = (*pidProfile).abs_control_limit as libc::c_float;
    acErrorLimit = (*pidProfile).abs_control_error_limit as libc::c_float;
}
#[no_mangle]
 ...
 -> libc::c_float {
    ...
    let mut angle: libc::c_float =
        (*pidProfile).levelAngleLimit as libc::c_int as libc::c_float *
            getRcDeflection(axis); // ANGLE IS IN CENTIDEGREES
    angle +=
        (gpsRescueAngle[axis as usize] / 100 as libc::c_int) as libc::c_float;
    angle =
        constrainf(angle,
...
pub unsafe extern "C" fn pidController(mut pidProfile: *const pidProfile_t,
    ...
    while axis_0 <= FD_YAW as libc::c_int {
        ...
        if itermRelax as libc::c_int != 0 &&
            ...
            if gyroRate >= gminac && gyroRate <= gmaxac {
                ...
                if fabsf(acErrorRate * dT) > fabsf(axisError[axis_0 as usize])
                   {
                    acErrorRate = -axisError[axis_0 as usize] / dT
                }
            } else {
                acErrorRate =
                    (if gyroRate > gmaxac { gmaxac } else { gminac }) -
                        gyroRate
            }
        ...
        if acGain > 0 as libc::c_int as libc::c_float &&
            ...
            acCorrection =
                constrainf(axisError[axis_0 as usize] * acGain, -acLimit,
                           acLimit);
