...
extern "C" {
    ...
    #[no_mangle]
    fn bmi160Detect(bus: *const busDevice_t) -> uint8_t;
    /*
 ...
 */
    ...
    // RF = Register Flag
    #[no_mangle]
    fn mpu6000SpiDetect(bus: *const busDevice_t) -> uint8_t;
    /*
 * This file is part of Cleanflight and Betaflight.
 *
 * Cleanflight and Betaflight are free software. You can redistribute
 ...
 */
    #[no_mangle]
    fn mpu6500SpiDetect(bus: *const busDevice_t) -> uint8_t;
    /*
 * This file is part of Cleanflight and Betaflight.
 *
 * Cleanflight and Betaflight are free software. You can redistribute
...
*/
    ...
    // RF = Register Flag
    #[no_mangle]
    fn mpu9250SpiResetGyro();
    #[no_mangle]
    fn mpu9250SpiDetect(bus: *const busDevice_t) -> uint8_t;
}
...
unsafe extern "C" fn detectSPISensorsAndUpdateDetectionResult(mut gyro:
 ...
 -> bool {
    ...
    spiBusSetInstance(&mut (*gyro).bus,
                      (0x40000000 as libc::c_int as
                           uint32_t).wrapping_add(0x3800 as libc::c_int as
                                                      libc::c_uint) as
                          *mut SPI_TypeDef);
    (*gyro).bus.busdev_u.spi.csnPin =
        if (*gyro).bus.busdev_u.spi.csnPin.is_null() {
            IOGetByTag(((1 as libc::c_int + 1 as libc::c_int) <<
                            4 as libc::c_int | 12 as libc::c_int) as ioTag_t)
        } else { (*gyro).bus.busdev_u.spi.csnPin };
    sensor = mpu6000SpiDetect(&mut (*gyro).bus);
    if sensor as libc::c_int != MPU_NONE as libc::c_int {
        (*gyro).mpuDetectionResult.sensor = sensor as mpuSensor_e;
        return 1 as libc::c_int != 0
    }
    spiBusSetInstance(&mut (*gyro).bus,
                      (0x40000000 as libc::c_int as
    ...
    (*gyro).bus.busdev_u.spi.csnPin =
        if (*gyro).bus.busdev_u.spi.csnPin.is_null() {
            IOGetByTag(((2 as libc::c_int + 1 as libc::c_int) <<
                            4 as libc::c_int | 14 as libc::c_int) as ioTag_t)
        } else { (*gyro).bus.busdev_u.spi.csnPin };
    sensor = mpu6500SpiDetect(&mut (*gyro).bus);
    ...
    sensor = mpu9250SpiDetect(&mut (*gyro).bus);
    if sensor as libc::c_int != MPU_NONE as libc::c_int {
        (*gyro).mpuDetectionResult.sensor = sensor as mpuSensor_e;
        (*gyro).mpuConfiguration.resetFn =
            Some(mpu9250SpiResetGyro as unsafe extern "C" fn() -> ());
        return 1 as libc::c_int != 0
    }
    spiBusSetInstance(&mut (*gyro).bus,
                      (0x40000000 as libc::c_int as
    ...
    sensor = bmi160Detect(&mut (*gyro).bus);
    if sensor as libc::c_int != MPU_NONE as libc::c_int {
        (*gyro).mpuDetectionResult.sensor = sensor as mpuSensor_e;
        return 1 as libc::c_int != 0
    }
