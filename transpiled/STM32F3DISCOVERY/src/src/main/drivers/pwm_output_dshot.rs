...
static mut dmaMotorTimers: [motorDmaTimer_t; 8] =
    [motorDmaTimer_t{timer: 0 as *const TIM_TypeDef as *mut TIM_TypeDef,
                     ...
                     dmaBurstBuffer: [0; 72],
                     timerDmaSources: 0,}; 8];
static mut dmaMotors: [motorDmaOutput_t; 12] =
    ...
    while i < dmaMotorTimerCount as libc::c_int {
        if useBurstDshot {
            ...
            TIM_DMAConfig(dmaMotorTimers[i as usize].timer,
                          ...
                          0x300 as libc::c_int as uint16_t);
            TIM_DMACmd(dmaMotorTimers[i as usize].timer,
                       0x100 as libc::c_int as uint16_t, ENABLE);
        } else {
...
unsafe extern "C" fn motor_DMA_IRQHandler(mut descriptor:
    ...
    if (*(*descriptor).dma).ISR &
        ...
        if useBurstDshot {
            ...
            TIM_DMACmd((*(*motor).timerHardware).tim,
                       0x100 as libc::c_int as uint16_t, DISABLE);
        } else {
...
pub unsafe extern "C" fn pwmDshotMotorHardwareConfig(mut timerHardware:
    ...
    TIM_OCInitStructure.TIM_Pulse = 0 as libc::c_int as uint32_t;
    timerOCInit(timer, (*timerHardware).channel, &mut TIM_OCInitStructure);
    timerOCPreloadConfig(timer, (*timerHardware).channel,
    ...
    if useBurstDshot {
        ...
        DMA_InitStructure.DMA_BufferSize =
            if pwmProtocolType as libc::c_uint ==
                   PWM_TYPE_PROSHOT1000 as libc::c_int as libc::c_uint {
                6 as libc::c_int
            } else { 18 as libc::c_int } as uint16_t;
    ...
    DMA_ITConfig(dmaRef, 0x2 as libc::c_int as uint32_t, ENABLE);
    (*motor).configured = 1 as libc::c_int != 0;
}
