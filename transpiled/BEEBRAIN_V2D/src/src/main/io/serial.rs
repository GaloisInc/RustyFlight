...
pub unsafe extern "C" fn pgResetFn_serialConfig(mut serialConfig_0:
    ...
    if (*serialConfig_0).portConfigs[0 as libc::c_int as usize].identifier as
           libc::c_int == SERIAL_PORT_USB_VCP as libc::c_int {
        let mut uart1Config: *mut serialPortConfig_t =
            serialFindPortConfiguration(SERIAL_PORT_USART1);
        if !uart1Config.is_null() {
            (*uart1Config).functionMask =
                FUNCTION_MSP as libc::c_int as uint16_t
        }
    }
