...
unsafe extern "C" fn ReadBit() -> int8_t {
    ...
    loop  {
        ...
        if micros() > timeout_timer {
            current_block = 14196964389350369303;
            break ;
        }
    }
    match current_block {
    ...
    *bt = 0 as libc::c_int as uint8_t;
    ...
    loop  {
        ...
        if bit as libc::c_int == -(1 as libc::c_int) {
            current_block = 17255006639721825009;
            break ;
        }
        if bit as libc::c_int == 1 as libc::c_int {
            *bt =
    ...
    match current_block {
        17255006639721825009 => { return 0 as libc::c_int as uint8_t }
        _ => {
            ckSumIn =
    ...
    if !(waitcycl == 0 as libc::c_int as libc::c_uint) {
        ...
        loop  {
            ...
            if ReadBit() as libc::c_int == -(1 as libc::c_int) {
                current_block = 9288361705615549601;
                break ;
            }
            i = i.wrapping_add(1)
        }
        match current_block {
            ...
            _ => {
                ...
                loop  {
                    ...
                    if bit as libc::c_int == -(1 as libc::c_int) {
                        current_block = 9288361705615549601; // NumTX
                        break ; // NumRX
                    } // RxStartAdr
