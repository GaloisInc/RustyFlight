...
extern "C" {
    ...
    #[no_mangle]
    static mut pidProfiles_SystemArray: [pidProfile_t; 2];
    #[no_mangle]
...
pub static mut pgResetTemplate_systemConfig: systemConfig_t =
    {
        let mut init =
            systemConfig_s{pidProfileIndex: 0 as libc::c_int as uint8_t,
                           ...
                           boardIdentifier: [67, 74, 77, 49, 0, 0],};
        init
    };
#[no_mangle]
...
unsafe extern "C" fn validateAndFixConfig() {
    ...
    loadControlRateProfile();
    if (*systemConfig()).pidProfileIndex as libc::c_int >= 2 as libc::c_int {
    ...
    if !rcSmoothingIsEnabled() ||
        ...
        while i_0 < 2 as libc::c_int as libc::c_uint {
...
pub unsafe extern "C" fn changePidProfile(mut pidProfileIndex: uint8_t) {
    if (pidProfileIndex as libc::c_int) < 2 as libc::c_int {
