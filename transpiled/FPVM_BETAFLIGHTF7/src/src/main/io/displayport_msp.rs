...
unsafe extern "C" fn heartbeat(mut displayPort: *mut displayPort_t)
 -> libc::c_int {
    ...
    return output(displayPort, 182 as libc::c_int as uint8_t,
                  subcmd.as_mut_ptr(),
...
unsafe extern "C" fn release(mut displayPort: *mut displayPort_t)
 -> libc::c_int {
    let mut subcmd: [uint8_t; 1] = [1 as libc::c_int as uint8_t];
    return output(displayPort, 182 as libc::c_int as uint8_t,
                  subcmd.as_mut_ptr(),
                  ::core::mem::size_of::<[uint8_t; 1]>() as libc::c_ulong as
                      libc::c_int);
...
unsafe extern "C" fn clearScreen(mut displayPort: *mut displayPort_t)
 -> libc::c_int {
    let mut subcmd: [uint8_t; 1] = [2 as libc::c_int as uint8_t];
    return output(displayPort, 182 as libc::c_int as uint8_t,
                  subcmd.as_mut_ptr(),
                  ::core::mem::size_of::<[uint8_t; 1]>() as libc::c_ulong as
                      libc::c_int);
...
unsafe extern "C" fn drawScreen(mut displayPort: *mut displayPort_t)
 -> libc::c_int {
    let mut subcmd: [uint8_t; 1] = [4 as libc::c_int as uint8_t];
    return output(displayPort, 182 as libc::c_int as uint8_t,
                  subcmd.as_mut_ptr(),
                  ::core::mem::size_of::<[uint8_t; 1]>() as libc::c_ulong as
                      libc::c_int);
...
unsafe extern "C" fn writeString(mut displayPort: *mut displayPort_t,
 ...
 -> libc::c_int {
    ...
    if len >= 30 as libc::c_int { len = 30 as libc::c_int }
    buf[0 as libc::c_int as usize] = 3 as libc::c_int as uint8_t;
    buf[1 as libc::c_int as usize] = row;
    buf[2 as libc::c_int as usize] = col;
    buf[3 as libc::c_int as usize] = 0 as libc::c_int as uint8_t;
    memcpy(&mut *buf.as_mut_ptr().offset(4 as libc::c_int as isize) as
               *mut uint8_t as *mut libc::c_void,
           string as *const libc::c_void, len as libc::c_ulong);
    return output(displayPort, 182 as libc::c_int as uint8_t,
                  buf.as_mut_ptr(), len + 4 as libc::c_int);
}
...
unsafe extern "C" fn isTransferInProgress(mut displayPort:
    ...
    return 0 as libc::c_int != 0; // XXX Will reflect NTSC/PAL in the future
}
...
unsafe extern "C" fn resync(mut displayPort: *mut displayPort_t) {
    (*displayPort).rows =
        (13 as libc::c_int +
             (*displayPortProfileMsp()).rowAdjust as libc::c_int) as uint8_t;
    (*displayPort).cols =
        (30 as libc::c_int +
             (*displayPortProfileMsp()).colAdjust as libc::c_int) as uint8_t;
