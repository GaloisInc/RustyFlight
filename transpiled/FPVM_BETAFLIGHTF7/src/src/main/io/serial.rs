 ...
 * If not, see <http://www.gnu.org/licenses/>.
 */
static mut serialPortUsageList: [serialPortUsage_t; 7] =
    [serialPortUsage_t{serialPort:
                           0 as *const serialPort_t as *mut serialPort_t,
                       function: FUNCTION_NONE,
                       identifier: SERIAL_PORT_USART1,}; 7];
#[no_mangle]
pub static mut serialPortIdentifiers: [serialPortIdentifier_e; 7] =
    [SERIAL_PORT_USB_VCP, SERIAL_PORT_USART1, SERIAL_PORT_USART2,
     SERIAL_PORT_USART3, SERIAL_PORT_USART6, SERIAL_PORT_SOFTSERIAL1,
...
pub unsafe extern "C" fn findSerialPortUsageByIdentifier(mut identifier:
 ...
 -> *mut serialPortUsage_t {
    let mut index: uint8_t = 0;
    index = 0 as libc::c_int as uint8_t;
    while (index as libc::c_int) < 7 as libc::c_int {
...
pub unsafe extern "C" fn serialInit(mut softserialEnabled: bool,
                                    mut serialPortToDisable:
                                        serialPortIdentifier_e) {
    serialPortCount = 7 as libc::c_int as uint8_t;
    memset(&mut serialPortUsageList as *mut [serialPortUsage_t; 7] as
               *mut libc::c_void, 0 as libc::c_int,
           ::core::mem::size_of::<[serialPortUsage_t; 7]>() as libc::c_ulong);
    let mut index: libc::c_int = 0 as libc::c_int;
    while index < 7 as libc::c_int {
