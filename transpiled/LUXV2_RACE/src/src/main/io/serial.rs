...
extern "C" {
    ...
    #[no_mangle]
    fn usbVcpOpen() -> *mut serialPort_t;
    #[no_mangle]
...
pub static mut serialPortIdentifiers: [serialPortIdentifier_e; 8] =
    [SERIAL_PORT_USB_VCP, SERIAL_PORT_USART1, SERIAL_PORT_USART2,
     SERIAL_PORT_USART3, SERIAL_PORT_UART4, SERIAL_PORT_UART5,
...
pub unsafe extern "C" fn pgResetFn_serialConfig(mut serialConfig_0:
    ...
    let mut serialTlemetryUartConfig: *mut serialPortConfig_t =
        serialFindPortConfiguration(SERIAL_PORT_USART1);
    if !serialTlemetryUartConfig.is_null() {
        (*serialTlemetryUartConfig).functionMask =
            FUNCTION_TELEMETRY_SMARTPORT as libc::c_int as uint16_t
    }
...
pub unsafe extern "C" fn findSerialPortUsageByIdentifier(mut identifier:
 ...
 -> *mut serialPortUsage_t {
    let mut index: uint8_t = 0;
    index = 0 as libc::c_int as uint8_t;
    while (index as libc::c_int) < 8 as libc::c_int {
...
pub unsafe extern "C" fn serialInit(mut softserialEnabled: bool,
                                    mut serialPortToDisable:
                                        serialPortIdentifier_e) {
    serialPortCount = 8 as libc::c_int as uint8_t;
            ...
            if !((*serialPinConfig()).ioTagTx[resourceIndex as usize] as
                ...
                serialPortCount = serialPortCount.wrapping_sub(1)
                      ...
                      !(softserialEnabled as libc::c_int != 0 &&
                            ((*serialPinConfig()).ioTagTx[(10 as libc::c_int +
                                                               SOFTSERIAL1 as
                                                                   libc::c_int)
                                                              as usize] as
                                 libc::c_int != 0 ||
                                 (*serialPinConfig()).ioTagRx[(10 as
                                                                   libc::c_int
                                                                   +
                                                                   SOFTSERIAL1
                                                                       as
