...
pub unsafe extern "C" fn arm_max_f32(mut pSrc: *mut float32_t,
    ...
    outIndex = 0 as libc::c_uint;
    /* Load first input value that act as reference value for comparision */
    let fresh0 = pSrc;
    pSrc = pSrc.offset(1);
    out = *fresh0;
    /* Loop unrolling */
    blkCnt = blockSize.wrapping_sub(1 as libc::c_uint) >> 2 as libc::c_uint;
    while blkCnt > 0 as libc::c_uint {
        ...
        if out < maxVal1 {
            ...
            out = maxVal1;
            outIndex = count.wrapping_add(1 as libc::c_uint)
        }
        /* compare for the maximum value */
        if out < maxVal2 {
            /* Update the maximum value and its index */
            out = maxVal2;
            outIndex = count.wrapping_add(2 as libc::c_uint)
        }
        /* Initialize maxVal to the next consecutive values one by one */
        let fresh3 = pSrc;
        pSrc = pSrc.offset(1);
        ...
        if out < maxVal1 {
            /* Update the maximum value and its index */
            out = maxVal1;
            outIndex = count.wrapping_add(3 as libc::c_uint)
        }
        /* compare for the maximum value */
        if out < maxVal2 {
            /* Update the maximum value and its index */
            out = maxVal2;
            outIndex = count.wrapping_add(4 as libc::c_uint)
        }
        count =
            (count as libc::c_uint).wrapping_add(4 as libc::c_uint) as
