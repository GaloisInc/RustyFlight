...
pub unsafe extern "C" fn mavlinkSendPosition() {
    ...
    mavlink_msg_global_position_int_pack(0 as libc::c_int as uint8_t,
                                         ...
                                         gpsSol.llh.lat, gpsSol.llh.lon,
                                         gpsSol.llh.alt * 1000 as libc::c_int,
                                         if sensors(SENSOR_RANGEFINDER as
                                                        libc::c_int as
                                                        uint32_t) as
                                                libc::c_int != 0 ||
                                                sensors(SENSOR_BARO as
                                                            libc::c_int as
                                                            uint32_t) as
                                                    libc::c_int != 0 {
...
pub unsafe extern "C" fn mavlinkSendHUDAndHeartbeat() {
    ...
    if sensors(SENSOR_RANGEFINDER as libc::c_int as uint32_t) as libc::c_int
           != 0 ||
           sensors(SENSOR_BARO as libc::c_int as uint32_t) as libc::c_int != 0
       {
        // Baro or sonar generally is a better estimate of altitude than GPS MSL altitude
        mavAltitude =
            (getEstimatedAltitude() as libc::c_double / 100.0f64) as
                libc::c_float
    } else if sensors(SENSOR_GPS as libc::c_int as uint32_t) {
