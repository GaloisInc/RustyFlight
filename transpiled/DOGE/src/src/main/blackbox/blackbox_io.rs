 ...
 */
    ...
    #[no_mangle]
    fn flashfsGetWriteBufferFreeSpace() -> uint32_t;
    #[no_mangle]
    fn flashfsGetWriteBufferSize() -> uint32_t;
    #[no_mangle]
    fn flashfsWriteByte(byte: uint8_t);
    #[no_mangle]
    fn flashfsWrite(data: *const uint8_t, len: libc::c_uint, sync: bool);
    #[no_mangle]
    fn flashfsFlushAsync() -> bool;
    #[no_mangle]
    fn flashfsClose();
    #[no_mangle]
...
pub unsafe extern "C" fn blackboxOpen() {
    let mut sharedBlackboxAndMspPort: *mut serialPort_t =
        findSharedSerialPort(FUNCTION_BLACKBOX as libc::c_int as uint16_t,
                             FUNCTION_MSP); // Write byte asynchronously
...
pub unsafe extern "C" fn blackboxWriteString(mut s: *const libc::c_char)
 -> libc::c_int {
    let mut length: libc::c_int = 0; // Write asynchronously
    let mut pos: *const uint8_t = 0 as *const uint8_t;
    match (*blackboxConfig()).device as libc::c_int {
        1 => {
            length = strlen(s) as libc::c_int;
            flashfsWrite(s as *const uint8_t, length as libc::c_uint,
                         0 as libc::c_int != 0);
        }
        3 | _ => {
            // USE_FLASHFS
    ...
    match (*blackboxConfig()).device as libc::c_int {
        ...
        1 => {
            if !flashfsIsSupported() ||
                   isBlackboxDeviceFull() as libc::c_int != 0 {
                return 0 as libc::c_int != 0
            }
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn blackboxEraseAll() {
    match (*blackboxConfig()).device as libc::c_int {
        1 => { flashfsEraseCompletely(); }
        _ => { }
    };
}
/* *
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn isBlackboxErased() -> bool {
    match (*blackboxConfig()).device as libc::c_int {
        1 => { return flashfsIsReady() }
        _ => {
            //not supported
            return 1 as libc::c_int != 0
        }
...
pub unsafe extern "C" fn blackboxReplenishHeaderBudget() {
    ...
    match (*blackboxConfig()).device as libc::c_int {
        ...
        1 => { freeSpace = flashfsGetWriteBufferFreeSpace() as int32_t }
...
pub unsafe extern "C" fn blackboxDeviceReserveBufferSpace(mut bytes: int32_t)
 -> blackboxBufferReserveStatus_e {
    ...
    match (*blackboxConfig()).device as libc::c_int {
        ...
        1 => {
            if bytes > flashfsGetWriteBufferSize() as int32_t {
                return BLACKBOX_RESERVE_PERMANENT_FAILURE
            }
            if bytes > flashfsGetWriteBufferFreeSpace() as int32_t {
                /*
