...
pub unsafe extern "C" fn transponderIrHardwareInit(mut ioTag: ioTag_t,
    ...
    IOConfigGPIOAF(transponderIO,
                   (GPIO_Mode_AF as libc::c_int |
                        ...
                        (GPIO_PuPd_DOWN as libc::c_int) << 5 as libc::c_int)
                       as ioConfig_t, (*timerHardware_0).alternateFunction);
    ...
    if (*timerHardware_0).output as libc::c_int &
        ...
        TIM_OCInitStructure.TIM_OCIdleState = 0x100 as libc::c_int as uint16_t
    }
    ...
    timerOCInit(timer, (*timerHardware_0).channel, &mut TIM_OCInitStructure);
    timerOCPreloadConfig(timer, (*timerHardware_0).channel,
                         0x8 as libc::c_int as uint16_t);
    TIM_CtrlPWMOutputs(timer, ENABLE);
    /* configure DMA */
    dmaRef =
        (*timerHardware_0).dmaRef; // load number of bytes to be transferred
    DMA_Cmd(dmaRef, DISABLE);
    DMA_DeInit(dmaRef);
    DMA_StructInit(&mut DMA_InitStructure);
    DMA_InitStructure.DMA_PeripheralBaseAddr =
        timerCCR(timer, (*timerHardware_0).channel) as uint32_t;
    DMA_InitStructure.DMA_MemoryBaseAddr =
        &mut (*transponder_0).transponderIrDMABuffer as
            *mut transponderIrDMABuffer_t as uint32_t;
    DMA_InitStructure.DMA_DIR = 0x10 as libc::c_int as uint32_t;
    DMA_InitStructure.DMA_M2M = 0 as libc::c_int as uint32_t;
    DMA_InitStructure.DMA_BufferSize =
        (*transponder_0).dma_buffer_size as uint16_t;
    ...
    TIM_DMACmd(timer, timerDmaSource((*timerHardware_0).channel), ENABLE);
...
pub unsafe extern "C" fn transponderIrDMAEnable(mut transponder_0:
                                                    *mut transponder_t) {
    DMA_SetCurrDataCounter(dmaRef,
                           (*transponder_0).dma_buffer_size as uint16_t);
    TIM_SetCounter(timer, 0 as libc::c_int as uint32_t);
