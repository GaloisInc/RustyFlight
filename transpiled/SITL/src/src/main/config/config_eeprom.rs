...
extern "C" {
    #[no_mangle]
    static mut eepromData: [uint8_t; 32768];
    #[no_mangle]
...
pub unsafe extern "C" fn isEEPROMVersionValid() -> bool {
    let mut p: *const uint8_t = eepromData.as_mut_ptr();
    let mut header: *const configHeader_t = p as *const configHeader_t;
    if (*header).eepromConfigVersion as libc::c_int != 171 as libc::c_int {
...
pub unsafe extern "C" fn isEEPROMStructureValid() -> bool {
    let mut p: *const uint8_t = eepromData.as_mut_ptr();
    let mut header: *const configHeader_t = p as *const configHeader_t;
    if (*header).magic_be as libc::c_int != 0xbe as libc::c_int {
        return 0 as libc::c_int != 0
    }
    let mut crc: uint16_t = 0xffff as libc::c_int as uint16_t;
    crc =
    ...
    loop  {
        ...
        if (*record).size as libc::c_int == 0 as libc::c_int { break ; }
...
unsafe extern "C" fn findEEPROM(mut reg: *const pgRegistry_t,
 ...
 -> *const configRecord_t {
    let mut p: *const uint8_t = eepromData.as_mut_ptr(); // skip header
    p =
        p.offset(::core::mem::size_of::<configHeader_t>() as libc::c_ulong as
                     isize);
    loop  {
        let mut record: *const configRecord_t = p as *const configRecord_t;
        if (*record).size as libc::c_int == 0 as libc::c_int ||
        ...
        if pgN(reg) as libc::c_int == (*record).pgn as libc::c_int &&
               ((*record).flags as libc::c_int & 0x3 as libc::c_int) as
                   libc::c_uint == classification as libc::c_uint {
            return record
        }
        p = p.offset((*record).size as libc::c_int as isize)
    }
    ...
    while reg < __pg_registry_end.as_ptr() {
        ...
        if !rec.is_null() {
            ...
            if !pgLoad(reg, (*rec).pg.as_ptr() as *const libc::c_void,
                       ((*rec).size as
                            libc::c_ulong).wrapping_sub(6 as libc::c_ulong) as
                           libc::c_int, (*rec).version as libc::c_int) {
                success = 0 as libc::c_int != 0
            }
        } else { pgReset(reg); success = 0 as libc::c_int != 0 }
        reg = reg.offset(1)
    }
    return success;
}
unsafe extern "C" fn writeSettingsToEEPROM() -> bool {
    ...
    config_streamer_start(&mut streamer, eepromData.as_mut_ptr() as uintptr_t,
                          (&mut *eepromData.as_mut_ptr().offset((::core::mem::size_of::<[uint8_t; 32768]>()
                                                                     as
                                                                     libc::c_ulong).wrapping_div(::core::mem::size_of::<uint8_t>()
                                                                                                     as
                                                                    ...
                                                                    as isize)
                               as
                               *mut uint8_t).wrapping_offset_from(eepromData.as_mut_ptr())
                              as libc::c_long as libc::c_int);
    let mut header: configHeader_t =
        {
            let mut init =
                configHeader_t{eepromConfigVersion:
                                   171 as libc::c_int as uint8_t,
                               magic_be: 0xbe as libc::c_int as uint8_t,};
            init
        };
    ...
    let mut footer: configFooter_t =
        {
            let mut init =
                configFooter_t{terminator: 0 as libc::c_int as uint16_t,};
            init
        };
    config_streamer_write(&mut streamer,
    ...
    let invertedBigEndianCrc: uint16_t =
        !((crc as libc::c_int & 0xff as libc::c_int) << 8 as libc::c_int |
    ...
    let success: bool =
        config_streamer_finish(&mut streamer) == 0 as libc::c_int;
    return success;
}
#[no_mangle]
pub unsafe extern "C" fn writeConfigToEEPROM() {
    ...
    // write it
    let mut attempt: libc::c_int = 0 as libc::c_int;
    while attempt < 3 as libc::c_int && !success {
        if writeSettingsToEEPROM() { success = 1 as libc::c_int != 0 }
        attempt += 1
    }
