...
pub unsafe extern "C" fn udpInit(mut link: *mut udpLink_t,
 ...
 -> libc::c_int {
    let mut one: libc::c_int = 1 as libc::c_int; // can multi-bind
    (*link).fd =
        socket(2 as libc::c_int, SOCK_DGRAM as libc::c_int,
               IPPROTO_UDP as libc::c_int); // nonblock
    if (*link).fd == -(1 as libc::c_int) { return -(2 as libc::c_int) }
    setsockopt((*link).fd, 1 as libc::c_int, 2 as libc::c_int,
               &mut one as *mut libc::c_int as *const libc::c_void,
               ::core::mem::size_of::<libc::c_int>() as libc::c_ulong as
                   socklen_t);
    fcntl((*link).fd, 4 as libc::c_int,
          fcntl((*link).fd, 3 as libc::c_int, 0 as libc::c_int) |
              0o4000 as libc::c_int);
    (*link).isServer = isServer;
    memset(&mut (*link).si as *mut sockaddr_in as *mut libc::c_void,
           0 as libc::c_int,
           ::core::mem::size_of::<sockaddr_in>() as libc::c_ulong);
    (*link).si.sin_family = 2 as libc::c_int as sa_family_t;
    (*link).si.sin_port = __bswap_16(port as __uint16_t);
    (*link).port = port;
    if addr.is_null() {
        (*link).si.sin_addr.s_addr = __bswap_32(0 as libc::c_int as in_addr_t)
...
pub unsafe extern "C" fn udpSend(mut link: *mut udpLink_t,
    ...
    return sendto((*link).fd, data, size, 0 as libc::c_int,
                  __CONST_SOCKADDR_ARG{__sockaddr__:
                                           &mut (*link).si as *mut sockaddr_in
                                               as *mut sockaddr,},
                  ::core::mem::size_of::<sockaddr_in>() as libc::c_ulong as
 ...
 -> libc::c_int {
    ...
    tv.tv_sec =
        timeout_ms.wrapping_div(1000 as libc::c_int as libc::c_uint) as
            __time_t;
    tv.tv_usec =
        (timeout_ms.wrapping_rem(1000 as libc::c_int as libc::c_uint) as
             libc::c_ulong).wrapping_mul(1000 as libc::c_ulong) as
            __suseconds_t;
    if select((*link).fd + 1 as libc::c_int, &mut fds, 0 as *mut fd_set,
              0 as *mut fd_set, &mut tv) != 1 as libc::c_int {
        return -(1 as libc::c_int)
    }
    let mut len: socklen_t = 0;
    let mut ret: libc::c_int = 0;
    ret =
        recvfrom((*link).fd, data, size, 0 as libc::c_int,
