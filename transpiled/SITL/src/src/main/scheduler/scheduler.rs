...
extern "C" {
    ...
    fn memset(_: *mut libc::c_void, _: libc::c_int, _: libc::c_ulong)
     -> *mut libc::c_void;
    #[no_mangle]
    static mut cfTasks: [cfTask_t; 18];
    #[no_mangle]
    fn micros() -> timeUs_t;
}
...
#[no_mangle]
pub static mut averageSystemLoadPercent: uint16_t = 0i32 as uint16_t;
static mut taskQueuePos: libc::c_int = 0i32;
static mut taskQueueSize: libc::c_int = 0i32;
// No need for a linked list for the queue, since items are only inserted at startup
static mut taskQueueArray: [*mut cfTask_t; 19] =
    [0 as *const cfTask_t as *mut cfTask_t; 19];
...
pub unsafe extern "C" fn queueClear() {
    memset(taskQueueArray.as_mut_ptr() as *mut libc::c_void, 0i32,
           ::core::mem::size_of::<[*mut cfTask_t; 19]>() as libc::c_ulong);
    taskQueuePos = 0i32;
    taskQueueSize = 0i32;
}
#[no_mangle]
pub unsafe extern "C" fn queueContains(mut task: *mut cfTask_t) -> bool {
    ...
    while ii < taskQueueSize {
        if taskQueueArray[ii as usize] == task { return 1i32 != 0 }
        ii += 1
    }
    return 0i32 != 0;
}
...
pub unsafe extern "C" fn queueAdd(mut task: *mut cfTask_t) -> bool {
    ...
    while ii <= taskQueueSize {
        if taskQueueArray[ii as usize].is_null() ||
            ...
            memmove(&mut *taskQueueArray.as_mut_ptr().offset((ii + 1i32) as
                                                                 isize) as
    ...
    while ii < taskQueueSize {
        if taskQueueArray[ii as usize] == task {
            memmove(&mut *taskQueueArray.as_mut_ptr().offset(ii as isize) as
                        *mut *mut cfTask_t as *mut libc::c_void,
                    &mut *taskQueueArray.as_mut_ptr().offset((ii + 1i32) as
                                                                 isize) as
                        *mut *mut cfTask_t as *const libc::c_void,
                    (::core::mem::size_of::<*mut cfTask_t>() as
                         libc::c_ulong).wrapping_mul((taskQueueSize - ii) as
...
pub unsafe extern "C" fn taskSystemLoad(mut currentTimeUs: timeUs_t) {
    ...
    if totalWaitingTasksSamples > 0i32 as libc::c_uint {
        averageSystemLoadPercent =
            (100i32 as
                 libc::c_uint).wrapping_mul(totalWaitingTasks).wrapping_div(totalWaitingTasksSamples)
                as uint16_t;
        totalWaitingTasksSamples = 0i32 as uint32_t;
        totalWaitingTasks = 0i32 as uint32_t
    }
    averageSystemLoadPercent = 0i32 as uint16_t;
}
#[no_mangle]
...
pub unsafe extern "C" fn getCheckFuncInfo(mut checkFuncInfo:
    ...
    (*checkFuncInfo).averageExecutionTime =
        checkFuncMovingSumExecutionTime.wrapping_div(32i32 as libc::c_uint);
}
#[no_mangle]
pub unsafe extern "C" fn getTaskInfo(mut taskId: cfTaskId_e,
    ...
    (*taskInfo).averageExecutionTime =
        cfTasks[taskId as
                    usize].movingSumExecutionTime.wrapping_div(32i32 as
                                                                   libc::c_uint);
...
pub unsafe extern "C" fn schedulerResetTaskStatistics(mut taskId:
                                                          cfTaskId_e) {
    if taskId as libc::c_uint == TASK_SELF as libc::c_int as libc::c_uint {
        (*currentTask).movingSumExecutionTime = 0i32 as timeUs_t;
        (*currentTask).totalExecutionTime = 0i32 as timeUs_t;
        (*currentTask).maxExecutionTime = 0i32 as timeUs_t
    } else if (taskId as libc::c_uint) <
                  TASK_COUNT as libc::c_int as libc::c_uint {
        cfTasks[taskId as usize].movingSumExecutionTime = 0i32 as timeUs_t;
        cfTasks[taskId as usize].totalExecutionTime = 0i32 as timeUs_t;
        cfTasks[taskId as usize].maxExecutionTime = 0i32 as timeUs_t
    };
}
#[no_mangle]
...
pub unsafe extern "C" fn schedulerInit() {
    calculateTaskStatistics = 1i32 != 0;
    queueClear();
...
pub unsafe extern "C" fn scheduler() {
    ...
    let mut outsideRealtimeGuardInterval: bool = 1i32 != 0;
    ...
    while !task.is_null() &&
        ...
        let nextExecuteAt: timeUs_t =
            (*task).lastExecutedAt.wrapping_add((*task).desiredPeriod as
                                                    libc::c_uint);
        if currentTimeUs.wrapping_sub(nextExecuteAt) as timeDelta_t >= 0i32 {
            outsideRealtimeGuardInterval = 0i32 != 0;
            break ;
        } else { task = queueNext() }
    }
    // The task to be invoked
    let mut selectedTask: *mut cfTask_t = 0 as *mut cfTask_t;
    let mut selectedTaskDynamicPriority: uint16_t = 0i32 as uint16_t;
    // Update task dynamic priorities
    let mut waitingTasks: uint16_t = 0i32 as uint16_t;
    ...
    while !task_0.is_null() {
        ...
        if (*task_0).checkFunc.is_some() {
            ...
            if (*task_0).dynamicPriority as libc::c_int > 0i32 {
                                                                                  ...
                                                                                  timeDelta_t)
             {
                if calculateTaskStatistics {
                    ...
                    checkFuncMovingSumExecutionTime =
                        (checkFuncMovingSumExecutionTime as
                             libc::c_uint).wrapping_add(checkFuncExecutionTime.wrapping_sub(checkFuncMovingSumExecutionTime.wrapping_div(32i32
                                                                                                                                             as
                                                                                                                                             libc::c_uint)))
                ...
                (*task_0).taskAgeCycles = 1i32 as uint16_t;
                (*task_0).dynamicPriority =
                    (1i32 + (*task_0).staticPriority as libc::c_int) as
                        uint16_t;
                waitingTasks = waitingTasks.wrapping_add(1)
            } else { (*task_0).taskAgeCycles = 0i32 as uint16_t }
        } else {
            ...
            (*task_0).taskAgeCycles =
                currentTimeUs.wrapping_sub((*task_0).lastExecutedAt).wrapping_div((*task_0).desiredPeriod
                                                                                      ...
                                                                                      libc::c_uint)
                    as uint16_t;
            if (*task_0).taskAgeCycles as libc::c_int > 0i32 {
                (*task_0).dynamicPriority =
                    (1i32 +
                         (*task_0).staticPriority as libc::c_int *
                             (*task_0).taskAgeCycles as libc::c_int) as
        ...
        if (*task_0).dynamicPriority as libc::c_int >
            ...
            let taskCanBeChosenForScheduling: bool =
                outsideRealtimeGuardInterval as libc::c_int != 0 ||
                    (*task_0).taskAgeCycles as libc::c_int > 1i32 ||
                    (*task_0).staticPriority as libc::c_int ==
                        TASK_PRIORITY_REALTIME as libc::c_int;
    ...
    if !selectedTask.is_null() {
        ...
        (*selectedTask).dynamicPriority = 0i32 as uint16_t;
        // Execute task
        if calculateTaskStatistics {
            ...
            (*selectedTask).movingSumExecutionTime =
                ((*selectedTask).movingSumExecutionTime as
                     libc::c_uint).wrapping_add(taskExecutionTime.wrapping_sub((*selectedTask).movingSumExecutionTime.wrapping_div(32i32
                                                                                                                                       as
                                                                                                                                       libc::c_uint)))
                    as timeUs_t as timeUs_t;
