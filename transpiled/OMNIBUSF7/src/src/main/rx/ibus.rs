...
static mut ibusFrameDone: bool = 0 as libc::c_int != 0;
...
unsafe extern "C" fn isValidIa6bIbusPacketLength(mut length: uint8_t)
 -> bool {
    return length as libc::c_int == 4 as libc::c_int ||
               length as libc::c_int == 32 as libc::c_int;
}
...
unsafe extern "C" fn ibusDataReceive(mut c: uint16_t,
    ...
    ibusTime = micros();
    if ibusTime.wrapping_sub(ibusTimeLast) >
           500 as libc::c_int as libc::c_uint {
        ibusFramePosition = 0 as libc::c_int as uint8_t;
        rxBytesToIgnore = 0 as libc::c_int as uint8_t
    } else if rxBytesToIgnore != 0 {
        rxBytesToIgnore = rxBytesToIgnore.wrapping_sub(1);
        return
    }
    ibusTimeLast = ibusTime;
    if ibusFramePosition as libc::c_int == 0 as libc::c_int {
        if isValidIa6bIbusPacketLength(c as uint8_t) {
            ibusModel = 0 as libc::c_int as uint8_t;
            ibusSyncByte = c as uint8_t;
            ibusFrameSize = c as uint8_t;
            ibusChannelOffset = 2 as libc::c_int as uint8_t;
            ibusChecksum = 0xffff as libc::c_int as uint16_t
        } else if ibusSyncByte as libc::c_int == 0 as libc::c_int &&
                      c as libc::c_int == 0x55 as libc::c_int {
            ibusModel = 1 as libc::c_int as uint8_t;
            ibusSyncByte = 0x55 as libc::c_int as uint8_t;
            ibusFrameSize = 31 as libc::c_int as uint8_t;
            ibusChecksum = 0 as libc::c_int as uint16_t;
            ibusChannelOffset = 1 as libc::c_int as uint8_t
        } else if ibusSyncByte as libc::c_int != c as libc::c_int { return }
    }
    ibus[ibusFramePosition as usize] = c as uint8_t;
    if ibusFramePosition as libc::c_int ==
           ibusFrameSize as libc::c_int - 1 as libc::c_int {
        ibusFrameDone = 1 as libc::c_int != 0
...
unsafe extern "C" fn isChecksumOkIa6() -> bool {
    ...
    chksum = ibusChecksum;
    rxsum =
        (ibus[(ibusFrameSize as libc::c_int - 2 as libc::c_int) as usize] as
             libc::c_int +
             ((ibus[(ibusFrameSize as libc::c_int - 1 as libc::c_int) as
        ...
        chksum =
            (chksum as libc::c_int +
                 (ibus[offset as usize] as libc::c_int +
                      ((ibus[(offset as libc::c_int + 1 as libc::c_int) as
                                 usize] as libc::c_int) << 8 as libc::c_int)))
                as uint16_t;
        i = i.wrapping_add(1);
        offset = (offset as libc::c_int + 2 as libc::c_int) as uint8_t
    }
    return chksum as libc::c_int == rxsum as libc::c_int;
}
unsafe extern "C" fn checksumIsOk() -> bool {
    if ibusModel as libc::c_int == 1 as libc::c_int {
...
unsafe extern "C" fn updateChannelData() {
    ...
    while (i as libc::c_int) < 14 as libc::c_int {
        ibusChannelData[i as usize] =
            (ibus[offset as usize] as libc::c_int +
                 ((ibus[(offset as libc::c_int + 1 as libc::c_int) as usize]
...
unsafe extern "C" fn ibusFrameStatus(mut rxRuntimeConfig:
    ...
    ibusFrameDone = 0 as libc::c_int != 0;
    if checksumIsOk() {
        if ibusModel as libc::c_int == 1 as libc::c_int ||
               ibusSyncByte as libc::c_int == 0x20 as libc::c_int {
            updateChannelData();
            frameStatus = RX_FRAME_COMPLETE as libc::c_int as uint8_t
        } else { rxBytesToIgnore = respondToIbusRequest(ibus.as_mut_ptr()) }
    }
    return frameStatus;
}
...
pub unsafe extern "C" fn ibusInit(mut rxConfig: *const rxConfig_t,
 ...
 -> bool {
    ibusSyncByte = 0 as libc::c_int as uint8_t; // TODO - Verify speed
    ...
    if portShared { initSharedIbusTelemetry(ibusPort); }
