...
unsafe extern "C" fn ReadBit() -> int8_t {
    ...
    loop  {
        ...
        if micros() > timeout_timer {
            current_block = 2253656888985693936;
            break ;
        }
    }
    match current_block {
    ...
    *bt = 0 as libc::c_int as uint8_t;
    ...
    loop  {
        ...
        if bit as libc::c_int == -(1 as libc::c_int) {
            current_block = 3817464872396296982;
            break ;
        }
        if bit as libc::c_int == 1 as libc::c_int {
            *bt =
    ...
    match current_block {
        3817464872396296982 => { return 0 as libc::c_int as uint8_t }
        _ => {
            ckSumIn =
    ...
    if !(waitcycl == 0 as libc::c_int as libc::c_uint) {
        ...
        loop  {
            ...
            if ReadBit() as libc::c_int == -(1 as libc::c_int) {
                current_block = 5758729159740778740;
                break ;
            }
            i = i.wrapping_add(1)
        }
        match current_block {
            ...
            _ => {
                ...
                loop  {
                    ...
                    if bit as libc::c_int == -(1 as libc::c_int) {
                        current_block = 5758729159740778740; // NumTX
                        break ; // NumRX
                    } // RxStartAdr
