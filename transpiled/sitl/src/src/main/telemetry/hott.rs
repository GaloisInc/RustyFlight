...
static mut lastHoTTRequestCheckAt: uint32_t = 0i32 as uint32_t;
static mut lastMessagesPreparedAt: uint32_t = 0i32 as uint32_t;
static mut lastHottAlarmSoundTime: uint32_t = 0i32 as uint32_t;
static mut hottIsSending: bool = 0i32 != 0;
...
static mut hottTelemetryEnabled: bool = 0i32 != 0;
...
unsafe extern "C" fn initialiseEAMMessage(mut msg: *mut HOTT_EAM_MSG_t,
                                          mut size: size_t) {
    memset(msg as *mut libc::c_void, 0i32, size);
    (*msg).start_byte = 0x7ci32 as uint8_t;
    (*msg).eam_sensor_id = 0x8ei32 as uint8_t;
    (*msg).sensor_id = 0xe0i32 as uint8_t;
    (*msg).stop_byte = 0x7di32 as uint8_t;
}
unsafe extern "C" fn initialiseGPSMessage(mut msg: *mut HOTT_GPS_MSG_t,
                                          mut size: size_t) {
    memset(msg as *mut libc::c_void, 0i32, size);
    (*msg).start_byte = 0x7ci32 as uint8_t;
    (*msg).gps_sensor_id = 0x8ai32 as uint8_t;
    (*msg).sensor_id = 0xa0i32 as uint8_t;
    (*msg).stop_byte = 0x7di32 as uint8_t;
}
...
pub unsafe extern "C" fn addGPSCoordinates(mut hottGPSMessage_0:
    ...
    let mut deg: int16_t =
        (latitude as libc::c_long / 10000000i64) as int16_t;
    let mut sec: int32_t =
        ((latitude as libc::c_long - deg as libc::c_long * 10000000i64) *
             6i32 as libc::c_long) as int32_t;
    let mut min: int8_t = (sec as libc::c_long / 1000000i64) as int8_t;
    sec = (sec as libc::c_long % 1000000i64 / 100i64) as int32_t;
    let mut degMin: uint16_t =
        (deg as libc::c_long * 100i64 + min as libc::c_long) as uint16_t;
    (*hottGPSMessage_0).pos_NS = (latitude < 0i32) as libc::c_int as uint8_t;
    (*hottGPSMessage_0).pos_NS_dm_L = degMin as uint8_t;
    (*hottGPSMessage_0).pos_NS_dm_H =
        (degMin as libc::c_int >> 8i32) as uint8_t;
    (*hottGPSMessage_0).pos_NS_sec_L = sec as uint8_t;
    (*hottGPSMessage_0).pos_NS_sec_H = (sec >> 8i32) as uint8_t;
    deg = (longitude as libc::c_long / 10000000i64) as int16_t;
    sec =
        ((longitude as libc::c_long - deg as libc::c_long * 10000000i64) *
             6i32 as libc::c_long) as int32_t;
    min = (sec as libc::c_long / 1000000i64) as int8_t;
    sec = (sec as libc::c_long % 1000000i64 / 100i64) as int32_t;
    degMin = (deg as libc::c_long * 100i64 + min as libc::c_long) as uint16_t;
    (*hottGPSMessage_0).pos_EW = (longitude < 0i32) as libc::c_int as uint8_t;
    (*hottGPSMessage_0).pos_EW_dm_L = degMin as uint8_t;
    (*hottGPSMessage_0).pos_EW_dm_H =
        (degMin as libc::c_int >> 8i32) as uint8_t;
    (*hottGPSMessage_0).pos_EW_sec_L = sec as uint8_t;
    (*hottGPSMessage_0).pos_EW_sec_H = (sec >> 8i32) as uint8_t;
}
#[no_mangle]
pub unsafe extern "C" fn hottPrepareGPSResponse(mut hottGPSMessage_0:
    ...
    if gpsSol.numSat as libc::c_int >= 5i32 {
        (*hottGPSMessage_0).gps_fix_char =
    ...
    let speed: uint16_t =
        (gpsSol.groundSpeed as libc::c_int * 36i32 / 1000i32) as
            uint16_t; // gpsSol.llh.alt in m ; offset = 500 -> O m
    (*hottGPSMessage_0).gps_speed_L =
        (speed as libc::c_int & 0xffi32) as uint8_t;
    (*hottGPSMessage_0).gps_speed_H =
        (speed as libc::c_int >> 8i32) as uint8_t;
    (*hottGPSMessage_0).home_distance_L =
        (GPS_distanceToHome as libc::c_int & 0xffi32) as uint8_t;
    (*hottGPSMessage_0).home_distance_H =
        (GPS_distanceToHome as libc::c_int >> 8i32) as uint8_t;
    ...
    let hottGpsAltitude: uint16_t =
        (altitude as libc::c_int / 100i32 + 500i32) as uint16_t;
    (*hottGPSMessage_0).altitude_L =
        (hottGpsAltitude as libc::c_int & 0xffi32) as uint8_t;
    (*hottGPSMessage_0).altitude_H =
        (hottGpsAltitude as libc::c_int >> 8i32) as uint8_t;
...
unsafe extern "C" fn updateAlarmBatteryStatus(mut hottEAMMessage_0:
    ...
    if shouldTriggerBatteryAlarmNow() {
        ...
        if batteryState as libc::c_uint ==
            ...
            (*hottEAMMessage_0).warning_beeps = 0x10i32 as uint8_t;
...
unsafe extern "C" fn hottEAMUpdateBattery(mut hottEAMMessage_0:
    ...
    (*hottEAMMessage_0).main_voltage_L =
        (getBatteryVoltage() as libc::c_int & 0xffi32) as uint8_t;
    (*hottEAMMessage_0).main_voltage_H =
        (getBatteryVoltage() as libc::c_int >> 8i32) as uint8_t;
...
unsafe extern "C" fn hottEAMUpdateCurrentMeter(mut hottEAMMessage_0:
                                                   *mut HOTT_EAM_MSG_t) {
    let amp: int32_t = getAmperage() / 10i32;
    (*hottEAMMessage_0).current_L = (amp & 0xffi32) as uint8_t;
    (*hottEAMMessage_0).current_H = (amp >> 8i32) as uint8_t;
}
#[inline]
unsafe extern "C" fn hottEAMUpdateBatteryDrawnCapacity(mut hottEAMMessage_0:
                                                           *mut HOTT_EAM_MSG_t) {
    let mAh: int32_t = getMAhDrawn() / 10i32;
    (*hottEAMMessage_0).batt_cap_L = (mAh & 0xffi32) as uint8_t;
    (*hottEAMMessage_0).batt_cap_H = (mAh >> 8i32) as uint8_t;
}
#[inline]
unsafe extern "C" fn hottEAMUpdateAltitude(mut hottEAMMessage_0:
                                               *mut HOTT_EAM_MSG_t) {
    let hottEamAltitude: uint16_t =
        (getEstimatedAltitude() / 100i32 + 500i32) as uint16_t;
    (*hottEAMMessage_0).altitude_L =
        (hottEamAltitude as libc::c_int & 0xffi32) as uint8_t;
    (*hottEAMMessage_0).altitude_H =
        (hottEamAltitude as libc::c_int >> 8i32) as uint8_t;
}
#[inline]
unsafe extern "C" fn hottEAMUpdateClimbrate(mut hottEAMMessage_0:
                                                *mut HOTT_EAM_MSG_t) {
    let vario: int32_t = getEstimatedVario() as int32_t;
    (*hottEAMMessage_0).climbrate_L = (30000i32 + vario & 0xffi32) as uint8_t;
    (*hottEAMMessage_0).climbrate_H = (30000i32 + vario >> 8i32) as uint8_t;
    (*hottEAMMessage_0).climbrate3s = (120i32 + vario / 100i32) as uint8_t;
}
#[no_mangle]
pub unsafe extern "C" fn hottPrepareEAMResponse(mut hottEAMMessage_0:
                                                    *mut HOTT_EAM_MSG_t) {
    // Reset alarms
    (*hottEAMMessage_0).warning_beeps = 0i32 as uint8_t;
    (*hottEAMMessage_0).alarm_invers1 = 0i32 as uint8_t;
...
unsafe extern "C" fn hottSerialWrite(mut c: uint8_t) {
    static mut serialWrites: uint8_t = 0i32 as uint8_t;
...
unsafe extern "C" fn flushHottRxBuffer() {
    while serialRxBytesWaiting(hottPort) > 0i32 as libc::c_uint {
        serialRead(hottPort);
    };
}
unsafe extern "C" fn workAroundForHottTelemetryOnUsart(mut instance:
    ...
    hottPort =
        openSerialPort((*instance).identifier as serialPortIdentifier_e,
                       ...
                       19200i32 as uint32_t, mode, portOptions);
}
...
unsafe extern "C" fn hottConfigurePortForTX() {
    ...
    hottIsSending = 1i32 != 0;
    hottMsgCrc = 0i32 as uint8_t;
}
...
pub unsafe extern "C" fn configureHoTTTelemetryPort() {
    ...
    hottPort =
        openSerialPort((*portConfig).identifier, FUNCTION_TELEMETRY_HOTT,
                       None, 0 as *mut libc::c_void, 19200i32 as uint32_t,
                       MODE_RXTX, portOptions);
    if hottPort.is_null() { return }
    hottConfigurePortForRX();
    hottTelemetryEnabled = 1i32 != 0;
}
...
unsafe extern "C" fn hottSendTelemetryData() {
    ...
    if hottMsgRemainingBytesToSendCount as libc::c_int == 0i32 {
        hottConfigurePortForRX();
        return
    }
    hottMsgRemainingBytesToSendCount =
...
unsafe extern "C" fn shouldPrepareHoTTMessages(mut currentMicros: uint32_t)
 -> bool {
    return currentMicros.wrapping_sub(lastMessagesPreparedAt) >=
               (1000i32 * 1000i32 / 5i32) as libc::c_uint;
}
#[inline]
unsafe extern "C" fn shouldCheckForHoTTRequest() -> bool {
    if hottIsSending { return 0i32 != 0 }
    return 1i32 != 0;
}
#[no_mangle]
...
pub unsafe extern "C" fn handleHoTTTelemetry(mut currentTimeUs: timeUs_t) {
    ...
    if hottIsSending {
        if currentTimeUs.wrapping_sub(serialTimer) < 3000i32 as libc::c_uint {
            return
        }
    }
