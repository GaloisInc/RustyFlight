    ...
    tmpreg =
        *((0x40000000 as libc::c_int as
               uint32_t).wrapping_add(0x10000 as libc::c_int as
                                          libc::c_uint).wrapping_add(0x38 as
                                                                         libc::c_int
                                                                         as
                                                                         libc::c_uint).wrapping_add(OPAMP_Selection)
              as *mut uint32_t);
    /* !< Clear the inverting and non inverting bits selection bits */
    tmpreg &= 0xffffff93 as libc::c_uint;
...
pub unsafe extern "C" fn OPAMP_StructInit(mut OPAMP_InitStruct:
    ...
    (*OPAMP_InitStruct).OPAMP_NonInvertingInput =
        0 as libc::c_int as uint32_t;
    (*OPAMP_InitStruct).OPAMP_InvertingInput = 0 as libc::c_int as uint32_t;
}
/* *
...
pub unsafe extern "C" fn OPAMP_PGAConfig(mut OPAMP_Selection: uint32_t,
    ...
    ::core::ptr::write_volatile(fresh0,
                                (::core::ptr::read_volatile::<uint32_t>(fresh0
                                                                            ...
                                                                            *const uint32_t)
                                     as libc::c_uint &
                                     !(0x3c000 as libc::c_int as uint32_t)) as
...
pub unsafe extern "C" fn OPAMP_TimerControlledMuxConfig(mut OPAMP_Selection:
    ...
    tmpreg |=
        (*OPAMP_InitStruct).OPAMP_InvertingInput << 3 as libc::c_int |
            (*OPAMP_InitStruct).OPAMP_NonInvertingInput << 7 as libc::c_int;
    /* !< Write to OPAMPx_CSR register */
    ::core::ptr::write_volatile((0x40000000 as libc::c_int as
                                     uint32_t).wrapping_add(0x10000 as
                                                                libc::c_int as
...
pub unsafe extern "C" fn OPAMP_GetOutputLevel(mut OPAMP_Selection: uint32_t)
 -> uint32_t {
    let mut opampout: uint32_t = 0 as libc::c_int as uint32_t;
    /* Check the parameters */
    ...
    if *((0x40000000 as libc::c_int as
        ...
        opampout = 0x40000000 as libc::c_int as uint32_t
    } else { opampout = 0 as libc::c_int as uint32_t }
...
pub unsafe extern "C" fn OPAMP_OffsetTrimConfig(mut OPAMP_Selection: uint32_t,
    ...
    tmpreg &= !((0x1f as libc::c_int as uint32_t) << OPAMP_Input);
