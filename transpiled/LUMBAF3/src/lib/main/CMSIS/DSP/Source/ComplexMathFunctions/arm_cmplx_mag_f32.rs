...
#[inline(always)]
unsafe extern "C" fn arm_sqrt_f32(mut in_0: float32_t,
                                  mut pOut: *mut float32_t) -> arm_status {
    if in_0 >= 0.0f32 {
        *pOut = sqrtf(in_0);
        return ARM_MATH_SUCCESS
    } else { *pOut = 0.0f32; return ARM_MATH_ARGUMENT_ERROR };
}
/* *****************************************************************************
 * @file     arm_math.h
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn arm_cmplx_mag_f32(mut pSrc: *mut float32_t,
                                           mut pDst: *mut float32_t,
                                           mut numSamples: uint32_t) {
    let mut realIn: float32_t =
        0.; /* Temporary variables to hold input values */
    let mut imagIn: float32_t = 0.;
    /* Run the below code for Cortex-M4 and Cortex-M3 */
    let mut blkCnt: uint32_t = 0; /* loop counter */
    /*loop Unrolling */
    blkCnt = numSamples >> 2 as libc::c_uint;
   ...
   ** a second loop below computes the remaining 1 to 3 samples. */
    while blkCnt > 0 as libc::c_uint {
        ...
        let fresh2 = pDst;
        pDst = pDst.offset(1);
        arm_sqrt_f32(realIn * realIn + imagIn * imagIn, fresh2);
        let fresh3 = pSrc;
        pSrc = pSrc.offset(1);
        realIn = *fresh3;
        let fresh4 = pSrc;
        ...
        let fresh5 = pDst;
        pDst = pDst.offset(1);
        arm_sqrt_f32(realIn * realIn + imagIn * imagIn, fresh5);
        let fresh6 = pSrc;
        pSrc = pSrc.offset(1);
        realIn = *fresh6;
        let fresh7 = pSrc;
        ...
        let fresh8 = pDst;
        pDst = pDst.offset(1);
        arm_sqrt_f32(realIn * realIn + imagIn * imagIn, fresh8);
        let fresh9 = pSrc;
        pSrc = pSrc.offset(1);
        realIn = *fresh9;
        let fresh10 = pSrc;
        ...
        let fresh11 = pDst;
        pDst = pDst.offset(1);
        arm_sqrt_f32(realIn * realIn + imagIn * imagIn, fresh11);
        /* Decrement the loop counter */
        blkCnt = blkCnt.wrapping_sub(1)
    }
    /* If the numSamples is not a multiple of 4, compute any remaining output samples here.
   ** No loop unrolling is used. */
    blkCnt = numSamples.wrapping_rem(0x4 as libc::c_uint);
    while blkCnt > 0 as libc::c_uint {
        /* C[0] = sqrt(A[0] * A[0] + A[1] * A[1]) */
        let fresh12 = pSrc;
        pSrc = pSrc.offset(1);
        ...
        let fresh14 = pDst;
        pDst = pDst.offset(1);
        arm_sqrt_f32(realIn * realIn + imagIn * imagIn, fresh14);
        /* Decrement the loop counter */
        blkCnt = blkCnt.wrapping_sub(1)
    };
