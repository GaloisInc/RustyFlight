...
//
    ...
    #[no_mangle]
    fn serialRemovePort(identifier: serialPortIdentifier_e);
    #[no_mangle]
    fn flashfsInit();
    #[no_mangle]
    fn ledStripInit();
...
pub unsafe extern "C" fn init() {
    ...
    if !readSuccess || !isEEPROMVersionValid() ||
           strncasecmp((*systemConfig()).boardIdentifier.as_ptr(),
                       b"SRF3\x00" as *const u8 as *const libc::c_char,
    ...
    spiPinConfigure(spiPinConfig(0i32));
    // Initialize CS lines and keep them high
    spiPreInit();
    spiInit(SPIDEV_2);
    // USE_SPI
    i2cHardwareConfigure(i2cConfig(0i32));
...
// XXX And these no longer work properly as FEATURE_RANGEFINDER does control HCSR04 runtime configuration.
    if feature(FEATURE_RANGEFINDER as libc::c_int as uint32_t) as libc::c_int
           != 0 &&
           feature(FEATURE_SOFTSERIAL as libc::c_int as uint32_t) as
               libc::c_int != 0 {
    ...
    while i < 10i32 {
        ledToggle(1i32);
        ledToggle(0i32);
        delay(25i32 as timeMs_t);
        if (*beeperConfig()).beeper_off_flags &
               (1i32 << BEEPER_SYSTEM_INIT as libc::c_int - 1i32) as
                   libc::c_uint == 0 {
            systemBeep(1i32 != 0);
        }
        delay(25i32 as timeMs_t);
