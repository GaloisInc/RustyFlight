...
pub const CRSF_FRAME_ORIGIN_DEST_SIZE: C2RustUnnamed_3 = 2;
pub const CRSF_FRAME_RX_MSP_FRAME_SIZE: C2RustUnnamed_3 = 8;
pub const CRSF_FRAME_TX_MSP_FRAME_SIZE: C2RustUnnamed_3 = 58;
#[derive ( Copy, Clone )]
#[repr(C)]
...
static mut crsfFrameDone: bool = 0i32 != 0;
...
static mut crsfFrameStartAtUs: uint32_t = 0i32 as uint32_t;
...
unsafe extern "C" fn crsfFrameCRC() -> uint8_t {
    ...
    let mut crc: uint8_t =
        crc8_dvb_s2(0i32 as uint8_t, crsfFrame.frame.type_0);
...
unsafe extern "C" fn crsfDataReceive(mut c: uint16_t,
    ...
    static mut crsfFramePosition: uint8_t = 0i32 as uint8_t;
    let currentTimeUs: uint32_t = micros();
    if currentTimeUs >
           crsfFrameStartAtUs.wrapping_add(1100i32 as libc::c_uint) {
        // We've received a character after max time needed to complete a frame,
        // so this must be the start of a new frame.
        crsfFramePosition = 0i32 as uint8_t
    }
    if crsfFramePosition as libc::c_int == 0i32 {
        crsfFrameStartAtUs = currentTimeUs
    }
    // assume frame is 5 bytes long until we have received the frame length
    // full frame length includes the length of the address and framelength fields
    let fullFrameLength: libc::c_int =
        if (crsfFramePosition as libc::c_int) < 3i32 {
            5i32
        } else {
            (crsfFrame.frame.frameLength as libc::c_int +
        ...
        if crsfFrameDone {
            ...
            if crsfFrame.frame.type_0 as libc::c_int !=
                ...
                if crc as libc::c_int ==
                       crsfFrame.bytes[(fullFrameLength - 1i32) as usize] as
                           libc::c_int {
                    match crsfFrame.frame.type_0 as libc::c_int {
                        122 | 124 => {
                            let mut frameStart: *mut uint8_t =
                                (&mut crsfFrame.frame.payload as
                                     ...
                                     *mut uint8_t).offset(CRSF_FRAME_ORIGIN_DEST_SIZE
                                                              as libc::c_int
                                                              as isize);
                            if bufferCrsfMspFrame(frameStart,
...
pub unsafe extern "C" fn crsfRxInit(mut rxConfig: *const rxConfig_t,
    ...
    (*rxRuntimeConfig).rxRefreshRate = 6667i32 as uint16_t;
    ...
    serialPort =
        openSerialPort((*portConfig).identifier, FUNCTION_RX_SERIAL,
                       ...
                       420000i32 as uint32_t, MODE_RXTX,
