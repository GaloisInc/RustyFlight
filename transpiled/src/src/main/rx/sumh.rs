...
static mut sumhFrameDone: bool = 0i32 != 0;
...
unsafe extern "C" fn sumhFrameStatus(mut rxRuntimeConfig:
    ...
    sumhFrameDone = 0i32 != 0;
    if !(sumhFrame[0] as libc::c_int == 0xa8i32 &&
             sumhFrame[(21i32 - 2i32) as usize] as libc::c_int == 0i32) {
        return RX_FRAME_PENDING as libc::c_int as uint8_t
    }
    channelIndex = 0i32 as uint8_t;
    while (channelIndex as libc::c_int) < 8i32 {
        sumhChannels[channelIndex as usize] =
            (((sumhFrame[(((channelIndex as libc::c_int) << 1i32) + 3i32) as
                             usize] as uint32_t) <<
                  8i32).wrapping_add(sumhFrame[(((channelIndex as libc::c_int)
                                                     << 1i32) + 4i32) as
                                                   usize] as libc::c_uint) as
                 libc::c_float / 6.4f32 - 375i32 as libc::c_float) as
                uint32_t;
...
unsafe extern "C" fn sumhReadRawRC(mut rxRuntimeConfig:
    ...
    if chan as libc::c_int >= 8i32 { return 0i32 as uint16_t }
