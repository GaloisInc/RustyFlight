...
pub unsafe extern "C" fn pgResetFn_motorConfig(mut motorConfig_0:
    ...
    if hardwareMotorType as libc::c_int == MOTOR_BRUSHED as libc::c_int {
        (*motorConfig_0).minthrottle = 1000i32 as uint16_t;
        (*motorConfig_0).dev.motorPwmRate = 16000i32 as uint16_t;
        (*motorConfig_0).dev.motorPwmProtocol =
            PWM_TYPE_BRUSHED as libc::c_int as uint8_t;
        (*motorConfig_0).dev.useUnsyncedPwm = 1i32 as uint8_t
    } else {
...
pub unsafe extern "C" fn initEscEndpoints() {
    ...
    match (*motorConfig()).dev.motorPwmProtocol as libc::c_int {
        9 | 8 | 7 | 6 | 5 => {
            disarmMotorOutput = 0i32 as libc::c_float;
            if feature(FEATURE_3D as libc::c_int as uint32_t) {
                motorOutputLow =
                    48i32 as libc::c_float +
                        (1047i32 - 48i32) as libc::c_float / 100.0f32 *
                            (0.01f32 *
                ...
                motorOutputLow =
                    48i32 as libc::c_float +
                        (2047i32 - 48i32) as libc::c_float / 100.0f32 *
                            (0.01f32 *
                                 (*motorConfig()).digitalIdleOffsetValue as
                                     libc::c_int as libc::c_float)
            }
            motorOutputHigh = 2047i32 as libc::c_float;
            deadbandMotor3dHigh =
                1048i32 as libc::c_float +
                    (2047i32 - 1048i32) as libc::c_float / 100.0f32 *
                        (0.01f32 *
                             (*motorConfig()).digitalIdleOffsetValue as
                                 libc::c_int as libc::c_float);
            deadbandMotor3dLow = 1047i32 as libc::c_float
        }
        _ => {
            if feature(FEATURE_3D as libc::c_int as uint32_t) {
                disarmMotorOutput =
...
pub unsafe extern "C" fn convertExternalToMotor(mut externalValue: uint16_t)
 -> libc::c_float {
    ...
    match isMotorProtocolDshot() as libc::c_int {
        1 => {
            externalValue =
                constrain(externalValue as libc::c_int, 1000i32, 2000i32) as
                    uint16_t;
            if feature(FEATURE_3D as libc::c_int as uint32_t) {
                if externalValue as libc::c_int == 1500i32 {
                    motorValue = 0i32 as uint16_t
                } else if (externalValue as libc::c_int) < 1500i32 {
                    motorValue =
                        scaleRange(externalValue as libc::c_int, 1000i32,
                                   1500i32 - 1i32, 1047i32, 48i32) as uint16_t
                } else {
                    motorValue =
                        scaleRange(externalValue as libc::c_int,
                                   1500i32 + 1i32, 2000i32, 1048i32, 2047i32)
                            as uint16_t
                }
            } else {
                motorValue =
                    if externalValue as libc::c_int == 1000i32 {
                        0i32
                    } else {
                        scaleRange(externalValue as libc::c_int,
                                   1000i32 + 1i32, 2000i32, 48i32, 2047i32)
                    } as uint16_t
            }
        }
...
pub unsafe extern "C" fn convertMotorToExternal(mut motorValue: libc::c_float)
 -> uint16_t {
    ...
    match isMotorProtocolDshot() as libc::c_int {
        1 => {
            if feature(FEATURE_3D as libc::c_int as uint32_t) {
                if motorValue == 0i32 as libc::c_float ||
                       motorValue < 48i32 as libc::c_float {
                    externalValue = 1500i32 as uint16_t
                } else if motorValue <= 1047i32 as libc::c_float {
                    externalValue =
                        scaleRange(motorValue as libc::c_int, 48i32, 1047i32,
                                   1500i32 - 1i32, 1000i32) as uint16_t
                } else {
                    externalValue =
                        scaleRange(motorValue as libc::c_int, 1048i32,
                                   2047i32, 1500i32 + 1i32, 2000i32) as
                            uint16_t
                }
            } else {
                externalValue =
                    if motorValue < 48i32 as libc::c_float {
                        1000i32
                    } else {
                        scaleRange(motorValue as libc::c_int, 48i32, 2047i32,
                                   1000i32 + 1i32, 2000i32)
                    } as uint16_t
            }
        }
