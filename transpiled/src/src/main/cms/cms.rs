...
static mut cmsCurrentDevice: libc::c_int = -1i32;
/*
 * This file is part of Cleanflight and Betaflight.
 *
 * Cleanflight and Betaflight are free software. You can redistribute
...
pub unsafe extern "C" fn cmsDisplayPortRegister(mut pDisplay:
                                                    *mut displayPort_t)
 -> bool {
    if cmsDeviceCount == 4i32 { return 0i32 != 0 } // -1 Okay
...
unsafe extern "C" fn cmsDisplayPortSelectCurrent() -> *mut displayPort_t {
    if cmsDeviceCount == 0i32 { return 0 as *mut displayPort_t }
    if cmsCurrentDevice < 0i32 { cmsCurrentDevice = 0i32 }
    return cmsDisplayPorts[cmsCurrentDevice as usize];
}
unsafe extern "C" fn cmsDisplayPortSelectNext() -> *mut displayPort_t {
    if cmsDeviceCount == 0i32 { return 0 as *mut displayPort_t }
    cmsCurrentDevice = (cmsCurrentDevice + 1i32) % cmsDeviceCount;
    return cmsDisplayPorts[cmsCurrentDevice as usize];
}
#[no_mangle]
pub unsafe extern "C" fn cmsDisplayPortSelect(mut instance:
                                                  *mut displayPort_t)
 -> bool {
    if cmsDeviceCount == 0i32 { return 0i32 != 0 }
    let mut i: libc::c_int = 0i32;
    while i < cmsDeviceCount {
        if cmsDisplayPortSelectNext() == instance { return 1i32 != 0 }
        i += 1
    }
    return 0i32 != 0;
}
...
unsafe extern "C" fn cmsPageNext(mut instance: *mut displayPort_t) {
    cmsPageSelect(instance,
                  (currentCtx.page as libc::c_int + 1i32) as int8_t);
}
...
unsafe extern "C" fn cmsFormatFloat(mut value: int32_t,
    ...
    let mut k: uint8_t = 0;
    // np. 3450
    itoa(100000i32 + value, floatString,
         10i32); // Create string from abs of integer value
    // 103450
    *floatString.offset(0) = *floatString.offset(1);
    *floatString.offset(1) = *floatString.offset(2);
    *floatString.offset(2) = '.' as i32 as libc::c_char;
    // 03.450
    // usuwam koncowe zera i kropke
    // Keep the first decimal place
    k = 5i32 as uint8_t;
    while k as libc::c_int > 3i32 {
        if !(*floatString.offset(k as isize) as libc::c_int == '0' as i32 ||
                 *floatString.offset(k as isize) as libc::c_int == '.' as i32)
           {
            break ;
        }
        *floatString.offset(k as isize) = 0i32 as libc::c_char;
        k = k.wrapping_sub(1)
    }
    // oraz zero wiodonce
    if *floatString.offset(0) as libc::c_int == '0' as i32 {
        *floatString.offset(0) = ' ' as i32 as libc::c_char
    };
}
...
unsafe extern "C" fn cmsPadLeftToSize(mut buf: *mut libc::c_char,
    ...
    *buf.offset(size as isize) = 0i32 as libc::c_char;
}
unsafe extern "C" fn cmsPadToSize(mut buf: *mut libc::c_char,
    ...
    *buf.offset(size as isize) =
        0i32 as libc::c_char; // Make room for null terminator.
...
unsafe extern "C" fn cmsDrawMenuEntry(mut pDisplay: *mut displayPort_t,
 ...
 -> libc::c_int {
    ...
    match (*p).type_0 as libc::c_uint {
        10 => {
            if (*p).flags as libc::c_int & 0x1i32 != 0 && !(*p).data.is_null()
               {
                ...
                cnt =
                    cmsDrawMenuItemValue(pDisplay, buff.as_mut_ptr(), row,
                                         12i32 as uint8_t);
                (*p).flags = ((*p).flags as libc::c_int & !0x1i32) as uint8_t
            }
        }
        3 | 4 => {
            if (*p).flags as libc::c_int & 0x1i32 != 0 {
                buff[0] = 0i32 as libc::c_char;
                if (*p).type_0 as libc::c_uint ==
                       OME_Submenu as libc::c_int as libc::c_uint &&
                       (*p).func.is_some() &&
                       (*p).flags as libc::c_int & 0x8i32 != 0 {
                    ...
                    strncpy(buff.as_mut_ptr(), str, 12i32 as libc::c_ulong);
                }
            ...
            if (*p).flags as libc::c_int & 0x1i32 != 0 {
                ...
                let mut str_0: *mut libc::c_char =
                    *(*ptr).names.offset(*(*ptr).val as isize) as
                        *mut libc::c_char;
                strncpy(buff.as_mut_ptr(), str_0, 12i32 as libc::c_ulong);
                cnt =
                    cmsDrawMenuItemValue(pDisplay, buff.as_mut_ptr(), row,
                                         12i32 as uint8_t);
                (*p).flags = ((*p).flags as libc::c_int & !0x1i32) as uint8_t
            ...
            if (*p).flags as libc::c_int & 0x1i32 != 0 && !(*p).data.is_null()
               {
                ...
                if *val as libc::c_int & 0x800i32 != 0 {
                    strcpy(buff.as_mut_ptr(),
                           b"YES\x00" as *const u8 as *const libc::c_char);
                } else {
                    strcpy(buff.as_mut_ptr(),
            ...
            if (*p).flags as libc::c_int & 0x1i32 != 0 && !(*p).data.is_null()
               {
                ...
                itoa(*(*ptr_0).val as libc::c_int, buff.as_mut_ptr(), 10i32);
                cnt =
                    cmsDrawMenuItemValue(pDisplay, buff.as_mut_ptr(), row,
                                         5i32 as uint8_t);
                (*p).flags = ((*p).flags as libc::c_int & !0x1i32) as uint8_t
            ...
            if (*p).flags as libc::c_int & 0x1i32 != 0 && !(*p).data.is_null()
               {
                ...
                itoa(*(*ptr_1).val as libc::c_int, buff.as_mut_ptr(), 10i32);
                cnt =
                    cmsDrawMenuItemValue(pDisplay, buff.as_mut_ptr(), row,
                                         5i32 as uint8_t);
                (*p).flags = ((*p).flags as libc::c_int & !0x1i32) as uint8_t
            ...
            if (*p).flags as libc::c_int & 0x1i32 != 0 && !(*p).data.is_null()
               {
                ...
                itoa(*(*ptr_2).val as libc::c_int, buff.as_mut_ptr(), 10i32);
                cnt =
                    cmsDrawMenuItemValue(pDisplay, buff.as_mut_ptr(), row,
                                         5i32 as uint8_t);
                (*p).flags = ((*p).flags as libc::c_int & !0x1i32) as uint8_t
            ...
            if (*p).flags as libc::c_int & 0x1i32 != 0 && !(*p).data.is_null()
               {
                let mut ptr_3: *mut OSD_UINT16_t =
                    (*p).data as *mut OSD_UINT16_t;
                itoa(*(*ptr_3).val as libc::c_int, buff.as_mut_ptr(), 10i32);
                cnt =
                    cmsDrawMenuItemValue(pDisplay, buff.as_mut_ptr(), row,
                                         5i32 as uint8_t);
                (*p).flags = ((*p).flags as libc::c_int & !0x1i32) as uint8_t
            ...
            if (*p).flags as libc::c_int & 0x1i32 != 0 && !(*p).data.is_null()
               {
                ...
                cnt =
                    displayWrite(pDisplay,
                                 (leftMenuColumn as libc::c_int + 1i32 +
...
unsafe extern "C" fn cmsDrawMenu(mut pDisplay: *mut displayPort_t,
    ...
    let mut drawPolled: bool = 0i32 != 0;
    static mut lastPolledUs: uint32_t = 0i32 as uint32_t;
    if currentTimeUs > lastPolledUs.wrapping_add(100000i32 as libc::c_uint) {
        drawPolled = 1i32 != 0;
    ...
    if (*pDisplay).cleared {
        ...
        while (*p).type_0 as libc::c_uint !=
                  OME_END as libc::c_int as libc::c_uint {
            (*p).flags = ((*p).flags as libc::c_int | 0x2i32) as uint8_t;
            (*p).flags = ((*p).flags as libc::c_int | 0x1i32) as uint8_t;
            p = p.offset(1);
            i = i.wrapping_add(1)
        }
        (*pDisplay).cleared = 0i32 != 0
    } else if drawPolled {
        p = pageTop;
        while p <= pageTop.offset(pageMaxRow as libc::c_int as isize) {
            if (*p).flags as libc::c_int & 0x4i32 != 0 {
                (*p).flags = ((*p).flags as libc::c_int | 0x1i32) as uint8_t
            }
            p = p.offset(1)
        }
    ...
    if (*pDisplay).cursorRow as libc::c_int >= 0i32 &&
    ...
    if room < 30i32 as libc::c_uint { return }
    ...
    while (i as libc::c_int) < maxMenuItems as libc::c_int &&
              (*p).type_0 as libc::c_uint !=
                  OME_END as libc::c_int as libc::c_uint {
        if (*p).flags as libc::c_int & 0x2i32 != 0 {
...
unsafe extern "C" fn cmsMenuCountPage(mut pDisplay: *mut displayPort_t) {
    ...
    pageCount =
        ((p.wrapping_offset_from((*currentCtx.menu).entries) as libc::c_long -
              1i32 as libc::c_long) / maxMenuItems as libc::c_long +
             1i32 as libc::c_long) as int8_t;
}
...
pub unsafe extern "C" fn cmsMenuChange(mut pDisplay: *mut displayPort_t,
 ...
 -> libc::c_long {
    ...
    if pMenu.is_null() { return 0i32 as libc::c_long }
    if pMenu != currentCtx.menu {
        ...
        currentCtx.cursorRow = 0i32 as int8_t;
        ...
        cmsPageSelect(pDisplay, 0i32 as int8_t);
    } else {
...
unsafe extern "C" fn cmsMenuBack(mut pDisplay: *mut displayPort_t)
 -> libc::c_long {
    ...
    if menuStackIdx == 0 { return 0i32 as libc::c_long }
...
pub unsafe extern "C" fn cmsMenuOpen() {
        ...
        currentCtx =
            {
                let mut init =
                    cmsCtx_s{menu: &mut menuMain,
                             page: 0i32 as uint8_t,
                             cursorRow: 0i32 as int8_t,};
                init
            };
    ...
    if ((*pCurrentDisplay).cols as libc::c_int) < 18i32 {
        smallScreen = 1i32 != 0;
        linesPerMenuItem = 2i32 as uint8_t;
        leftMenuColumn = 0i32 as uint8_t;
        ...
        smallScreen = 0i32 != 0;
        linesPerMenuItem = 1i32 as uint8_t;
        leftMenuColumn = 2i32 as uint8_t;
        rightMenuColumn =
            ((*pCurrentDisplay).cols as libc::c_int - 2i32) as uint8_t;
        maxMenuItems =
            ((*pCurrentDisplay).rows as libc::c_int - 2i32) as uint8_t
    }
...
pub unsafe extern "C" fn cmsMenuExit(mut pDisplay: *mut displayPort_t,
 ...
 -> libc::c_long {
    ...
    if exitType == 2i32 {
        ...
        displayWrite(pDisplay, 5i32 as uint8_t, 3i32 as uint8_t,
                     b"REBOOTING...\x00" as *const u8 as *const libc::c_char);
        displayResync(pDisplay);
        stopMotors();
        stopPwmAllMotors();
        delay(200i32 as timeMs_t);
        systemReset();
    }
...
unsafe extern "C" fn cmsHandleKey(mut pDisplay: *mut displayPort_t,
    ...
    let mut res: uint16_t = 250i32 as uint16_t;
    let mut p: *mut OSD_Entry = 0 as *mut OSD_Entry;
    if currentCtx.menu.is_null() { return res }
    if key as libc::c_int == 6i32 { cmsMenuOpen(); return 500i32 as uint16_t }
    if key as libc::c_int == 5i32 {
        cmsMenuBack(pDisplay);
        return 500i32 as uint16_t
    }
    if key as libc::c_int == 2i32 {
    ...
    if key as libc::c_int == 1i32 {
        currentCtx.cursorRow -= 1;
        // Skip non-title labels
        if (*pageTop.offset(currentCtx.cursorRow as libc::c_int as
                                isize)).type_0 as libc::c_uint ==
               OME_Label as libc::c_int as libc::c_uint &&
               currentCtx.cursorRow as libc::c_int > 0i32 {
            currentCtx.cursorRow -= 1
        }
        if currentCtx.cursorRow as libc::c_int == -1i32 ||
    ...
    if key as libc::c_int == 2i32 || key as libc::c_int == 1i32 { return res }
    ...
    match (*p).type_0 as libc::c_uint {
        3 => {
            if key as libc::c_int == 4i32 {
                ...
                res = 500i32 as uint16_t
            }
        }
        4 => {
            retval = 0;
            if (*p).func.is_some() && key as libc::c_int == 4i32 {
                retval =
                    (*p).func.expect("non-null function pointer")(pDisplay,
                                                                  (*p).data);
                if retval == -1i32 as libc::c_long { cmsMenuBack(pDisplay); }
                res = 500i32 as uint16_t
            }
        }
        2 => {
        ...
        1 => { cmsMenuBack(pDisplay); res = 500i32 as uint16_t }
        5 => {
            if !(*p).data.is_null() {
                let mut val: *mut uint8_t = (*p).data as *mut uint8_t;
                if key as libc::c_int == 4i32 {
                    *val = 1i32 as uint8_t
                } else { *val = 0i32 as uint8_t }
                (*p).flags = ((*p).flags as libc::c_int | 0x1i32) as uint8_t
            }
        }
        12 => {
            if !(*p).data.is_null() {
                ...
                if key as libc::c_int == 4i32 {
                    *val_0 = (*val_0 as libc::c_int | 0x800i32) as uint16_t
                } else {
                    *val_0 = (*val_0 as libc::c_int % !0x800i32) as uint16_t
                }
                (*p).flags = ((*p).flags as libc::c_int | 0x1i32) as uint8_t
            ...
            if (*p).type_0 as libc::c_uint ==
                ...
                if key as libc::c_int == 4i32 {
                    if (*(*ptr_0).val as libc::c_int) <
                        ...
                        *(*ptr_0).val =
                            (*(*ptr_0).val as libc::c_int + 1i32) as uint8_t
                    }
                } else if *(*ptr_0).val as libc::c_int > 0i32 {
                    *(*ptr_0).val =
                        (*(*ptr_0).val as libc::c_int - 1i32) as uint8_t
                }
                if (*p).func.is_some() {
...
pub unsafe extern "C" fn cmsHandleKeyWithRepeat(mut pDisplay:
 ...
 -> uint16_t {
    let mut ret: uint16_t = 0i32 as uint16_t;
    let mut i: libc::c_int = 0i32;
...
pub unsafe extern "C" fn cmsUpdate(mut currentTimeUs: uint32_t) {
    ...
    static mut rcDelayMs: int16_t = 250i32 as int16_t;
    static mut holdCount: libc::c_int = 1i32;
    static mut repeatCount: libc::c_int = 1i32;
    static mut repeatBase: libc::c_int = 0i32;
    static mut lastCalledMs: uint32_t = 0i32 as uint32_t;
    static mut lastCmsHeartBeatMs: uint32_t = 0i32 as uint32_t;
    let currentTimeMs: uint32_t =
        currentTimeUs.wrapping_div(1000i32 as libc::c_uint);
    if !cmsInMenu {
        // Detect menu invocation
        if rcData[THROTTLE as libc::c_int as usize] as libc::c_int > 1250i32
               &&
               (rcData[THROTTLE as libc::c_int as usize] as libc::c_int) <
                   1750i32 &&
               (rcData[YAW as libc::c_int as usize] as libc::c_int) < 1250i32
               &&
               rcData[PITCH as libc::c_int as usize] as libc::c_int > 1750i32
               && armingFlags as libc::c_int & ARMED as libc::c_int == 0 {
            cmsMenuOpen();
            rcDelayMs = 500i32 as int16_t
        ...
        // Scan 'key' first
        //
        let mut key: uint8_t = 0i32 as uint8_t;
        if rcData[THROTTLE as libc::c_int as usize] as libc::c_int > 1250i32
               &&
               (rcData[THROTTLE as libc::c_int as usize] as libc::c_int) <
                   1750i32 &&
            ...
            key = 3i32 as uint8_t
                      ...
                      1750i32 {
            key = 4i32 as uint8_t
        } else if rcData[YAW as libc::c_int as usize] as libc::c_int > 1750i32
                      ||
        ...
        if rcDelayMs as libc::c_int > 0i32 {
            rcDelayMs =
                (rcDelayMs as
            ...
            if (key as libc::c_int == 3i32 || key as libc::c_int == 4i32) &&
                   holdCount > 20i32 {
                // Decrease rcDelayMs reciprocally
                rcDelayMs =
                    (rcDelayMs as libc::c_int / (holdCount - 20i32)) as
                        int16_t;
                // When we reach the scheduling limit,
                if rcDelayMs as libc::c_int <= 50i32 {
                    // start calling handler multiple times.
                    if repeatBase == 0i32 { repeatBase = holdCount }
                    repeatCount =
                        repeatCount + (holdCount - repeatBase) / 5i32;
                    if repeatCount > 5i32 { repeatCount = 5i32 }
                }
            }
        }
        cmsDrawMenu(pCurrentDisplay, currentTimeUs);
        if currentTimeMs >
               lastCmsHeartBeatMs.wrapping_add(500i32 as libc::c_uint) {
...
pub unsafe extern "C" fn cmsHandler(mut currentTimeUs: timeUs_t) {
    if cmsDeviceCount < 0i32 { return }
    static mut lastCalledUs: timeUs_t = 0i32 as timeUs_t;
    if currentTimeUs >= lastCalledUs.wrapping_add(50000i32 as libc::c_uint) {
