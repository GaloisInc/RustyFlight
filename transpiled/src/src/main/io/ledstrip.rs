...
pub static mut ledStripConfig_Registry: pgRegistry_t =
    unsafe {
        {
            let mut init =
                pgRegistry_s{pgn: (27i32 | 0i32 << 12i32) as pgn_t,
                             size:
...
pub static mut ledStripConfig_Copy: ledStripConfig_t =
    ledStripConfig_t{ledConfigs: [0; 32],
                     ...
                     ioTag: 0,
                     ledstrip_grb_rgb: LED_GRB,};
static mut ledStripInitialised: bool = 0i32 != 0;
static mut ledStripEnabled: bool = 1i32 != 0;
#[no_mangle]
pub static mut hsv: [hsvColor_t; 14] =
    [{
         let mut init =
             hsvColor_s{h: 0i32 as uint16_t,
                        s: 0i32 as uint8_t,
                        v: 0i32 as uint8_t,};
         init
     },
     ...
     {
         let mut init =
             hsvColor_s{h: 30i32 as uint16_t,
                        s: 0i32 as uint8_t,
                        v: 255i32 as uint8_t,};
         init
     },
     {
         let mut init =
             hsvColor_s{h: 60i32 as uint16_t,
                        s: 0i32 as uint8_t,
                        v: 255i32 as uint8_t,};
         init
     },
     {
         let mut init =
             hsvColor_s{h: 90i32 as uint16_t,
                        s: 0i32 as uint8_t,
                        v: 255i32 as uint8_t,};
         init
     },
     {
         let mut init =
             hsvColor_s{h: 120i32 as uint16_t,
                        s: 0i32 as uint8_t,
                        v: 255i32 as uint8_t,};
         init
     },
     {
         let mut init =
             hsvColor_s{h: 150i32 as uint16_t,
                        s: 0i32 as uint8_t,
                        v: 255i32 as uint8_t,};
         init
     },
     {
         let mut init =
             hsvColor_s{h: 180i32 as uint16_t,
                        s: 0i32 as uint8_t,
                        v: 255i32 as uint8_t,};
         init
     },
     {
         let mut init =
             hsvColor_s{h: 210i32 as uint16_t,
                        s: 0i32 as uint8_t,
                        v: 255i32 as uint8_t,};
         init
     },
     {
         let mut init =
             hsvColor_s{h: 240i32 as uint16_t,
                        s: 0i32 as uint8_t,
                        v: 255i32 as uint8_t,};
         init
     },
     {
         let mut init =
             hsvColor_s{h: 270i32 as uint16_t,
                        s: 0i32 as uint8_t,
                        v: 255i32 as uint8_t,};
         init
     },
     {
         let mut init =
             hsvColor_s{h: 300i32 as uint16_t,
                        s: 0i32 as uint8_t,
                        v: 255i32 as uint8_t,};
         init
     },
     {
         let mut init =
             hsvColor_s{h: 330i32 as uint16_t,
                        s: 0i32 as uint8_t,
                        v: 255i32 as uint8_t,};
         init
     }];
...
pub unsafe extern "C" fn pgResetFn_ledStripConfig(mut ledStripConfig_0:
    ...
    memset((*ledStripConfig_0).colors.as_mut_ptr() as *mut libc::c_void, 0i32,
           (::core::mem::size_of::<[hsvColor_t; 14]>() as
                libc::c_ulong).wrapping_div(::core::mem::size_of::<hsvColor_t>()
                                                ...
                                                libc::c_ulong).wrapping_mul(::core::mem::size_of::<hsvColor_t>()
                                                                                as
                                                                                libc::c_ulong));
    let mut colorIndex: libc::c_uint = 0i32 as libc::c_uint;
    ...
    memcpy_fn(&mut (*ledStripConfig_0).specialColors as
              ...
              ::core::mem::size_of::<[specialColorIndexes_t; 1]>() as
                  libc::c_ulong);
    (*ledStripConfig_0).ledstrip_visual_beeper = 0i32 as uint8_t;
    (*ledStripConfig_0).ledstrip_aux_channel =
        THROTTLE as libc::c_int as uint8_t;
    (*ledStripConfig_0).ioTag =
        timerioTagGetByUsage(TIM_USE_LED, 0i32 as uint8_t);
}
static mut scaledThrottle: libc::c_int = 0;
static mut auxInput: libc::c_int = 0;
unsafe extern "C" fn updateDimensions() {
    let mut maxX: libc::c_int = 0i32;
    let mut minX: libc::c_int = 0xfi32;
    let mut maxY: libc::c_int = 0i32;
    let mut minY: libc::c_int = 0xfi32;
    let mut ledIndex: libc::c_int = 0i32;
    ...
    ledGridRows = (maxY - minY + 1i32) as uint8_t;
    if minX < maxX {
        lowestXValueForEast = ((minX + maxX) / 2i32 + 1i32) as int8_t;
        highestXValueForWest = ((minX + maxX - 1i32) / 2i32) as int8_t
    } else {
        lowestXValueForEast = (0xfi32 / 2i32) as int8_t;
        highestXValueForWest =
            (lowestXValueForEast as libc::c_int - 1i32) as int8_t
    }
    if minY < maxY {
        lowestYValueForSouth = ((minY + maxY) / 2i32 + 1i32) as int8_t;
        highestYValueForNorth = ((minY + maxY - 1i32) / 2i32) as int8_t
    } else {
        lowestYValueForSouth = (0xfi32 / 2i32) as int8_t;
        highestYValueForNorth =
            (lowestYValueForSouth as libc::c_int - 1i32) as int8_t
    };
}
unsafe extern "C" fn updateLedCount() {
    let mut count: libc::c_int = 0i32;
    let mut countRing: libc::c_int = 0i32;
    let mut countScanner: libc::c_int = 0i32;
    let mut ledIndex: libc::c_int = 0i32;
    while ledIndex < 32i32 {
        let mut ledConfig: *const ledConfig_t =
            &*(*(ledStripConfig as
                     unsafe extern "C" fn()
                         ->
...
pub unsafe extern "C" fn parseLedStripConfig(mut ledIndex: libc::c_int,
 ...
 -> bool {
    if ledIndex >= 32i32 { return 0i32 != 0 }
                     ...
                     unsafe extern "C" fn()
                         ->
                             *mut ledStripConfig_t)()).ledConfigs.as_mut_ptr().offset(ledIndex
                                                                                          ...
                                                                                          isize)
            as *mut ledConfig_t;
    memset(ledConfig as *mut libc::c_void, 0i32,
    ...
    let mut baseFunction: libc::c_int = 0i32;
    let mut overlay_flags: libc::c_int = 0i32;
    let mut direction_flags: libc::c_int = 0i32;
    ...
    while (parseState as libc::c_uint) <
        ...
        chunkIndex = chunkIndex + 1;
        chunk[fresh2 as usize] = 0i32 as libc::c_char;
        ...
        match parseState as libc::c_uint {
            ...
            2 => {
                ...
                while *ch != 0 {
                    ...
                    while (dir as libc::c_uint) < 6i32 as libc::c_uint {
                        if directionCodes[dir as usize] as libc::c_int ==
                               *ch as libc::c_int {
                            direction_flags |= 1i32 << dir as libc::c_uint;
                            break ;
                        } else { dir += 1 }
                    }
                    ch = ch.offset(1)
            ...
            3 => {
                ...
                while *ch_0 != 0 {
                    ...
                    while (fn_0 as libc::c_uint) < 7i32 as libc::c_uint {
                    ...
                    while (ol as libc::c_uint) < 6i32 as libc::c_uint {
                        if overlayCodes[ol as usize] as libc::c_int ==
                               *ch_0 as libc::c_int {
                            overlay_flags |= 1i32 << ol as libc::c_uint;
                            break ;
                        } else { ol += 1 }
                    }
                    ch_0 = ch_0.offset(1)
    ...
    *ledConfig =
        (((x & 0xfi32) << 4i32 | (y & 0xfi32) << 0i32) << 0i32 |
             color << 18i32 | direction_flags << 22i32 | baseFunction << 8i32
             | overlay_flags << 12i32 | 0i32 << 28i32) as ledConfig_t;
    reevaluateLedConfig();
    return 1i32 != 0;
}
#[no_mangle]
pub unsafe extern "C" fn generateLedConfig(mut ledConfig: *mut ledConfig_t,
    ...
    memset(ledConfigBuffer as *mut libc::c_void, 0i32, bufferSize);
...
static mut flightModeToLed: [C2RustUnnamed_2; 6] =
    [{
         let mut init =
     ...
     {
         let mut init =
             C2RustUnnamed_2{flightMode: MAG_MODE as libc::c_int as uint16_t,
                             ledMode:
                                 LED_MODE_MAG as libc::c_int as uint8_t,};
         init
     },
     {
         let mut init =
     ...
     {
         let mut init =
             C2RustUnnamed_2{flightMode: 0i32 as uint16_t,
                             ledMode:
...
unsafe extern "C" fn applyLedFixedLayers() {
    ...
    while ledIndex < ledCounts.count as libc::c_int {
        ...
        let mut hOffset: libc::c_int = 359i32 + 1i32;
        ...
        match fn_0 {
            0 => {
                ...
                if ledGetOverlayBit(ledConfig,
                    ...
                    let mut centerPWM: libc::c_int =
                        (1000i32 + 2000i32) / 2i32;
                    if auxInput < centerPWM {
                        color.h =
                            scaleRange(auxInput, 1000i32, centerPWM,
                        ...
                        color.h =
                            scaleRange(auxInput, centerPWM, 2000i32,
            ...
            1 => {
                ...
                while (i as libc::c_ulong) <
                          (::core::mem::size_of::<[C2RustUnnamed_2; 6]>() as
            ...
            3 => {
                ...
                hOffset +=
                    scaleRange(calculateBatteryPercentageRemaining() as
                                   libc::c_int, 0i32, 100i32, -30i32, 120i32)
            }
            4 => {
                color = hsv[COLOR_RED as libc::c_int as usize];
                hOffset +=
                    scaleRange(getRssiPercent() as libc::c_int, 0i32, 100i32,
                               -30i32, 120i32)
            }
            _ => { }
        }
        if fn_0 != LED_FUNCTION_COLOR as libc::c_int &&
               ledGetOverlayBit(ledConfig,
                   ...
                   libc::c_int != 0 {
            hOffset +=
                scaleRange(auxInput, 1000i32, 2000i32, 0i32, 359i32 + 1i32)
        }
        color.h =
            ((color.h as libc::c_int + hOffset) % (359i32 + 1i32)) as
                uint16_t;
...
unsafe extern "C" fn applyLedWarningLayer(mut updateNow: bool,
    ...
    static mut warningFlashCounter: uint8_t = 0i32 as uint8_t;
    static mut warningFlags: uint8_t = 0i32 as uint8_t;
    if updateNow {
        // keep counter running, so it stays in sync with blink
        warningFlashCounter = warningFlashCounter.wrapping_add(1);
        warningFlashCounter =
            (warningFlashCounter as libc::c_int & 0xfi32) as uint8_t;
        if warningFlashCounter as libc::c_int == 0i32 {
            // update when old flags was processed
            warningFlags = 0i32 as uint8_t; // w_w_
            if (*batteryConfig()).voltageMeterSource as libc::c_uint !=
                ...
                warningFlags =
                    (warningFlags as libc::c_int |
                         1i32 << WARNING_LOW_BATTERY as libc::c_int) as
                        uint8_t
            }
            if failsafeIsActive() {
                warningFlags =
                    (warningFlags as libc::c_int |
                         1i32 << WARNING_FAILSAFE as libc::c_int) as uint8_t
            }
            if armingFlags as libc::c_int & ARMED as libc::c_int == 0 &&
                   isArmingDisabled() as libc::c_int != 0 {
                warningFlags =
                    (warningFlags as libc::c_int |
                         1i32 << WARNING_ARMING_DISABLED as libc::c_int) as
                        uint8_t
            }
        }
        *timer =
            (*timer as
                 libc::c_uint).wrapping_add((1000i32 * 1000i32 / 10i32) as
    ...
    if warningFlags != 0 {
        let mut colorOn: bool =
            warningFlashCounter as libc::c_int % 2i32 == 0i32;
        let mut warningId: warningFlags_e =
            (warningFlashCounter as libc::c_int / 4i32) as warningFlags_e;
        if warningFlags as libc::c_int & 1i32 << warningId as libc::c_uint !=
               0 {
    ...
    if !warningColor.is_null() {
        applyLedHsv(((1i32 << LED_OVERLAY_WARNING as libc::c_int) << 12i32) as
                        uint32_t, warningColor);
    };
}
unsafe extern "C" fn applyLedVtxLayer(mut updateNow: bool,
                                      mut timer: *mut timeUs_t) {
    static mut frequency: uint16_t = 0i32 as uint16_t;
    static mut power: uint8_t = 255i32 as uint8_t;
    static mut pit: uint8_t = 255i32 as uint8_t;
    static mut showSettings: uint8_t = 0i32 as uint8_t;
    static mut lastCheck: uint16_t = 0i32 as uint16_t;
    static mut blink: bool = 0i32 != 0;
    let mut vtxDevice: *const vtxDevice_t = vtxCommonDevice();
    if vtxDevice.is_null() { return }
    let mut band: uint8_t = 255i32 as uint8_t;
    let mut channel: uint8_t = 255i32 as uint8_t;
    let mut check: uint16_t = 0i32 as uint16_t;
    if updateNow {
        ...
        frequency =
            vtx58frequencyTable[(band as libc::c_int - 1i32) as
                                    usize][(channel as libc::c_int - 1i32) as
        ...
        check =
            (pit as libc::c_int + ((power as libc::c_int) << 1i32) +
                 ((band as libc::c_int) << 4i32) +
                 ((channel as libc::c_int) << 8i32)) as uint16_t;
        if showSettings == 0 &&
               check as libc::c_int != lastCheck as libc::c_int {
            // check if last vtx values have changed.
            // display settings for 3 seconds.
            showSettings = 15i32 as uint8_t
        ...
        *timer =
            (*timer as
                 libc::c_uint).wrapping_add((1000i32 * 1000i32 / 5i32) as
                                                libc::c_uint) as timeUs_t as
                timeUs_t
    }
    ...
    if showSettings != 0 {
        // show settings
        let mut vtxLedCount: uint8_t = 0i32 as uint8_t;
        let mut i: libc::c_int = 0i32;
        while i < ledCounts.count as libc::c_int &&
                  (vtxLedCount as libc::c_int) < 6i32 {
            let mut ledConfig: *const ledConfig_t =
                &*(*(ledStripConfig as
                         unsafe extern "C" fn()
                             ->
        ...
        let mut colorIndex: libc::c_int = 0i32; // blink when in pit mode
        if frequency as libc::c_int <= 5672i32 {
            colorIndex = COLOR_WHITE as libc::c_int
        } else if frequency as libc::c_int <= 5711i32 {
            colorIndex = COLOR_RED as libc::c_int
        } else if frequency as libc::c_int <= 5750i32 {
            colorIndex = COLOR_ORANGE as libc::c_int
        } else if frequency as libc::c_int <= 5789i32 {
            colorIndex = COLOR_YELLOW as libc::c_int
        } else if frequency as libc::c_int <= 5829i32 {
            colorIndex = COLOR_GREEN as libc::c_int
        } else if frequency as libc::c_int <= 5867i32 {
            colorIndex = COLOR_BLUE as libc::c_int
        } else if frequency as libc::c_int <= 5906i32 {
        ...
        color_0.v =
            if pit as libc::c_int != 0 {
                if blink as libc::c_int != 0 { 15i32 } else { 0i32 }
            } else { 255i32 } as uint8_t;
        applyLedHsv(((1i32 << LED_OVERLAY_VTX as libc::c_int) << 12i32) as
                        uint32_t, &mut color_0);
    };
}
unsafe extern "C" fn applyLedBatteryLayer(mut updateNow: bool,
                                          mut timer: *mut timeUs_t) {
    static mut flash: bool = 0i32 != 0;
    let mut timerDelayUs: libc::c_int = 1000i32 * 1000i32 / 1i32;
    if updateNow {
        match getBatteryState() as libc::c_uint {
            0 => {
                flash = 1i32 != 0;
                timerDelayUs = 1000i32 * 1000i32 / 1i32
            }
            1 => { flash = !flash; timerDelayUs = 1000i32 * 1000i32 / 2i32 }
            _ => { flash = !flash; timerDelayUs = 1000i32 * 1000i32 / 8i32 }
        }
    }
    *timer =
    ...
    if !flash {
        ...
        applyLedHsv(((LED_FUNCTION_BATTERY as libc::c_int) << 8i32) as
                        uint32_t, bgc);
    };
}
unsafe extern "C" fn applyLedRssiLayer(mut updateNow: bool,
                                       mut timer: *mut timeUs_t) {
    static mut flash: bool = 0i32 != 0;
    let mut timerDelay: libc::c_int = 1000i32 * 1000i32 / 1i32;
    if updateNow {
        let mut state: libc::c_int = getRssiPercent() as libc::c_int;
        if state > 50i32 {
            flash = 1i32 != 0;
            timerDelay = 1000i32 * 1000i32 / 1i32
        } else if state > 20i32 {
            flash = !flash;
            timerDelay = 1000i32 * 1000i32 / 2i32
        } else { flash = !flash; timerDelay = 1000i32 * 1000i32 / 8i32 }
    }
    *timer =
    ...
    if !flash {
        let mut bgc: *const hsvColor_t = getSC(LED_SCOLOR_BACKGROUND);
        applyLedHsv(((LED_FUNCTION_RSSI as libc::c_int) << 8i32) as uint32_t,
                    bgc);
    };
}
unsafe extern "C" fn applyLedIndicatorLayer(mut updateNow: bool,
    ...
    if updateNow {
        if rxIsReceivingSignal() {
            ...
            *timer =
                (*timer as
                     libc::c_uint).wrapping_add((1000i32 * 1000i32 /
                                                     (5i32 +
                                                          45i32 * scale /
                                                              (500i32 -
                                                                   25i32))) as
    ...
    if rcCommand[ROLL as libc::c_int as usize] > 25i32 as libc::c_float {
        quadrants =
    ...
    if rcCommand[PITCH as libc::c_int as usize] > 25i32 as libc::c_float {
        quadrants =
            ::core::mem::transmute::<libc::c_uint,
                                     quadrant_e>(quadrants as libc::c_uint |
                                                     QUADRANT_NORTH as
...
unsafe extern "C" fn updateLedRingCounts() {
    ...
    if ledCounts.ring as libc::c_int % 6i32 == 0i32 {
        seqLen = 6i32
    } else {
        ...
        while seqLen > 6i32 && seqLen % 2i32 == 0i32 { seqLen /= 2i32 }
    }
...
unsafe extern "C" fn applyLedThrustRingLayer(mut updateNow: bool,
    ...
    let mut ledRingIndex: libc::c_int = 0i32;
    if updateNow {
        rotationPhase =
            if rotationPhase as libc::c_int > 0i32 {
                (rotationPhase as libc::c_int) - 1i32
            } else { (ledCounts.ringSeqLen as libc::c_int) - 1i32 } as
                uint8_t;
        *timer =
            (*timer as
                 libc::c_uint).wrapping_add((1000i32 * 1000i32 /
                                                 (5i32 +
                                                      45i32 * scaledThrottle /
                                                          100i32)) as
                                                libc::c_uint) as timeUs_t as
                timeUs_t
        // 5 - 50Hz update rate
            ...
            if armingFlags as libc::c_int & ARMED as libc::c_int != 0 {
                applyColor =
                    ((ledRingIndex + rotationPhase as libc::c_int) %
                         ledCounts.ringSeqLen as libc::c_int) < 2i32
            } else {
                applyColor = ledRingIndex % 2i32 == 0
                // alternating pattern
            }
            if applyColor {
...
unsafe extern "C" fn brightnessForLarsonIndex(mut larsonParameters:
 ...
 -> libc::c_int {
    ...
    static mut larsonLowValue: libc::c_int = 8i32;
    if ({
    ...
    if (*larsonParameters).direction as libc::c_int == offset {
        return (*larsonParameters).currentBrightness as libc::c_int - 127i32
    }
    return 255i32 - (*larsonParameters).currentBrightness as libc::c_int;
}
unsafe extern "C" fn larsonScannerNextStep(mut larsonParameters:
                                               *mut larsonParameters_t,
                                           mut delta: libc::c_int) {
    if (*larsonParameters).currentBrightness as libc::c_int > 255i32 - delta {
        (*larsonParameters).currentBrightness = 127i32 as uint8_t;
        if (*larsonParameters).currentIndex as libc::c_int >=
               ledCounts.larson as libc::c_int ||
               ((*larsonParameters).currentIndex as libc::c_int) < 0i32 {
...
unsafe extern "C" fn applyLarsonScannerLayer(mut updateNow: bool,
    ...
    static mut larsonParameters: larsonParameters_t =
        {
            let mut init =
                larsonParameters_s{currentBrightness: 0i32 as uint8_t,
                                   currentIndex: 0i32 as int8_t,
                                   direction: 1i32 as int8_t,};
            init
        };
    if updateNow {
        larsonScannerNextStep(&mut larsonParameters, 15i32);
        *timer =
            (*timer as
                 libc::c_uint).wrapping_add((1000i32 * 1000i32 / 60i32) as
                                                libc::c_uint) as timeUs_t as
                timeUs_t
    }
    let mut scannerLedIndex: libc::c_int = 0i32;
    let mut i: libc::c_uint = 0i32 as libc::c_uint;
...
unsafe extern "C" fn applyLedBlinkLayer(mut updateNow: bool,
                                        mut timer: *mut timeUs_t) {
    let blinkPattern: uint16_t = 0x8005i32 as uint16_t; // 0b1000000000000101;
    static mut blinkMask: uint16_t = 0; // b_b_____...
    if updateNow {
        blinkMask = (blinkMask as libc::c_int >> 1i32) as uint16_t;
        if blinkMask as libc::c_int <= 1i32 { blinkMask = blinkPattern }
        *timer =
            (*timer as
                 libc::c_uint).wrapping_add((1000i32 * 1000i32 / 10i32) as
                                                libc::c_uint) as timeUs_t as
    ...
    let mut ledOn: bool = blinkMask as libc::c_int & 1i32 != 0;
    if !ledOn {
        let mut i: libc::c_int = 0i32;
        while i < ledCounts.count as libc::c_int {
            let mut ledConfig: *const ledConfig_t =
...
static mut layerTable:
       [Option<unsafe extern "C" fn(_: bool, _: *mut timeUs_t) -> ()>; 8] =
...
pub unsafe extern "C" fn updateRequiredOverlay() {
    disabledTimerMask = 0i32 as uint16_t;
    disabledTimerMask =
...
pub unsafe extern "C" fn ledStripUpdate(mut currentTimeUs: timeUs_t) {
    ...
    if IS_RC_MODE_ACTIVE(BOXLEDLOW) as libc::c_int != 0 &&
        ...
        if ledStripEnabled { ledStripDisable(); ledStripEnabled = 0i32 != 0 }
        return
    }
    ledStripEnabled = 1i32 != 0;
    let now: uint32_t = currentTimeUs;
    // test all led timers, setting corresponding bits
    let mut timActive: uint32_t = 0i32 as uint32_t;
    let mut timId: timId_e = timBlink;
    while (timId as libc::c_uint) <
              timTimerCount as libc::c_int as libc::c_uint {
        if disabledTimerMask as libc::c_int & 1i32 << timId as libc::c_uint ==
               0 {
            // sanitize timer value, so that it can be safely incremented. Handles inital timerVal value.
            let delta: timeDelta_t = cmpTimeUs(now, timerVal[timId as usize]);
            // max delay is limited to 5s
            if !(delta < 0i32 && delta > -(5i32 * 1000i32 * 1000i32)) {
                timActive |=
                    (1i32 << timId as libc::c_uint) as
                        libc::c_uint; // not ready yet
                if delta >= 100i32 * 1000i32 || delta < 0i32 {
    ...
    scaledThrottle =
        if armingFlags as libc::c_int & ARMED as libc::c_int != 0 {
            scaleRange(rcData[THROTTLE as libc::c_int as usize] as
                           libc::c_int, 1000i32, 2000i32, 0i32, 100i32)
        } else { 0i32 };
    auxInput =
    ...
    while (timId_0 as libc::c_ulong) <
              (::core::mem::size_of::<[Option<unsafe extern "C" fn(_: bool,
                                                                   _:
                                                                       *mut timeUs_t)
                                                  -> ()>; 8]>() as
                   libc::c_ulong).wrapping_div(::core::mem::size_of::<Option<unsafe extern "C" fn(_:
                                                                                                      bool,
                                                                                                  _:
                                                                                                      *mut timeUs_t)
                                                                                 ->
        ...
        let mut updateNow: bool =
            timActive & (1i32 << timId_0 as libc::c_uint) as libc::c_uint !=
                0;
...
pub unsafe extern "C" fn parseColor(mut index: libc::c_int,
 ...
 -> bool {
    ...
    static mut hsv_limit: [uint16_t; 3] =
        [359i32 as uint16_t, 255i32 as uint16_t, 255i32 as uint16_t];
    let mut componentIndex: libc::c_int = 0i32;
    while result as libc::c_int != 0 &&
              componentIndex < HSV_VALUE as libc::c_int + 1i32 {
        ...
        if val > hsv_limit[componentIndex as usize] as libc::c_int {
            ...
            if !remainingCharacters.is_null() {
                ...
                // skip separator
            } else if componentIndex < HSV_VALUE as libc::c_int + 1i32 - 1i32
             {
                result = 0i32 != 0
            }
            componentIndex += 1
    ...
    if !result {
        memset(color as *mut libc::c_void, 0i32,
...
pub unsafe extern "C" fn setModeColor(mut modeIndex: ledModeIndex_e,
    ...
    // check color
    if colorIndex < 0i32 || colorIndex >= 16i32 { return 0i32 != 0 }
    if (modeIndex as libc::c_uint) < 6i32 as libc::c_uint {
        // modeIndex_e is unsigned, so one-sided test is enough
        if modeColorIndex < 0i32 || modeColorIndex >= 6i32 {
            return 0i32 != 0
        }
        ...
        if modeColorIndex < 0i32 || modeColorIndex >= 11i32 {
            return 0i32 != 0
        }
        ...
        if modeColorIndex < 0i32 || modeColorIndex >= 1i32 {
            return 0i32 != 0
        }
...
pub unsafe extern "C" fn ledStripInit() {
    ...
    ledStripInitialised = 0i32 != 0;
}
#[no_mangle]
pub unsafe extern "C" fn ledStripEnable() {
    reevaluateLedConfig();
    ledStripInitialised = 1i32 != 0;
