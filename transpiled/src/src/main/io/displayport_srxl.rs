...
unsafe extern "C" fn srxlClearScreen(mut displayPort: *mut displayPort_t)
 -> libc::c_int {
    ...
    srxlWriteString(displayPort, 1i32 as uint8_t, 0i32 as uint8_t,
                    b"BETAFLIGHT\x00" as *const u8 as *const libc::c_char);
    if (*displayPort).grabCount as libc::c_int == 0i32 {
        srxlWriteString(displayPort, 0i32 as uint8_t, 2i32 as uint8_t,
                        b"MENU:THR MID\x00" as *const u8 as
                            *const libc::c_char);
        srxlWriteString(displayPort, 2i32 as uint8_t, 3i32 as uint8_t,
                        b"+ YAW LEFT\x00" as *const u8 as
                            *const libc::c_char);
        srxlWriteString(displayPort, 2i32 as uint8_t, 4i32 as uint8_t,
...
unsafe extern "C" fn srxlGrab(mut displayPort: *mut displayPort_t)
 -> libc::c_int {
    (*displayPort).grabCount = 1i32 as int8_t;
...
pub unsafe extern "C" fn displayPortSrxlInit() -> *mut displayPort_t {
    ...
    srxlDisplayPort.rows = 9i32 as uint8_t;
    srxlDisplayPort.cols = 12i32 as uint8_t;
