...
pub static mut vtxConfig_Registry: pgRegistry_t =
    unsafe {
        {
            let mut init =
                pgRegistry_s{pgn: (515i32 | 1i32 << 12i32) as pgn_t,
                             size:
...
static mut locked: uint8_t = 0i32 as uint8_t;
#[no_mangle]
...
pub unsafe extern "C" fn vtxIncrementBand() {
    vtxUpdateBandAndChannel(1i32 as uint8_t, 0i32 as uint8_t);
}
#[no_mangle]
...
pub unsafe extern "C" fn vtxIncrementChannel() {
    vtxUpdateBandAndChannel(0i32 as uint8_t, 1i32 as uint8_t);
}
#[no_mangle]
...
pub unsafe extern "C" fn vtxUpdateActivatedChannel() {
    ...
    if locked == 0 && !vtxCommonDevice().is_null() {
        static mut lastIndex: uint8_t = -1i32 as uint8_t;
        let mut index: uint8_t = 0i32 as uint8_t;
        while (index as libc::c_int) < 10i32 {
...
pub unsafe extern "C" fn vtxCycleBandOrChannel(bandStep: uint8_t,
    ...
    if !vtxDevice.is_null() {
        let mut band: uint8_t = 0i32 as uint8_t;
        let mut channel: uint8_t = 0i32 as uint8_t;
...
pub unsafe extern "C" fn vtxCyclePower(powerStep: uint8_t) {
    let mut vtxDevice: *const vtxDevice_t = vtxCommonDevice();
    if !vtxDevice.is_null() {
        let mut power: uint8_t = 0i32 as uint8_t;
        let mut capability: vtxDeviceCapability_t =
            vtxDeviceCapability_t{bandCount: 0,
                                  channelCount: 0,
                                  powerCount: 0,
