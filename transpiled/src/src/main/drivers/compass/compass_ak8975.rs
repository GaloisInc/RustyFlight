...
unsafe extern "C" fn constrain(mut amt: libc::c_int, mut low: libc::c_int,
    ...
    if amt < low {
        ...
        return high
    } else { return amt }; // power down before entering fuse mode
}
unsafe extern "C" fn ak8975Init(mut mag: *mut magDev_t) -> bool {
    let mut asa: [uint8_t; 3] = [0; 3]; // Enter Fuse ROM access mode
    let mut status: uint8_t = 0; // Read the x-, y-, and z-axis asa values
    let mut busdev: *mut busDevice_t =
        &mut (*mag).busdev; // power down after reading.
    busWriteRegister(busdev, 0xai32 as uint8_t, 0i32 as uint8_t);
    delay(20i32 as timeMs_t);
    busWriteRegister(busdev, 0xai32 as uint8_t, 0xfi32 as uint8_t);
    delay(10i32 as timeMs_t);
    busReadRegisterBuffer(busdev, 0x10i32 as uint8_t, asa.as_mut_ptr(),
                          ::core::mem::size_of::<[uint8_t; 3]>() as
                              libc::c_ulong as uint8_t);
    delay(10i32 as timeMs_t);
    (*mag).magGain[X as libc::c_int as usize] =
        (asa[X as libc::c_int as usize] as libc::c_int + 128i32) as int16_t;
    (*mag).magGain[Y as libc::c_int as usize] =
        (asa[Y as libc::c_int as usize] as libc::c_int + 128i32) as int16_t;
    (*mag).magGain[Z as libc::c_int as usize] =
        (asa[Z as libc::c_int as usize] as libc::c_int + 128i32) as int16_t;
    busWriteRegister(busdev, 0xai32 as uint8_t, 0i32 as uint8_t);
    delay(10i32 as timeMs_t);
    // Clear status registers
    busReadRegisterBuffer(busdev, 0x2i32 as uint8_t, &mut status,
                          1i32 as uint8_t);
    busReadRegisterBuffer(busdev, 0x9i32 as uint8_t, &mut status,
                          1i32 as uint8_t);
    // Trigger first measurement
    busWriteRegister(busdev, 0xai32 as uint8_t,
                     (0x10i32 | 0x1i32) as
                         uint8_t); // read from AK8975_MAG_REG_HXL to AK8975_MAG_REG_HZH
    return 1i32 != 0; // start reading again    uint8_t status2 = buf[6];
}
unsafe extern "C" fn parseMag(mut raw: *mut uint8_t, mut gain: int16_t)
 -> int16_t {
    let mut ret: libc::c_int =
        ((*raw.offset(1) as libc::c_int) << 8i32 |
             *raw.offset(0) as libc::c_int) as int16_t as libc::c_int *
            gain as libc::c_int / 256i32;
    return constrain(ret, -32767i32 - 1i32, 32767i32) as int16_t;
}
unsafe extern "C" fn ak8975Read(mut mag: *mut magDev_t,
    ...
    ack =
        busReadRegisterBuffer(busdev, 0x2i32 as uint8_t, &mut status,
                              1i32 as uint8_t);
    if !ack || status as libc::c_int & 0x1i32 == 0i32 { return 0i32 != 0 }
...
pub unsafe extern "C" fn ak8975Detect(mut mag: *mut magDev_t) -> bool {
    ...
    if (*busdev).bustype as libc::c_uint ==
        ...
        (*busdev).busdev_u.i2c.address = 0xci32 as uint8_t
    }
    let mut ack: bool =
        busReadRegisterBuffer(busdev, 0i32 as uint8_t, &mut sig,
                              1i32 as uint8_t);
    if !ack || sig as libc::c_int != 0x48i32 {
        // 0x48 / 01001000 / 'H'
        return 0i32 != 0
    }
    (*mag).init =
