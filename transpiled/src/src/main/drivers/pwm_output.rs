 ...
 */
static mut pwmWrite:
       Option<unsafe extern "C" fn(_: uint8_t, _: libc::c_float) -> ()> =
    None;
...
static mut pwmCompleteWrite: Option<unsafe extern "C" fn(_: uint8_t) -> ()> =
    None;
#[no_mangle]
pub static mut loadDmaBuffer:
           Option<unsafe extern "C" fn(_: *mut uint32_t, _: libc::c_int,
                                       _: uint16_t) -> uint8_t> =
    None;
...
static mut freqBeep: uint16_t = 0i32 as uint16_t;
static mut pwmMotorsEnabled: bool = 0i32 != 0;
static mut isDshot: bool = 0i32 != 0;
#[no_mangle]
pub static mut useBurstDshot: bool = 0i32 != 0;
unsafe extern "C" fn pwmOCConfig(mut tim: *mut TIM_TypeDef,
    ...
    timerOCPreloadConfig(tim, channel, 0x8i32 as uint16_t);
}
#[no_mangle]
pub unsafe extern "C" fn pwmOutConfig(mut channel: *mut timerChannel_t,
    ...
    ::core::ptr::write_volatile((*channel).ccr, 0i32 as timCCR_t);
}
...
unsafe extern "C" fn loadDmaBufferDshot(mut dmaBuffer: *mut uint32_t,
    ...
    while i < 16i32 {
        *dmaBuffer.offset((i * stride) as isize) =
            if packet as libc::c_int & 0x8000i32 != 0 { 14i32 } else { 7i32 }
                as uint32_t;
        packet = ((packet as libc::c_int) << 1i32) as uint16_t;
        i += 1
    }
    return 18i32 as uint8_t;
}
unsafe extern "C" fn loadDmaBufferProshot(mut dmaBuffer: *mut uint32_t,
    ...
    while i < 4i32 {
        *dmaBuffer.offset((i * stride) as isize) =
            (24i32 + ((packet as libc::c_int & 0xf000i32) >> 12i32) * 3i32) as
                uint32_t;
        packet = ((packet as libc::c_int) << 4i32) as uint16_t;
        i += 1
        // Shift 4 bits
    }
    return 6i32 as uint8_t;
}
#[no_mangle]
...
pub unsafe extern "C" fn pwmDisableMotors() {
    pwmShutdownPulsesForAllMotors(8i32 as uint8_t);
        ...
        8 | 7 | 6 | 5 => {
            ...
            pwmCompleteWrite =
                Some(pwmCompleteDshotMotorUpdate as
                         unsafe extern "C" fn(_: uint8_t) -> ());
            isDshot = 1i32 != 0;
            if (*motorConfig).useBurstDshot != 0 { useBurstDshot = 1i32 != 0 }
        }
        ...
        // margin of safety is 4 periods when unsynced
            let pwmRateHz: libc::c_uint =
                if useUnsyncedPwm as libc::c_int != 0 {
                    ...
                    ceilf(1i32 as libc::c_float /
                              ((sMin + sLen) * 4i32 as libc::c_float))
                } as libc::c_uint;
            let clock: uint32_t = timerClock((*timerHardware).tim);
            /* used to find the desired timer frequency for max resolution */
            let prescaler: libc::c_uint =
                clock.wrapping_div(pwmRateHz).wrapping_add(0xffffi32 as
                                                               libc::c_uint).wrapping_div(0x10000i32
                                                                                              as
...
pub unsafe extern "C" fn getDshotHz(mut pwmProtocolType:
    ...
    match pwmProtocolType as libc::c_uint {
        9 => { return (24i32 * 1000000i32) as uint32_t }
        8 => { return (24i32 * 1000000i32) as uint32_t }
        7 => { return (12i32 * 1000000i32) as uint32_t }
        6 => { return (6i32 * 1000000i32) as uint32_t }
        5 | _ => { return (3i32 * 1000000i32) as uint32_t }
    };
}
#[no_mangle]
pub unsafe extern "C" fn allMotorsAreIdle(mut motorCount: uint8_t) -> bool {
    let mut allMotorsIdle: bool = 1i32 != 0;
    let mut i: libc::c_uint = 0i32 as libc::c_uint;
    while i < motorCount as libc::c_uint {
        let mut motor: *const motorDmaOutput_t =
            getMotorDmaOutput(i as uint8_t);
        if (*motor).value != 0 { allMotorsIdle = 0i32 != 0 }
        i = i.wrapping_add(1)
    }
...
pub unsafe extern "C" fn pwmDshotCommandIsProcessing() -> bool {
    return dshotCommandControl.nextCommandAtUs != 0 &&
               ...
               dshotCommandControl.repeats as libc::c_int > 0i32;
}
#[no_mangle]
pub unsafe extern "C" fn pwmWriteDshotCommand(mut index: uint8_t,
    ...
    if !isMotorProtocolDshot() || command as libc::c_int > 47i32 ||
           pwmDshotCommandIsQueued() as libc::c_int != 0 {
        return
    }
    let mut repeats: uint8_t = 1i32 as uint8_t;
    let mut delayAfterCommandUs: timeUs_t = 1000i32 as timeUs_t;
    match command as libc::c_int {
        7 | 8 | 9 | 10 | 12 | 20 | 21 => { repeats = 10i32 as uint8_t }
        1 | 2 | 3 | 4 | 5 => { delayAfterCommandUs = 100000i32 as timeUs_t }
        _ => { }
    }
    if blocking {
        delayMicroseconds((10000i32 - 1000i32) as timeUs_t);
        while repeats != 0 {
            delayMicroseconds(1000i32 as timeUs_t);
            let mut i: uint8_t = 0i32 as uint8_t;
            while (i as libc::c_int) < motorCount as libc::c_int {
                if i as libc::c_int == index as libc::c_int ||
                       index as libc::c_int == 255i32 {
            ...
            pwmCompleteDshotMotorUpdate(0i32 as uint8_t);
        ...
        dshotCommandControl.nextCommandAtUs =
            timeNowUs.wrapping_add(10000i32 as libc::c_uint);
        ...
        while i_0 < motorCount as libc::c_uint {
            if index as libc::c_uint == i_0 || index as libc::c_int == 255i32
               {
...
pub unsafe extern "C" fn pwmDshotCommandOutputIsEnabled(mut motorCount:
    ...
    if dshotCommandControl.repeats as libc::c_int > 0i32 {
        ...
        if dshotCommandControl.repeats as libc::c_int > 0i32 {
            dshotCommandControl.nextCommandAtUs =
                timeNowUs.wrapping_add(1000i32 as libc::c_uint)
        } else {
            dshotCommandControl.nextCommandAtUs =
                timeNowUs.wrapping_add(dshotCommandControl.delayAfterCommandUs)
        }
    } else { dshotCommandControl.nextCommandAtUs = 0i32 as timeUs_t }
    return 1i32 != 0;
}
#[no_mangle]
pub unsafe extern "C" fn prepareDshotPacket(motor: *mut motorDmaOutput_t)
 -> uint16_t {
    let mut packet: uint16_t =
        (((*motor).value as libc::c_int) << 1i32 |
    ...
    ::core::ptr::write_volatile(&mut (*motor).requestTelemetry as *mut bool,
                                0i32 != 0);
    // compute checksum
    let mut csum: libc::c_int = 0i32; // xor data by nibbles
    let mut csum_data: libc::c_int = packet as libc::c_int;
    let mut i: libc::c_int = 0i32;
    while i < 3i32 { csum ^= csum_data; csum_data >>= 4i32; i += 1 }
    csum &= 0xfi32;
    // append checksum
    packet = ((packet as libc::c_int) << 4i32 | csum) as uint16_t;
    return packet;
}
#[no_mangle]
pub unsafe extern "C" fn pwmWriteServo(mut index: uint8_t,
                                       mut value: libc::c_float) {
    if (index as libc::c_int) < 8i32 &&
...
pub unsafe extern "C" fn servoDevInit(mut servoConfig:
    ...
    while (servoIndex as libc::c_int) < 8i32 {
        ...
        IOInit(servos[servoIndex as usize].io, OWNER_SERVO,
               (servoIndex as libc::c_int + 1i32) as uint8_t);
        ...
        pwmOutConfig(&mut (*servos.as_mut_ptr().offset(servoIndex as
                                                           isize)).channel,
                     timer, (1i32 * 1000000i32) as uint32_t,
                     (1i32 * 1000000i32 /
...
pub unsafe extern "C" fn pwmWriteBeeper(mut onoffBeep: bool) {
    ...
    if onoffBeep as libc::c_int == 1i32 {
        ::core::ptr::write_volatile(beeperPwm.channel.ccr,
                                    (1i32 * 1000000i32 /
                                         freqBeep as libc::c_int / 2i32) as
                                        timCCR_t);
        beeperPwm.enabled = 1i32 != 0
    } else {
        ::core::ptr::write_volatile(beeperPwm.channel.ccr, 0i32 as timCCR_t);
        beeperPwm.enabled = 0i32 != 0
    };
}
#[no_mangle]
...
pub unsafe extern "C" fn beeperPwmInit(tag: ioTag_t,
    ...
    if !beeperIO.is_null() && !timer.is_null() {
        ...
        IOInit(beeperPwm.io, OWNER_BEEPER, (0i32 + 1i32) as uint8_t);
        ...
        pwmOutConfig(&mut beeperPwm.channel, timer,
                     ...
                     (1i32 * 1000000i32 / freqBeep as libc::c_int) as
                         uint16_t,
                     (1i32 * 1000000i32 / freqBeep as libc::c_int / 2i32) as
                         uint16_t, 0i32 as uint8_t);
        ::core::ptr::write_volatile(beeperPwm.channel.ccr, 0i32 as timCCR_t);
        beeperPwm.enabled = 0i32 != 0
    };
}
