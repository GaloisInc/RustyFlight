...
static mut ppmFrameCount: uint8_t = 0i32 as uint8_t;
static mut lastPPMFrameCount: uint8_t = 0i32 as uint8_t;
static mut ppmCountDivisor: uint8_t = 1i32 as uint8_t;
...
static mut ppmEventIndex: uint8_t = 0i32 as uint8_t;
#[no_mangle]
pub unsafe extern "C" fn ppmISREvent(mut source: eventSource_e,
                                     mut capture: uint32_t) {
    ppmEventIndex =
        ((ppmEventIndex as libc::c_int + 1i32) as
...
unsafe extern "C" fn ppmResetDevice() {
    ppmDev.pulseIndex = 0i32 as uint8_t;
    ppmDev.currentCapture = 0i32 as uint32_t;
    ppmDev.currentTime = 0i32 as uint32_t;
    ppmDev.deltaTime = 0i32 as uint32_t;
    ppmDev.largeCounter = 0i32 as uint32_t;
    ppmDev.numChannels = -1i32 as int8_t;
    ppmDev.numChannelsPrevFrame = -1i32 as int8_t;
    ppmDev.stableFramesSeenCount = 0i32 as uint8_t;
    ppmDev.tracking = 0i32 != 0;
    ppmDev.overflowed = 0i32 != 0;
}
unsafe extern "C" fn ppmOverflowCallback(mut cbRec: *mut timerOvrHandlerRec_t,
    ...
    ppmDev.largeCounter =
        (ppmDev.largeCounter as
             libc::c_uint).wrapping_add((capture as libc::c_int + 1i32) as
                                            libc::c_uint) as uint32_t as
            uint32_t;
    if capture as libc::c_int == 0x10000i32 - 1i32 {
        ppmDev.overflowed = 1i32 != 0
    };
}
unsafe extern "C" fn ppmEdgeCallback(mut cbRec: *mut timerCCHandlerRec_t,
    ...
    if currentTime > previousTime {
        ...
        ppmDev.deltaTime =
            ((0x10000i32 / ppmCountDivisor as libc::c_int) as
    ...
    if ppmDev.deltaTime > 2700i32 as libc::c_uint {
        if ppmDev.pulseIndex as libc::c_int ==
               ppmDev.numChannelsPrevFrame as libc::c_int &&
               ppmDev.pulseIndex as libc::c_int >= 4i32 &&
               ppmDev.pulseIndex as libc::c_int <= 12i32 {
            /* If we see n simultaneous frames of the same
               number of channels we save it as our frame size */
            if (ppmDev.stableFramesSeenCount as libc::c_int) < 25i32 {
        ...
        } else { ppmDev.stableFramesSeenCount = 0i32 as uint8_t }
        ...
        if ppmDev.pulseIndex as libc::c_int ==
            ...
            while i < 12i32 {
                captures[i as usize] = 0i32 as uint16_t;
                i += 1
            }
            ppmFrameCount = ppmFrameCount.wrapping_add(1)
        }
        ppmDev.tracking = 1i32 != 0;
        ppmDev.numChannelsPrevFrame = ppmDev.pulseIndex as int8_t;
        ppmDev.pulseIndex = 0i32 as uint8_t
    } else if ppmDev.tracking {
        /* Valid pulse duration 0.75 to 2.5 ms*/
        if ppmDev.deltaTime > 750i32 as libc::c_uint &&
               ppmDev.deltaTime < 2250i32 as libc::c_uint &&
               (ppmDev.pulseIndex as libc::c_int) < 12i32 {
            ...
            i = 0i32;
            while i < 12i32 {
                ppmDev.captures[i as usize] = 0i32 as uint32_t;
                i += 1
            }
        }
    };
...
pub unsafe extern "C" fn isPWMDataBeingReceived() -> bool {
    ...
    channel = 0i32;
    while channel < 12i32 {
        if captures[channel as usize] as libc::c_int != 0i32 {
            return 1i32 != 0
        }
        channel += 1
    }
...
unsafe extern "C" fn pwmOverflowCallback(mut cbRec: *mut timerOvrHandlerRec_t,
    ...
    let mut pwmInputPort: *mut pwmInputPort_t =
        ({
             ...
             (__mptr as *mut libc::c_char).offset(-32) as *mut pwmInputPort_t
         });
    (*pwmInputPort).missedEvents =
        (*pwmInputPort).missedEvents.wrapping_add(1);
    if (*pwmInputPort).missedEvents as libc::c_int > 10i32 {
        captures[(*pwmInputPort).channel as usize] = 0i32 as uint16_t;
        (*pwmInputPort).missedEvents = 0i32 as uint8_t
    };
}
unsafe extern "C" fn pwmEdgeCallback(mut cbRec: *mut timerCCHandlerRec_t,
    ...
    let mut pwmInputPort: *mut pwmInputPort_t =
        ({
             ...
             (__mptr as *mut libc::c_char).offset(-24) as *mut pwmInputPort_t
         });
    let mut timerHardwarePtr: *const timerHardware_t =
        (*pwmInputPort).timerHardware;
    if (*pwmInputPort).state as libc::c_int == 0i32 {
        (*pwmInputPort).rise = capture;
        (*pwmInputPort).state = 1i32 as uint8_t;
        ...
        // switch state
        (*pwmInputPort).state = 0i32 as uint8_t;
        pwmICConfig((*timerHardwarePtr).tim, (*timerHardwarePtr).channel,
                    0i32 as uint16_t);
        (*pwmInputPort).missedEvents = 0i32 as uint8_t
    };
...
pub unsafe extern "C" fn pwmICConfig(mut tim: *mut TIM_TypeDef,
    ...
    if inputFilteringMode as libc::c_uint ==
        ...
        TIM_ICInitStructure.TIM_ICFilter = 0x3i32 as uint16_t
    } else { TIM_ICInitStructure.TIM_ICFilter = 0i32 as uint16_t }
...
pub unsafe extern "C" fn pwmRxInit(mut pwmConfig: *const pwmConfig_t) {
    ...
    while channel < 8i32 {
        ...
        if !timer.is_null() {
            (*port).state = 0i32 as uint8_t;
            (*port).missedEvents = 0i32 as uint8_t;
            ...
            IOInit(io, OWNER_PWMINPUT, (channel + 1i32) as uint8_t);
            IOConfigGPIOAF(io,
            ...
            timerConfigure(timer, 0x10000i32 as uint16_t,
            ...
            timerChConfigCallbacks(timer, &mut (*port).edgeCb,
                                   &mut (*port).overflowCb);
            pwmICConfig((*timer).tim, (*timer).channel, 0i32 as uint16_t);
        }
...
pub unsafe extern "C" fn ppmRxInit(mut ppmConfig: *const ppmConfig_t) {
    ppmResetDevice();
    let mut port: *mut pwmInputPort_t =
        &mut *pwmInputPorts.as_mut_ptr().offset(0) as *mut pwmInputPort_t;
    ...
    IOInit(io, OWNER_PPMINPUT, 0i32 as uint8_t);
    IOConfigGPIOAF(io,
                   (GPIO_Mode_AF as libc::c_int | 0i32 << 2i32 |
                        (GPIO_OType_PP as libc::c_int) << 4i32 |
                        (GPIO_PuPd_NOPULL as libc::c_int) << 5i32) as
