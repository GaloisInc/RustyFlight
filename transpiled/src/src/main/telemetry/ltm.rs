...
unsafe extern "C" fn ltm_initialise_packet(mut ltm_id: uint8_t) {
    ltm_crc = 0 as libc::c_int as uint8_t;
...
unsafe extern "C" fn ltm_serialise_16(mut v: uint16_t) {
    ...
    ltm_serialise_8((v as libc::c_int >> 8 as libc::c_int) as uint8_t);
}
unsafe extern "C" fn ltm_serialise_32(mut v: uint32_t) {
    ltm_serialise_8(v as uint8_t);
    ltm_serialise_8((v >> 8 as libc::c_int) as uint8_t);
    ltm_serialise_8((v >> 16 as libc::c_int) as uint8_t);
    ltm_serialise_8((v >> 24 as libc::c_int) as uint8_t);
}
...
unsafe extern "C" fn ltm_gframe() {
    let mut gps_fix_type: uint8_t = 0 as libc::c_int as uint8_t;
    let mut ltm_alt: int32_t = 0;
    if !sensors(SENSOR_GPS as libc::c_int as uint32_t) { return }
    if stateFlags as libc::c_int & GPS_FIX as libc::c_int == 0 {
        gps_fix_type = 1 as libc::c_int as uint8_t
    } else if (gpsSol.numSat as libc::c_int) < 5 as libc::c_int {
        gps_fix_type = 2 as libc::c_int as uint8_t
    } else { gps_fix_type = 3 as libc::c_int as uint8_t }
    ltm_initialise_packet('G' as i32 as uint8_t);
    ltm_serialise_32(gpsSol.llh.lat as uint32_t);
    ltm_serialise_32(gpsSol.llh.lon as uint32_t);
    ltm_serialise_8((gpsSol.groundSpeed as libc::c_int / 100 as libc::c_int)
                        as uint8_t);
    ltm_alt =
        if sensors(SENSOR_RANGEFINDER as libc::c_int as uint32_t) as
               libc::c_int != 0 ||
               sensors(SENSOR_BARO as libc::c_int as uint32_t) as libc::c_int
                   != 0 {
            getEstimatedAltitude()
        } else { (gpsSol.llh.alt) * 100 as libc::c_int };
    ltm_serialise_32(ltm_alt as uint32_t);
    ltm_serialise_8(((gpsSol.numSat as libc::c_int) << 2 as libc::c_int |
                         gps_fix_type as libc::c_int) as uint8_t);
    ltm_finalise();
}
/*
...
unsafe extern "C" fn ltm_sframe() {
    ...
    if flightModeFlags as libc::c_int & PASSTHRU_MODE as libc::c_int != 0 {
        lt_flightmode = 0 as libc::c_int as uint8_t
    } else if flightModeFlags as libc::c_int & GPS_HOME_MODE as libc::c_int !=
                  0 {
        lt_flightmode = 13 as libc::c_int as uint8_t
    } else if flightModeFlags as libc::c_int & GPS_HOLD_MODE as libc::c_int !=
                  0 {
        lt_flightmode = 9 as libc::c_int as uint8_t
    } else if flightModeFlags as libc::c_int & HEADFREE_MODE as libc::c_int !=
                  0 {
        lt_flightmode = 4 as libc::c_int as uint8_t
    } else if flightModeFlags as libc::c_int & BARO_MODE as libc::c_int != 0 {
        lt_flightmode = 8 as libc::c_int as uint8_t
    } else if flightModeFlags as libc::c_int & ANGLE_MODE as libc::c_int != 0
     {
        lt_flightmode = 2 as libc::c_int as uint8_t
    } else if flightModeFlags as libc::c_int & HORIZON_MODE as libc::c_int !=
                  0 {
        lt_flightmode = 3 as libc::c_int as uint8_t
    } else {
        lt_flightmode = 1 as libc::c_int as uint8_t
    } //  current, not implemented
    lt_statemode =
        if armingFlags as libc::c_int & ARMED as libc::c_int != 0 {
            1 as libc::c_int
        } else { 0 as libc::c_int } as uint8_t; // scaled RSSI (uchar)
    if failsafeIsActive() {
        lt_statemode =
            (lt_statemode as libc::c_int | 2 as libc::c_int) as uint8_t
    } // no airspeed
    ltm_initialise_packet('S' as i32 as uint8_t);
    ltm_serialise_16((getBatteryVoltage() as libc::c_int * 100 as libc::c_int)
                         as uint16_t);
    ltm_serialise_16(0 as libc::c_int as uint16_t);
    ltm_serialise_8(constrain(scaleRange(getRssi() as libc::c_int,
                                         0 as libc::c_int,
                                         1023 as libc::c_int,
                                         0 as libc::c_int,
                                         255 as libc::c_int),
                        ...
                        uint8_t);
    ltm_serialise_8(0 as libc::c_int as uint8_t);
    ltm_serialise_8(((lt_flightmode as libc::c_int) << 2 as libc::c_int |
...
unsafe extern "C" fn ltm_aframe() {
    ...
    ltm_serialise_16((attitude.values.pitch as libc::c_int /
                          10 as libc::c_int) as uint16_t);
    ltm_serialise_16((attitude.values.roll as libc::c_int / 10 as libc::c_int)
                         as uint16_t);
    ltm_serialise_16((attitude.values.yaw as libc::c_int / 10 as libc::c_int)
                         as uint16_t);
    ltm_finalise();
}
/*
...
unsafe extern "C" fn ltm_oframe() {
    ...
    ltm_serialise_32(GPS_home[0 as libc::c_int as usize] as
                         uint32_t); // OSD always ON
    ltm_serialise_32(GPS_home[1 as libc::c_int as usize] as uint32_t);
    ltm_serialise_32(0 as libc::c_int as uint32_t);
    ltm_serialise_8(1 as libc::c_int as uint8_t);
...
unsafe extern "C" fn process_ltm() {
    ...
    ltm_aframe();
    if ltm_scheduler as libc::c_int & 1 as libc::c_int != 0 {
        ltm_gframe();
    } else { ltm_sframe(); }
    if ltm_scheduler as libc::c_int == 0 as libc::c_int { ltm_oframe(); }
    ltm_scheduler = ltm_scheduler.wrapping_add(1);
    ltm_scheduler =
        (ltm_scheduler as libc::c_int % 10 as libc::c_int) as uint8_t;
}
#[no_mangle]
pub unsafe extern "C" fn handleLtmTelemetry() {
    ...
    now = millis();
    if now.wrapping_sub(ltm_lastCycleTime) >=
