...
pub static mut adcConfig_Registry: pgRegistry_t =
    unsafe {
        {
            let mut init =
                pgRegistry_s{pgn: (510i32 | 0i32 << 12i32) as pgn_t,
                             size:
        ...
        (adcDeviceByInstance((0x40000000i32 as
                                  uint32_t).wrapping_add(0x10000000i32 as
                                                             libc::c_uint).wrapping_add(0x100i32
                                                                                            as
                                                                                            libc::c_uint)
                                 as *mut ADC_TypeDef) as libc::c_int + 1i32)
            as int8_t;
    (*adcConfig).vbat.enabled = 1i32 != 0;
    (*adcConfig).vbat.ioTag = (0i32 + 1i32 << 4i32 | 4i32) as ioTag_t;
    (*adcConfig).current.enabled = 1i32 != 0;
    (*adcConfig).current.ioTag = (0i32 + 1i32 << 4i32 | 5i32) as ioTag_t;
    (*adcConfig).rssi.enabled = 1i32 != 0;
    (*adcConfig).rssi.ioTag = (1i32 + 1i32 << 4i32 | 2i32) as ioTag_t;
}
// USE_ADC
