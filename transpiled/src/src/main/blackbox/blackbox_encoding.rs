...
pub unsafe extern "C" fn blackboxPrintfHeaderLine(mut name:
    ...
    let written: libc::c_int = blackboxPrintfv(fmt, va.as_va_list());
    blackboxWrite('\n' as i32 as uint8_t);
    blackboxHeaderBudget -= written + 3 as libc::c_int;
}
/* *
...
pub unsafe extern "C" fn blackboxWriteUnsignedVB(mut value: uint32_t) {
    ...
    while value > 127 as libc::c_int as libc::c_uint {
        blackboxWrite((value | 0x80 as libc::c_int as libc::c_uint) as
...
pub unsafe extern "C" fn blackboxWriteS16(mut value: int16_t) {
    blackboxWrite((value as libc::c_int & 0xff as libc::c_int) as uint8_t);
    blackboxWrite((value as libc::c_int >> 8 as libc::c_int &
...
pub unsafe extern "C" fn blackboxWriteTag2_3S32(mut values: *mut int32_t) {
    static mut NUM_FIELDS: libc::c_int = 3 as libc::c_int;
    ...
    while x < NUM_FIELDS {
        ...
        if *values.offset(x as isize) >= 32 as libc::c_int ||
               *values.offset(x as isize) < -(32 as libc::c_int) {
            selector = BITS_32 as libc::c_int;
            break ;
        } else {
            //Require more than 4 bits?
            if *values.offset(x as isize) >= 8 as libc::c_int ||
                   *values.offset(x as isize) < -(8 as libc::c_int) {
                if selector < BITS_6 as libc::c_int {
                    selector = BITS_6 as libc::c_int
                }
            } else if *values.offset(x as isize) >= 2 as libc::c_int ||
                          *values.offset(x as isize) < -(2 as libc::c_int) {
    ...
    match selector {
        0 => {
            blackboxWrite((selector << 6 as libc::c_int |
                               (*values.offset(0 as libc::c_int as isize) &
                                    0x3 as libc::c_int) << 4 as libc::c_int |
                               (*values.offset(1 as libc::c_int as isize) &
                                    0x3 as libc::c_int) << 2 as libc::c_int |
                               *values.offset(2 as libc::c_int as isize) &
        ...
        1 => {
            ...
            blackboxWrite((*values.offset(1 as libc::c_int as isize) <<
                               4 as libc::c_int |
                               *values.offset(2 as libc::c_int as isize) &
                                   0xf as libc::c_int) as uint8_t);
        }
        2 => {
            ...
            blackboxWrite(*values.offset(1 as libc::c_int as isize) as
                              uint8_t);
            blackboxWrite(*values.offset(2 as libc::c_int as isize) as
                              uint8_t);
        }
        3 => {
            /*
         ...
         * 3 - 32 bits
         */
            selector2 = 0 as libc::c_int;
            //Encode in reverse order so the first field is in the low bits:
            let mut x_0: libc::c_int = NUM_FIELDS - 1 as libc::c_int;
            while x_0 >= 0 as libc::c_int {
                selector2 <<= 2 as libc::c_int;
                if *values.offset(x_0 as isize) < 128 as libc::c_int &&
                       *values.offset(x_0 as isize) >= -(128 as libc::c_int) {
                    selector2 |= BYTES_1 as libc::c_int
                } else if *values.offset(x_0 as isize) < 32768 as libc::c_int
                              &&
                              *values.offset(x_0 as isize) >=
                                  -(32768 as libc::c_int) {
                    selector2 |= BYTES_2 as libc::c_int
                } else if *values.offset(x_0 as isize) <
                              8388608 as libc::c_int &&
                              *values.offset(x_0 as isize) >=
                                  -(8388608 as libc::c_int) {
            ...
            //Write the selectors
            blackboxWrite((selector << 6 as libc::c_int | selector2) as
                              uint8_t);
            ...
            while x_1 < NUM_FIELDS {
                match selector2 & 0x3 as libc::c_int {
                    0 => {
                    ...
                    1 => {
                        blackboxWrite(*values.offset(x_1 as isize) as
                                          uint8_t);
                        blackboxWrite((*values.offset(x_1 as isize) >>
...
pub unsafe extern "C" fn blackboxWriteTag2_3SVariable(mut values:
                                                          *mut int32_t)
 -> libc::c_int {
    static mut FIELD_COUNT: libc::c_int = 3 as libc::c_int;
    /*
     * Find out how many bits the largest value requires to encode, and use it to choose one of the packing schemes
     * below:
     *
    ...
    let mut selector2: libc::c_int = 0 as libc::c_int;
    // Require more than 877 bits?
    if *values.offset(0 as libc::c_int as isize) >= 256 as libc::c_int ||
           *values.offset(0 as libc::c_int as isize) < -(256 as libc::c_int)
           || *values.offset(1 as libc::c_int as isize) >= 128 as libc::c_int
           ||
           *values.offset(1 as libc::c_int as isize) < -(128 as libc::c_int)
           || *values.offset(2 as libc::c_int as isize) >= 128 as libc::c_int
           ||
           *values.offset(2 as libc::c_int as isize) < -(128 as libc::c_int) {
        selector = BITS_32_0 as libc::c_int
        // Require more than 554 bits?
    } else if *values.offset(0 as libc::c_int as isize) >= 16 as libc::c_int
                  ||
                  *values.offset(0 as libc::c_int as isize) <
                      -(16 as libc::c_int) ||
                  *values.offset(1 as libc::c_int as isize) >=
        ...
        // Require more than 2 bits?
    } else if *values.offset(0 as libc::c_int as isize) >= 2 as libc::c_int ||
                  *values.offset(0 as libc::c_int as isize) <
                      -(2 as libc::c_int) ||
                  *values.offset(1 as libc::c_int as isize) >=
                      2 as libc::c_int ||
    ...
    match selector {
        ...
        1 => {
            ...
            blackboxWrite((selector << 6 as libc::c_int |
                               ...
                               (*values.offset(1 as libc::c_int as isize) &
                                    0x1f as libc::c_int) >> 4 as libc::c_int)
                              as uint8_t);
            blackboxWrite(((*values.offset(1 as libc::c_int as isize) &
        ...
        2 => {
            ...
            blackboxWrite((selector << 6 as libc::c_int |
                              ...
                              as uint8_t);
            blackboxWrite(((*values.offset(0 as libc::c_int as isize) &
                                0x3 as libc::c_int) << 6 as libc::c_int |
                               (*values.offset(1 as libc::c_int as isize) &
                                    0x7f as libc::c_int) >> 1 as libc::c_int)
                              as uint8_t);
            blackboxWrite(((*values.offset(1 as libc::c_int as isize) &
         ...
         */
            selector2 = 0 as libc::c_int;
            //Encode in reverse order so the first field is in the low bits:
            let mut x: libc::c_int = FIELD_COUNT - 1 as libc::c_int;
            while x >= 0 as libc::c_int {
                selector2 <<= 2 as libc::c_int;
                if *values.offset(x as isize) < 128 as libc::c_int &&
                       *values.offset(x as isize) >= -(128 as libc::c_int) {
                    selector2 |= BYTES_1_0 as libc::c_int
                } else if *values.offset(x as isize) < 32768 as libc::c_int &&
                              *values.offset(x as isize) >=
                                  -(32768 as libc::c_int) {
                    selector2 |= BYTES_2_0 as libc::c_int
                } else if *values.offset(x as isize) < 8388608 as libc::c_int
                              &&
                              *values.offset(x as isize) >=
                                  -(8388608 as libc::c_int) {
            ...
            while x_0 < FIELD_COUNT {
                match selector2 & 0x3 as libc::c_int {
                    ...
                    1 => {
                        blackboxWrite(*values.offset(x_0 as isize) as
                                          uint8_t);
                        blackboxWrite((*values.offset(x_0 as isize) >>
                                           8 as libc::c_int) as uint8_t);
                    }
                    2 => {
                        blackboxWrite(*values.offset(x_0 as isize) as
...
pub unsafe extern "C" fn blackboxWriteTag8_4S16(mut values: *mut int32_t) {
    //Need to be enums rather than const ints if we want to switch on them (due to being C)
    let mut selector: uint8_t = 0 as libc::c_int as uint8_t;
    //Encode in reverse order so the first field is in the low bits:
    let mut x: libc::c_int = 3 as libc::c_int;
    while x >= 0 as libc::c_int {
        selector = ((selector as libc::c_int) << 2 as libc::c_int) as uint8_t;
        if *values.offset(x as isize) == 0 as libc::c_int {
            selector =
                (selector as libc::c_int | FIELD_ZERO as libc::c_int) as
                    uint8_t
        } else if *values.offset(x as isize) < 8 as libc::c_int &&
                      *values.offset(x as isize) >= -(8 as libc::c_int) {
            selector =
                (selector as libc::c_int | FIELD_4BIT as libc::c_int) as
                    uint8_t
        } else if *values.offset(x as isize) < 128 as libc::c_int &&
                      *values.offset(x as isize) >= -(128 as libc::c_int) {
            selector =
    ...
    let mut nibbleIndex: libc::c_int = 0 as libc::c_int;
    let mut buffer: uint8_t = 0 as libc::c_int as uint8_t;
    let mut x_0: libc::c_int = 0 as libc::c_int;
    while x_0 < 4 as libc::c_int {
        match selector as libc::c_int & 0x3 as libc::c_int {
            1 => {
                if nibbleIndex == 0 as libc::c_int {
                    //We fill high-bits first
                    buffer =
                        (*values.offset(x_0 as isize) << 4 as libc::c_int) as
                            uint8_t;
                    nibbleIndex = 1 as libc::c_int
                } else {
                    blackboxWrite((buffer as libc::c_int |
                                       *values.offset(x_0 as isize) &
                                           0xf as libc::c_int) as uint8_t);
                    nibbleIndex = 0 as libc::c_int
                }
            }
            2 => {
                if nibbleIndex == 0 as libc::c_int {
    ...
    if nibbleIndex == 1 as libc::c_int { blackboxWrite(buffer); };
}
/* *
...
pub unsafe extern "C" fn blackboxWriteTag8_8SVB(mut values: *mut int32_t,
    ...
    if valueCount > 0 as libc::c_int {
        //If we're only writing one field then we can skip the header
        if valueCount == 1 as libc::c_int {
            blackboxWriteSignedVB(*values.offset(0 as libc::c_int as isize));
        } else {
            //First write a one-byte header that marks which fields are non-zero
            header = 0 as libc::c_int as uint8_t;
            // First field should be in low bits of header
            let mut i: libc::c_int = valueCount - 1 as libc::c_int;
            while i >= 0 as libc::c_int {
                header =
                    ((header as libc::c_int) << 1 as libc::c_int) as uint8_t;
                if *values.offset(i as isize) != 0 as libc::c_int {
                    header =
                        (header as libc::c_int | 0x1 as libc::c_int) as
                            uint8_t
                }
                i -= 1
            }
            ...
            while i_0 < valueCount {
                if *values.offset(i_0 as isize) != 0 as libc::c_int {
...
pub unsafe extern "C" fn blackboxWriteU32(mut value: int32_t) {
    blackboxWrite((value & 0xff as libc::c_int) as uint8_t);
    blackboxWrite((value >> 8 as libc::c_int & 0xff as libc::c_int) as
                      uint8_t);
    blackboxWrite((value >> 16 as libc::c_int & 0xff as libc::c_int) as
                      uint8_t);
    blackboxWrite((value >> 24 as libc::c_int & 0xff as libc::c_int) as
                      uint8_t);
}
