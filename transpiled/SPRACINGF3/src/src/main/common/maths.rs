...
pub unsafe extern "C" fn atan2_approx(mut y: libc::c_float,
    ...
    if x < 0i32 as libc::c_float { res = 3.14159265358979323846f32 - res }
    if y < 0i32 as libc::c_float { res = -res }
    return res;
}
...
pub unsafe extern "C" fn applyDeadband(value: int32_t, deadband: int32_t)
 -> int32_t {
    if ({ let _x: int32_t = value; (if _x > 0i32 { _x } else { -_x }) }) <
           deadband {
        return 0i32
    }
    return if value >= 0i32 {
               (value) - deadband
           } else { (value) + deadband };
}
#[no_mangle]
pub unsafe extern "C" fn fapplyDeadband(value: libc::c_float,
 ...
 -> libc::c_float {
    if fabsf(value) < deadband { return 0i32 as libc::c_float }
    return if value >= 0i32 as libc::c_float {
               (value) - deadband
           } else { (value) + deadband };
}
#[no_mangle]
...
pub unsafe extern "C" fn devVariance(mut dev: *mut stdev_t) -> libc::c_float {
    return if (*dev).m_n > 1i32 {
               ((*dev).m_newS) / ((*dev).m_n - 1i32) as libc::c_float
           } else { 0.0f32 };
}
#[no_mangle]
...
pub unsafe extern "C" fn normalizeV(mut src: *mut fp_vector,
    ...
    length =
        sqrtf((*src).X * (*src).X + (*src).Y * (*src).Y +
                  (*src).Z * (*src).Z);
    if length != 0i32 as libc::c_float {
        (*dest).X = (*src).X / length;
        (*dest).Y = (*src).Y / length;
        (*dest).Z = (*src).Z / length
    };
...
pub unsafe extern "C" fn buildRotationMatrix(mut delta: *mut fp_angles_t,
    ...
    coszsinx = sinx * cosz;
    sinzsinx = sinx * sinz;
    (*matrix.offset(0))[X as libc::c_int as usize] = cosz * cosy;
    (*matrix.offset(0))[Y as libc::c_int as usize] = -cosy * sinz;
    (*matrix.offset(0))[Z as libc::c_int as usize] = siny;
    (*matrix.offset(1))[X as libc::c_int as usize] =
        sinzcosx + coszsinx * siny;
    (*matrix.offset(1))[Y as libc::c_int as usize] =
        coszcosx - sinzsinx * siny;
    (*matrix.offset(1))[Z as libc::c_int as usize] = -sinx * cosy;
    (*matrix.offset(2))[X as libc::c_int as usize] =
        sinzsinx - coszcosx * siny;
    (*matrix.offset(2))[Y as libc::c_int as usize] =
        coszsinx + sinzcosx * siny;
    (*matrix.offset(2))[Z as libc::c_int as usize] = cosy * cosx;
}
...
pub unsafe extern "C" fn rotateV(mut v: *mut fp_vector,
    ...
    (*v).X =
        v_tmp.X * matrix[0][X as libc::c_int as usize] +
            v_tmp.Y * matrix[1][X as libc::c_int as usize] +
            v_tmp.Z * matrix[2][X as libc::c_int as usize];
    (*v).Y =
        v_tmp.X * matrix[0][Y as libc::c_int as usize] +
            v_tmp.Y * matrix[1][Y as libc::c_int as usize] +
            v_tmp.Z * matrix[2][Y as libc::c_int as usize];
    (*v).Z =
        v_tmp.X * matrix[0][Z as libc::c_int as usize] +
            v_tmp.Y * matrix[1][Z as libc::c_int as usize] +
            v_tmp.Z * matrix[2][Z as libc::c_int as usize];
}
...
pub unsafe extern "C" fn quickMedianFilter3(mut v: *mut int32_t) -> int32_t {
    ...
    let mut i: int32_t = 0;
    i = 0i32;
    while i < 3i32 { p[i as usize] = *v.offset(i as isize); i += 1 }
    if p[0] > p[1] { let mut temp: int32_t = p[0]; p[0] = p[1]; p[1] = temp }
    if p[1] > p[2] {
        let mut temp_0: int32_t = p[1];
        p[1] = p[2];
        p[2] = temp_0
    }
    if p[0] > p[1] {
        let mut temp_1: int32_t = p[0];
        p[0] = p[1];
        p[1] = temp_1
    }
    return p[1];
...
pub unsafe extern "C" fn quickMedianFilter5(mut v: *mut int32_t) -> int32_t {
    ...
    let mut i: int32_t = 0;
    i = 0i32;
    while i < 5i32 { p[i as usize] = *v.offset(i as isize); i += 1 }
    if p[0] > p[1] { let mut temp: int32_t = p[0]; p[0] = p[1]; p[1] = temp }
    if p[3] > p[4] {
        let mut temp_0: int32_t = p[3];
        p[3] = p[4];
        p[4] = temp_0
    }
    if p[0] > p[3] {
    ...
    if p[1] > p[4] {
        let mut temp_2: int32_t = p[1];
        p[1] = p[4];
        p[4] = temp_2
    }
    if p[1] > p[2] {
        let mut temp_3: int32_t = p[1];
        p[1] = p[2];
        p[2] = temp_3
    }
    if p[2] > p[3] {
        let mut temp_4: int32_t = p[2];
        p[2] = p[3];
        p[3] = temp_4
    }
    if p[1] > p[2] {
        let mut temp_5: int32_t = p[1];
        p[1] = p[2];
        p[2] = temp_5
    }
    return p[2];
...
pub unsafe extern "C" fn quickMedianFilter7(mut v: *mut int32_t) -> int32_t {
    ...
    let mut i: int32_t = 0;
    i = 0i32;
    while i < 7i32 { p[i as usize] = *v.offset(i as isize); i += 1 }
    if p[0] > p[5] { let mut temp: int32_t = p[0]; p[0] = p[5]; p[5] = temp }
    if p[0] > p[3] {
        let mut temp_0: int32_t = p[0];
        p[0] = p[3];
        p[3] = temp_0
    }
    if p[1] > p[6] {
        let mut temp_1: int32_t = p[1];
        p[1] = p[6];
        p[6] = temp_1
    }
    if p[2] > p[4] {
        let mut temp_2: int32_t = p[2];
        p[2] = p[4];
        p[4] = temp_2
    }
    if p[0] > p[1] {
        let mut temp_3: int32_t = p[0];
        p[0] = p[1];
        p[1] = temp_3
    }
    if p[3] > p[5] {
        let mut temp_4: int32_t = p[3];
        p[3] = p[5];
        p[5] = temp_4
    }
    if p[2] > p[6] {
        let mut temp_5: int32_t = p[2];
        p[2] = p[6];
        p[6] = temp_5
    }
    if p[2] > p[3] {
        let mut temp_6: int32_t = p[2];
        p[2] = p[3];
        p[3] = temp_6
    }
    if p[3] > p[6] {
        let mut temp_7: int32_t = p[3];
        p[3] = p[6];
        p[6] = temp_7
    }
    if p[4] > p[5] {
        let mut temp_8: int32_t = p[4];
        p[4] = p[5];
        p[5] = temp_8
    }
    if p[1] > p[4] {
        let mut temp_9: int32_t = p[1];
        p[1] = p[4];
        p[4] = temp_9
    }
    if p[1] > p[3] {
        let mut temp_10: int32_t = p[1];
        p[1] = p[3];
        p[3] = temp_10
    }
    if p[3] > p[4] {
        let mut temp_11: int32_t = p[3];
        p[3] = p[4];
        p[4] = temp_11
    }
    return p[3];
...
pub unsafe extern "C" fn quickMedianFilter9(mut v: *mut int32_t) -> int32_t {
    ...
    let mut i: int32_t = 0;
    i = 0i32;
    while i < 9i32 { p[i as usize] = *v.offset(i as isize); i += 1 }
    if p[1] > p[2] { let mut temp: int32_t = p[1]; p[1] = p[2]; p[2] = temp }
    if p[4] > p[5] {
        let mut temp_0: int32_t = p[4];
        p[4] = p[5];
        p[5] = temp_0
    }
    if p[7] > p[8] {
        let mut temp_1: int32_t = p[7];
        p[7] = p[8];
        p[8] = temp_1
    }
    if p[0] > p[1] {
        let mut temp_2: int32_t = p[0];
        p[0] = p[1];
        p[1] = temp_2
    }
    if p[3] > p[4] {
        let mut temp_3: int32_t = p[3];
        p[3] = p[4];
        p[4] = temp_3
    }
    if p[6] > p[7] {
        let mut temp_4: int32_t = p[6];
        p[6] = p[7];
        p[7] = temp_4
    }
    if p[1] > p[2] {
    ...
    if p[4] > p[5] {
        let mut temp_6: int32_t = p[4];
        p[4] = p[5];
        p[5] = temp_6
    }
    if p[7] > p[8] {
        let mut temp_7: int32_t = p[7];
        p[7] = p[8];
        p[8] = temp_7
    }
    if p[0] > p[3] {
        let mut temp_8: int32_t = p[0];
        p[0] = p[3];
        p[3] = temp_8
    }
    if p[5] > p[8] {
        let mut temp_9: int32_t = p[5];
        p[5] = p[8];
        p[8] = temp_9
    }
    if p[4] > p[7] {
        let mut temp_10: int32_t = p[4];
        p[4] = p[7];
        p[7] = temp_10
    }
    if p[3] > p[6] {
    ...
    if p[1] > p[4] {
        let mut temp_12: int32_t = p[1];
        p[1] = p[4];
        p[4] = temp_12
    }
    if p[2] > p[5] {
        let mut temp_13: int32_t = p[2];
        p[2] = p[5];
        p[5] = temp_13
    }
    if p[4] > p[7] {
        let mut temp_14: int32_t = p[4];
        p[4] = p[7];
        p[7] = temp_14
    }
    if p[4] > p[2] {
        let mut temp_15: int32_t = p[4];
        p[4] = p[2];
        p[2] = temp_15
    }
    if p[6] > p[4] {
        let mut temp_16: int32_t = p[6];
        p[6] = p[4];
        p[4] = temp_16
    }
    if p[4] > p[2] {
        let mut temp_17: int32_t = p[4];
        p[4] = p[2];
        p[2] = temp_17
    }
    return p[4];
...
pub unsafe extern "C" fn quickMedianFilter3f(mut v: *mut libc::c_float)
 -> libc::c_float {
    ...
    while i < 3i32 { p[i as usize] = *v.offset(i as isize); i += 1 }
    if p[0] > p[1] {
        let mut temp: libc::c_float = p[0];
        p[0] = p[1];
        p[1] = temp
    }
    if p[1] > p[2] {
        let mut temp_0: libc::c_float = p[1];
        p[1] = p[2];
        p[2] = temp_0
    }
    if p[0] > p[1] {
        let mut temp_1: libc::c_float = p[0];
        p[0] = p[1];
        p[1] = temp_1
    }
    return p[1];
...
pub unsafe extern "C" fn quickMedianFilter5f(mut v: *mut libc::c_float)
 -> libc::c_float {
    ...
    if p[3] > p[4] {
        let mut temp_0: libc::c_float = p[3];
        p[3] = p[4];
        p[4] = temp_0
    }
    if p[0] > p[3] {
    ...
    if p[1] > p[4] {
        let mut temp_2: libc::c_float = p[1];
        p[1] = p[4];
        p[4] = temp_2
    }
    if p[1] > p[2] {
        let mut temp_3: libc::c_float = p[1];
        p[1] = p[2];
        p[2] = temp_3
    }
    if p[2] > p[3] {
        let mut temp_4: libc::c_float = p[2];
        p[2] = p[3];
        p[3] = temp_4
    }
    if p[1] > p[2] {
        let mut temp_5: libc::c_float = p[1];
        p[1] = p[2];
        p[2] = temp_5
    }
    return p[2];
...
pub unsafe extern "C" fn quickMedianFilter7f(mut v: *mut libc::c_float)
 -> libc::c_float {
    ...
    if p[0] > p[3] {
        let mut temp_0: libc::c_float = p[0];
        p[0] = p[3];
        p[3] = temp_0
    }
    if p[1] > p[6] {
        let mut temp_1: libc::c_float = p[1];
        p[1] = p[6];
        p[6] = temp_1
    }
    if p[2] > p[4] {
        let mut temp_2: libc::c_float = p[2];
        p[2] = p[4];
        p[4] = temp_2
    }
    if p[0] > p[1] {
        let mut temp_3: libc::c_float = p[0];
        p[0] = p[1];
        p[1] = temp_3
    }
    if p[3] > p[5] {
        let mut temp_4: libc::c_float = p[3];
        p[3] = p[5];
        p[5] = temp_4
    }
    if p[2] > p[6] {
        let mut temp_5: libc::c_float = p[2];
        p[2] = p[6];
        p[6] = temp_5
    }
    if p[2] > p[3] {
        let mut temp_6: libc::c_float = p[2];
        p[2] = p[3];
        p[3] = temp_6
    }
    if p[3] > p[6] {
        let mut temp_7: libc::c_float = p[3];
        p[3] = p[6];
        p[6] = temp_7
    }
    if p[4] > p[5] {
        let mut temp_8: libc::c_float = p[4];
        p[4] = p[5];
        p[5] = temp_8
    }
    if p[1] > p[4] {
        let mut temp_9: libc::c_float = p[1];
        p[1] = p[4];
        p[4] = temp_9
    }
    if p[1] > p[3] {
        let mut temp_10: libc::c_float = p[1];
        p[1] = p[3];
        p[3] = temp_10
    }
    if p[3] > p[4] {
        let mut temp_11: libc::c_float = p[3];
        p[3] = p[4];
        p[4] = temp_11
    }
    return p[3];
...
pub unsafe extern "C" fn quickMedianFilter9f(mut v: *mut libc::c_float)
 -> libc::c_float {
    ...
    while i < 9i32 { p[i as usize] = *v.offset(i as isize); i += 1 }
    if p[1] > p[2] {
        let mut temp: libc::c_float = p[1];
        p[1] = p[2];
        p[2] = temp
    }
    if p[4] > p[5] {
        let mut temp_0: libc::c_float = p[4];
        p[4] = p[5];
        p[5] = temp_0
    }
    if p[7] > p[8] {
        let mut temp_1: libc::c_float = p[7];
        p[7] = p[8];
        p[8] = temp_1
    }
    if p[0] > p[1] {
        let mut temp_2: libc::c_float = p[0];
        p[0] = p[1];
        p[1] = temp_2
    }
    if p[3] > p[4] {
        let mut temp_3: libc::c_float = p[3];
        p[3] = p[4];
        p[4] = temp_3
    }
    if p[6] > p[7] {
        let mut temp_4: libc::c_float = p[6];
        p[6] = p[7];
        p[7] = temp_4
    }
    if p[1] > p[2] {
    ...
    if p[4] > p[5] {
        let mut temp_6: libc::c_float = p[4];
        p[4] = p[5];
        p[5] = temp_6
    }
    if p[7] > p[8] {
        let mut temp_7: libc::c_float = p[7];
        p[7] = p[8];
        p[8] = temp_7
    }
    if p[0] > p[3] {
        let mut temp_8: libc::c_float = p[0];
        p[0] = p[3];
        p[3] = temp_8
    }
    if p[5] > p[8] {
        let mut temp_9: libc::c_float = p[5];
        p[5] = p[8];
        p[8] = temp_9
    }
    if p[4] > p[7] {
        let mut temp_10: libc::c_float = p[4];
        p[4] = p[7];
        p[7] = temp_10
    }
    if p[3] > p[6] {
    ...
    if p[1] > p[4] {
        let mut temp_12: libc::c_float = p[1];
        p[1] = p[4];
        p[4] = temp_12
    }
    if p[2] > p[5] {
        let mut temp_13: libc::c_float = p[2];
        p[2] = p[5];
        p[5] = temp_13
    }
    if p[4] > p[7] {
        let mut temp_14: libc::c_float = p[4];
        p[4] = p[7];
        p[7] = temp_14
    }
    if p[4] > p[2] {
        let mut temp_15: libc::c_float = p[4];
        p[4] = p[2];
        p[2] = temp_15
    }
    if p[6] > p[4] {
        let mut temp_16: libc::c_float = p[6];
        p[6] = p[4];
        p[4] = temp_16
    }
    if p[4] > p[2] {
        let mut temp_17: libc::c_float = p[4];
        p[4] = p[2];
        p[2] = temp_17
    }
    return p[4];
