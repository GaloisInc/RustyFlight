...
pub unsafe extern "C" fn MemManage_Handler() {
    ...
    let mut requiredStateForTransponder: uint8_t =
        (SYSTEM_STATE_CONFIG_LOADED as libc::c_int |
             SYSTEM_STATE_TRANSPONDER_ENABLED as libc::c_int) as uint8_t;
    if systemState as libc::c_int & requiredStateForTransponder as libc::c_int
           == requiredStateForTransponder as libc::c_int {
        transponderIrDisable();
    }
    ledSet(1 as libc::c_int, 0 as libc::c_int != 0);
    ledSet(0 as libc::c_int, 0 as libc::c_int != 0);
    loop  {
        delay(500 as libc::c_int as timeMs_t);
