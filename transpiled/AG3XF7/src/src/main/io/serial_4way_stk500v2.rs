...
unsafe extern "C" fn ReadBit() -> int8_t {
    ...
    loop  {
        ...
        if micros() > timeout_timer {
            current_block = 10156197871730651190;
            break ;
        }
    }
    match current_block {
    ...
    *bt = 0 as libc::c_int as uint8_t;
    ...
    loop  {
        ...
        if bit as libc::c_int == -(1 as libc::c_int) {
            current_block = 2072376878928743108;
            break ;
        }
        if bit as libc::c_int == 1 as libc::c_int {
            *bt =
    ...
    match current_block {
        2072376878928743108 => { return 0 as libc::c_int as uint8_t }
        _ => {
            ckSumIn =
    ...
    if !(waitcycl == 0 as libc::c_int as libc::c_uint) {
        ...
        loop  {
            ...
            if ReadBit() as libc::c_int == -(1 as libc::c_int) {
                current_block = 14918689971090150265;
                break ;
            }
            i = i.wrapping_add(1)
        }
        match current_block {
            14918689971090150265 => { }
            _ => {
                // learn timing
                HiLoTsh =
                    (LastBitTime >>
                ...
                loop  {
                    ...
                    if bit as libc::c_int == -(1 as libc::c_int) {
                        current_block = 14918689971090150265; // NumTX
                        break ; // NumRX
                    } // RxStartAdr
