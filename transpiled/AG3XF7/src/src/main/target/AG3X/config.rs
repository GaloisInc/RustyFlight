 ...
 * If not, see <http://www.gnu.org/licenses/>.
 */
static mut targetSerialPortFunction: [targetSerialPortFunction_t; 2] =
    [{
         let mut init =
             targetSerialPortFunction_s{identifier: SERIAL_PORT_USART1,
                                        function: FUNCTION_RX_SERIAL,};
         init
     },
     {
         let mut init =
             targetSerialPortFunction_s{identifier: SERIAL_PORT_UART4,
...
pub unsafe extern "C" fn targetConfiguration() {
    targetSerialPortFunctionConfig(targetSerialPortFunction.as_mut_ptr(),
                                   (::core::mem::size_of::<[targetSerialPortFunction_t; 2]>()
                                        as
