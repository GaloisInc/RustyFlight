...
unsafe extern "C" fn ReadBit() -> int8_t {
    ...
    loop  {
        ...
        if micros() > timeout_timer {
            current_block = 1194969416017902197;
            break ;
        }
    }
    match current_block {
...
unsafe extern "C" fn ReadByte(mut bt: *mut uint8_t) -> uint8_t {
    ...
    *bt = 0 as libc::c_int as uint8_t;
    ...
    match current_block {
        16754986508692159943 => { return 0 as libc::c_int as uint8_t }
        _ => {
            ckSumIn =
    ...
    if !(waitcycl == 0 as libc::c_int as libc::c_uint) {
        ...
        loop  {
            ...
            if ReadBit() as libc::c_int == -(1 as libc::c_int) {
                current_block = 11096047566704707238;
                break ;
            }
            i = i.wrapping_add(1)
        }
        match current_block {
            11096047566704707238 => { }
            _ => {
                // learn timing
                HiLoTsh =
                    (LastBitTime >>
                ...
                loop  {
                    ...
                    if bit as libc::c_int == -(1 as libc::c_int) {
                        current_block = 11096047566704707238; // NumTX
                        break ; // NumRX
                    } // RxStartAdr
